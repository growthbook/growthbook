...
The primary way to create metrics in GrowthBook is with **Fact Tables** where you write SQL to pull a set of rows and then define multiple metrics on top of those rows. There is also a [legacy method](/metrics/legacy-metrics) where each metric has its own self-contained SQL. Fact Tables are easier to use, faster to query, and more powerful, so we highly recommend using them for all new metrics.

If you learn best by example, you can jump straight into [Metric Examples and Use Cases](/metrics/metric-examples).
...
Quantile Metrics are useful when you care about comparing variations at a quantile (e.g. latency at P99) rather than the mean. These are also called percentile metrics. Learn more about when to use Quantile Metrics [here](https://docs.growthbook.io/statistics/quantile).
...
<Frame>
![Marking an Event Type column](https://docs.growthbook.io/images/metrics/inline-filter.png)
</Frame>
...
<Frame>
![Event Type Filter UI](https://docs.growthbook.io/images/metrics/event-type-filter-metric-form.png)
</Frame>
...
<Frame>
![Metric Windows](https://docs.growthbook.io/images/metric-windows.png)
</Frame>
...
<Frame>
![Metric Windows (User 2)](https://docs.growthbook.io/images/metric-windows-user-2.png)
</Frame>
...
1. Navigate to your Fact Table that has the metrics and the columns by which you want to slice. Ensure the Fact Table SQL is returning the relevant column. If it isn't you can click "Edit SQL" on the Fact Table page, add your column (e.g. `product_type`) to the SELECT statement, and then press "Confirm Changes". Only **string** and **boolean** columns can be used.
  ...
  <img src="https://docs.growthbook.io/images/metrics/metric-dim-config.png" alt="Configuring a column for metric slicing" />
...
2. With your column (`product_type`) in the Fact Table SQL, you now edit the column property in the "Columns" table. Click the edit icon next to your column and then check the "Enable Auto Slices" checkbox in the window that pops up.
  ...
  <img src="https://docs.growthbook.io/images/metrics/metric-dim-levels.png" alt="Configuring slice levels" />
...
- Manually when you click **Update** in the column modal
  ...
  ![Fact Table modal with auto-update slice levels checkbox](https://docs.growthbook.io/images/metrics/auto-slice-updates-fact-table.png)
...
![Column modal showing slice levels with Lock checkboxes](https://docs.growthbook.io/images/metrics/auto-slice-levels-locking.png)
...
Custom slices can be created using any non-user-identifier string or boolean column in your Fact Table. (Custom slicing is not limited to auto-slices columns.)
  ...
  <img src="https://docs.growthbook.io/images/metrics/custom-slices.png" alt="Adding custom slices" />
...
If you click the arrow icon next to "revenue" on the experiment results table, you will see these slices expand under the base metric row (revenue), each slice materialized as its own sub-row. You also can use the Filter button at the top of the results to filter all metrics by slices, or click in to a row for the [Metric Drilldown](/experimentation-analysis/experiment-results#metric-drilldown) to view slices with search, sorting, and time-series analysis capabilities.

  <img src="https://docs.growthbook.io/images/metrics/metric-dim-results.png" alt="Metric slice results" />
...
For more advanced analysis, use our [Product Analytics](/product-analytics) feature.
...
Read our dedicated guide on [Query Optimization](/experimentation-analysis/query-optimization) for best practices on reducing query costs across GrowthBook.
