...
[Feature flags](/features) are a very powerful developer tool. They give you deep control over how and when new functionality is released to your users.
...
Read more about [features here](/features).

## Experiment Analysis

GrowthBook needs to connect to your [SQL Data Warehouse](/experimentation-analysis/connecting-to-your-data-warehouse) in order to query experiment results. GrowthBook is extremely flexible and can support almost any schema structure with a little bit of configuration.
...
Once connected to a data source, you need to define [Fact Tables and Metrics](/metrics/metrics). Fact Tables are SQL queries that select raw data from your warehouse. Metrics are built on top of these Fact Tables and define what you want to measure in your experiments. For example, you might have an **Orders** fact table that selects one row per order, and then metrics for **Average Order Value**, **Total Revenue**, and **Number of Orders**.
...
Your data team can drill down into results by custom [dimensions](/experimentation-analysis/dimensions), view the raw SQL that GrowthBook is running on your data warehouse, and export results to a Jupyter notebook for even deeper analysis.
...
Learn how to get started with GrowthBook](/quick-start)[### Self-Host

Learn about running GrowthBook on your own infrastructure](/self-host)[### SDK Docs

Learn about the GrowthBook SDKs](/lib)
