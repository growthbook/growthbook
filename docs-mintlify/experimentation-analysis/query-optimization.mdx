...
Read more about [SQL Templates](/experimentation-analysis/sql-templates) and the different variables that are available.
...
Read more about [Fact Tables](/metrics/metrics) and how to [convert legacy metrics to Fact Tables](/metrics/legacy-metrics#migrating-legacy-metrics-to-fact-tables).
...
This optimization is available on Enterprise plans and is automatically enabled for the [Managed Warehouse](/experimentation-analysis/managed-warehouse) on all plans, since GrowthBook owns the compute.
...
In all other cases, the optimization runs automatically — no configuration required. You can confirm it ran by expanding an experiment's queries and looking for the **Fact Table Optimized** badge on combined queries.
...
Read more about [Data Pipeline Mode](/experimentation-analysis/data-pipeline).
...
Read more about this approach with some [examples and best practices](/metrics/metric-examples#pre-aggregated-tables).
