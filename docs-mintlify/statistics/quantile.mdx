...
Pro**Quantile Metrics** is available on Pro and Enterprise plans.
...
1. Navigate to your [Fact Table](/metrics/metrics) and select "Add Metric".
...
1. Can I pick any quantile level (e.g., P99.999)? No - the maximum range available is [0.001, 0.999], and that is only for experiments with large sample sizes (i.e., n > 3838). This is because inference can become unreliable for extreme quantile levels and small sample sizes. In general, if you want to compare quantiles at some value p∈(0,1)p \in (0, 1)p∈(0,1), and you want a 95% confidence interval, your sample size $n$ must be bigger than 4p/(1−p)4p/(1-p)4p/(1−p). Similarly, if you want extreme and small quantiles, you need n≥4(1−p)/pn \geq 4(1-p)/pn≥4(1−p)/p. For p=0.99p=0.99p=0.99 and p=0.01p=0.01p=0.01, this corresponds to n≥380n \geq 380n≥380.
...
4. How does quantile inference connect to mean inference? If you average the quantiles of a distribution, you get the distributional mean. That is, the average of $\{P1, P2, ..., P98, P99\}$ equals the mean of the distribution. Similarly, the average of the treatment effects at P1, P2, etc. equals the mean treatment effect. So quantile inference can be viewed as a decomposition of mean inference.
...
[here](#should-i-aggregate-by-experiment-user-before-taking-quantile)). Our implementation is based upon Algorithm 1 of [Yao, Li and Lu (2024)](https://arxiv.org/pdf/2401.14549.pdf). Define $\nu \in (0,1)$ as the quantile level of interest. Define $\alpha \in (0,1)$ as the false positive rate, and let $Z_{1-\alpha/2}$ be its associated critical value. Without loss of generality we focus on the control variation. Let $n$ be the control sample size. Define $Y_{ij}$ as the webpage latency for the $j$th session of the $i$th user (that is, cluster) in control. Define the observed control outcomes as $\{Y_1, Y_2, ..., Y_n\}$, where $n = \sum_{i=1}^{K} N_i$. Define the ordered control outcomes as $\{Y_{(1)}, Y_{(2)}, ..., Y_{(n)}\}.$

1. Compute $L, U = n\left(\nu \pm Z_{1-\alpha/2}\sqrt{\nu(1-\nu)/n}\right)$.
2. Fetch $Y_{n\nu}$, $Y_L$, and $Y_U$.
3. Compute $I_{ij} = 1\{Y_{ij} \le Y_{n\nu}\}$. Define $\bar{I} = n^{-1} \sum_{i=1}^{K} \sum_{j=1}^{N_i} I_{ij}$.
...
5. Define $\sigma_{I,c}^{2} = \mathrm{Var}(\bar{I})$ using the variance of ratios of means described below.
6. Compute $\sigma_{iid}^{2} = \left(\frac{Y_U - Y_L}{2Z_{1-\alpha/2}}\right)^2$.
7. The cluster-adjusted variance is $\sigma_{iid}^{2} \left(\sigma_{I,\text{iid}}^{2}/\sigma_{I,c}^{2}\right)$. The term in parentheses adjusts the variance for clustering. If there is no clustering (that is, inference is at the user level), then use $\sigma_{iid}^{2}$.

To further speed this algorithm, instead of finding the exact $(L, U)$, which requires pre-computing $n$ inside SQL, we instead construct a sequence of logarithmically increasing sample sizes $N^{\star} = \{n_1, n_2, ..., n_M\}$ and their associated intervals $\{(L_1,U_1), (L_2,U_2), ..., (L_M,U_M)\}$. We then output the quantiles associated with each interval, as well as $n$. We then find $n^{\star}$, defined as the largest element of $N^{\star}$ such that $n^{\star} \le n$. We then adjust the variance outputted by the algorithm above by a factor of $n^{\star}/n$. Currently we use 20 different values of $N^{\star}$, where the $k$th value is equal to $100 * 2^{k-1}$.

In this paragraph we describe the variance of a ratio of means, as described in step 5 above. Additionally, this is the standard formula used to calculate variance of a mean for a variation in GrowthBook t-tests. Here we define the user outcome in terms of the random variable $X_i$, as the formula below can be used for any outcome (for example, $Y_{ij}$ or $I_{ij}$). For the $i$th user define the sum of outcomes as $S_i = \sum_{j=1}^{N_i} Y_{ij}$. Then the mean outcome across users is $\bar{X} = \frac{\sum_{i=1}^{K} S_i}{\sum_{i=1}^{K} N_i}$. Define the mean sum of outcomes across users as $\bar{S} = K^{-1} \sum_{i=1}^{K} S_i$. Define the mean count across users as $\bar{N} = K^{-1} \sum_{i=1}^{K} N_i$. A formula for the variance of $\bar{X}$ is

$$\mathrm{Var}(\bar{X}) = \frac{1}{K\bar{N}^{2}}\left[\mathrm{Var}(S) - 2\frac{\bar{S}}{\bar{N}}\mathrm{Cov}(S,N) + \frac{\bar{S}^2}{\bar{N}^2}\mathrm{Var}(N)\right]$$

This is similar to the delta method approximation of the variance we use for ratio metrics and for relative effects.

Let $\hat{\mu}_{C,n\nu}$ be the $\nu$th sample quantile for control, and let its associated variance be $\hat{\sigma}^2_{C,n\nu}$.
Analogously define $\hat{\mu}_{T,n\nu}$ and $\hat{\sigma}^2_{T,n\nu}$ for treatment. These quantities are plugged into our lift estimators as described in the [Statistical Details](/statistics/details) page. The result
