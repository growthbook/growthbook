...
Pro**Sequential Testing** is available on Pro and Enterprise plans.
...
When running a frequentist analysis, the experimenter sets a confidence level, often written as $\alpha$. Many people, and GrowthBook, default to using α=5%\alpha = 5\%α=5% (sometimes it is written as α=0.05\alpha = 0.05α=0.05). These all mean the same thing: for a correctly specified frequentist analysis, you will only reject the null hypothesis 5% of the time when the null hypothesis is true. Usually in online experimentation, our null hypothesis is that metric averages in two variations are the same. In other words, $\alpha$ controls the _False Positive Rate_.
...
<Frame>
![Sequential Statistics Organization Settings](https://docs.growthbook.io/assets/images/sequential-settings-ad52629ca2b06243260394a00ad5ec29.png)
</Frame>
### Experiment settings
...
<Frame>
![Sequential Statistics Experiment Settings](https://docs.growthbook.io/assets/images/sequential-exp-settings-231190d9bbc36c9ca09d4d1ef25d29a7.png)
</Frame>
...
$$\hat{\mu} \pm \hat{\sigma}\sqrt{N}\sqrt{\frac{2(N\rho^2 + 1)}{N^{2}\rho^2}\log\left(\frac{\sqrt{N\rho^2 + 1}}{\alpha}\right)}$$

$$\rho = \sqrt{\frac{-2\log(\alpha) + \log(-2 \log(\alpha) + 1)}{N^*}}$$

In the above, $\hat{\mu}$ is our estimated uplift, $\hat{\sigma}$ is the estimated standard error of the uplift, $\alpha$ is our significance level (defaults to 0.05), and $N$ is the sum of the two variations' sample sizes.
