...
If your CDN supports invalidating URLs with simple HTTP PURGE requests, you can add an [SDK Webhook](/webhooks/sdk-webhooks) in GrowthBook and set the method as PURGE with no payload.

### Other

If the methods above are not available for your CDN, you can configure a custom [SDK Webhook](/webhooks/sdk-webhooks). We let you customize the HTTP method and headers, but if you need more control, you may need to set up a custom script that sits between the webhook and the CDN.
