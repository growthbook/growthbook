...
<Frame>
![Experiment rule UI](https://docs.growthbook.io/assets/images/feature-experiment-rules-e828dc2e35cd683a0110a4c133f6a449.png)
</Frame>
...
## Editing Feature Values from the Experiment Page

For experiments linked to a feature flag, you can edit the feature values served to each variation directly from the experiment page without navigating back to the feature. This is useful when you spot a wrong value during pre-launch review or need to add, remove, or rename variations before starting the experiment.

From the **Linked Features** section on the experiment page, open the feature flag's actions menu and select **Edit Feature Values**. From the modal you can:

- Edit the value served for each variation
- Add or remove variations — split weights auto-rebalance to sum to 100%
- Click **Set equal splits** to reset all weights to an even distribution

Saving changes creates a new draft revision on the linked feature. If [approval flows](/features/publishing-and-approval-flows) are enabled, the draft is published through your normal review process. If the experiment rule isn't live yet, edits apply to the existing draft instead of creating a new one.

<Info>
You can't edit feature values from the experiment page once the experiment has an approved scheduled start. Cancel the scheduled start or edit values directly on the feature page before scheduling.
</Info>
...
a link to view experiment results on the bottom of each experiment override rule. Read more about [experiment results](/experimentation-analysis/experiment-results).
