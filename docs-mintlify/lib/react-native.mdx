...
**React Native SDK Resources** · `v1.6.5`
...
- [React Native CLI example app](https://github.com/growthbook/examples/tree/main/react-native-cli)
...
Sticky bucketing ensures that users see the same experiment variant, even when user session, user login status, or experiment parameters change. See the [Sticky Bucketing docs](/sticky-bucketing) for more information. If your organization and experiment supports sticky bucketing, you must implement an instance of the `StickyBucketService` to use Sticky Bucketing.
...
Alternatively, CryptoJS ([crypto-js](https://www.npmjs.com/package/crypto-js)) provides a synchronous API:
