...
**Cloudflare Workers App & SDK Resources** · `v0.2.8`

- [edge-cloudflare](https://github.com/growthbook/growthbook-proxy/tree/main/packages/lib/edge-cloudflare)
- [npm](https://www.npmjs.com/package/@growthbook/edge-cloudflare)
- [Example worker](https://github.com/growthbook/growthbook-proxy/tree/main/packages/lib/edge-cloudflare/example)
...
1. Our turnkey Edge App
   - Automatically run server-side or hybrid [Visual Experiments](/visual-editor) without redraw flicker.
   - Automatically run server-side or hybrid [URL Redirect Experiments](/running-experiments/url-redirects) without flicker or delay.
...
```bash Yarn
yarn add @growthbook/edge-cloudflare
```

```bash pnpm
pnpm add @growthbook/edge-cloudflare
```

```bash Bun
bun add @growthbook/edge-cloudflare
```
...
1. Create an [SDK Webhook](/webhooks/sdk-webhooks) on the same SDK Connection that you are using for edge integration. You do not need to worry about the receiving end of the webhook (verifying GrowthBook signatures, etc).
...
Case Insensitive Regex≥ v0.2.7

Case Insensitive Membership≥ v0.2.7

Saved Group References≥ v0.1.12

Encrypted Features≥ v0.1.11

Streaming≥ v0.1.11

v2 Hashing≥ v0.1.11

Visual Editor≥ v0.1.11

SemVer Targeting≥ v0.1.11

Visual Editor (JS)≥ v0.1.11

Visual Editor Drag & Drop≥ v0.1.11

Prerequisites≥ v0.1.11

URL Redirects≥ v0.1.11

Sticky Bucketing≥ v0.1.11
