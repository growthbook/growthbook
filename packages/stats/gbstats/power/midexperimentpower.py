...
class MidExperimentPower(MidExperimentPowerBase):
    # case where scaling factor of 0 (i.e., no additional users) is sufficient
    @property
    def return_scaling_factor_zero(self) -> bool:
        return self.already_powered
    ...
    def calculate_scaling_factor(self) -> ScalingFactorResult:
        ...
        if self.return_scaling_factor_zero:
...
class MidExperimentPowerUseExperimentData(MidExperimentPower):
    @property
    ...
    @property
    def return_scaling_factor_zero(self) -> bool:
        return self.already_significant

    def power(self, scaling_factor: float) -> float:
        """
        Args:
            scaling_factor: multipicative factor for sample size.
            target_mde: postulated effect size.
        ...
        if self.sequential:
            ...
            v = MidExperimentPowerUseExperimentData.final_posterior_variance(
                self.sigma_2_posterior, self.sigmahat_2_delta, scaling_factor
            )
            s = np.sqrt(v)
            halfwidth = norm.ppf(1 - self.alpha / (2 * self.num_tests)) * s
        marginal_var = (
            MidExperimentPowerUseExperimentData.marginal_variance_delta_hat_prime(
                self.sigma_2_posterior, self.sigmahat_2_delta, scaling_factor
            )
        )
    ...
    def calculate_scaling_factor(self) -> ScalingFactorResult:
        ...
        scaling_factor_lower, converged_lower = self.calculate_scaling_factor_bound(
            upper=False
        )
        ...
        scaling_factor_upper, converged_upper = self.calculate_scaling_factor_bound(
            upper=True
        )
    ...
    def calculate_scaling_factor_bound(self, upper=True) -> Tuple[float, bool]:
        """
