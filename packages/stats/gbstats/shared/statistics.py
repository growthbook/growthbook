...
@dataclass
class ClusteredMeanStatistic(Statistic):
    sum: float
    sum_squares: float
    sum_cluster_sizes_squared: float
    cluster_sizes_cluster_sums_sum_of_products: float
    ...
    def variance(self):
        ...
        return (
            self.sum_squares 
            - 2 * self.mean * self.cluster_sizes_cluster_sums_sum_of_products
            + pow(self.mean, 2) * self.sum_cluster_sizes_squared
        )
