...
class TestEffectBayesianABTest(TestCase):
    def test_bayesian_effect_ab_test(self):
        ...
        b_improper_flat = EffectBayesianABTest(
            moment_result_absolute, effect_config_improper_flat
        ).compute_result()
        b_flat = EffectBayesianABTest(
            moment_result_absolute, effect_config_flat
        ).compute_result()
        b_relative_flat = EffectBayesianABTest(
            moment_result_relative, effect_config_flat_rel
        ).compute_result()
        b_informative = EffectBayesianABTest(
            moment_result_absolute, effect_config_inf
        ).compute_result()
        b_relative_informative = EffectBayesianABTest(
            moment_result_relative, effect_config_inf_rel
        ).compute_result()
        ...
        abs_result = EffectMoments(
            q_stat_c, q_stat_t, EffectMomentsConfig(difference_type="absolute")
        ).compute_result()

        b_flat = EffectBayesianABTest(abs_result, effect_config_flat).compute_result()
...
class TestGaussianEffectRelativeAbsolutePriors(TestCase):
    def test_bayesian_effect_relative_effect(self):
        ...
        rel_result = EffectMoments(
            stat_c, stat_t, EffectMomentsConfig(difference_type="relative")
        ).compute_result()

        abs_test = EffectBayesianABTest(abs_result, abs_config_inf)
        rel_test = EffectBayesianABTest(rel_result, rel_config_inf)
