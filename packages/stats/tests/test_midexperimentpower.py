...
class TestMidExperimentPower(TestCase):
    def setUp(self):
        ...
        self.moments_result = EffectMoments(
            [(self.stat_a, self.stat_b)], EffectMomentsConfig()
        ).compute_result()
        ...
        self.m_freq = MidExperimentPower(
            self.moments_result, self.res_freq, self.config, self.power_config_freq
        )
        self.m_seq = MidExperimentPower(
            self.moments_result, self.res_seq, self.config, self.power_config_seq
        )
