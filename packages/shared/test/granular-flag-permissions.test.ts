  ...
  describe("pre-merge access is preserved", () => {
    const BASELINE: {
      ...
      },
      {
        ...
        model: "feature",
        // No "revert": deliberately not preserved. Legacy revert needed one atom
        // from EACH policy, which independent grants cannot express — putting
        // the revert atom on either policy would escalate it alone. Under-grant
        // fails closed; an admin can add FlagsRevert.
    ...
    it("does not let the deployment-only legacy role revert", () => {
      // Legacy revert demanded manageFeatures, which SDKPayloadPublish never
      // carried — revert here would be new authority, not preserved.
