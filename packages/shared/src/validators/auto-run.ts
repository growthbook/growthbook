...
export const autoRunArtifactKinds = [
  "sdk-connection",
  "feature",
  "experiment",
  "attribute",
...
export const autoRunMetadata = z.record(z.string().max(60), metadataValue);

// `by` drives the "you created this" / "we set this up for you" split on the page.
// It cannot be inferred later, so it is required.
export const autoRunArtifact = z
  .object({
    kind: z.enum(autoRunArtifactKinds),
...
export type AutoRunInterface = z.infer<typeof autoRunValidator>;
export type AutoRunArtifact = z.infer<typeof autoRunArtifact>;
export type AutoRunMetadata = z.infer<typeof autoRunMetadata>;
...
export function autoRunMetaString(
  metadata: AutoRunMetadata,
  key: string,
): string | null {
...
export const apiAutoRunInterface = namedSchema(
  "AutoRun",
  apiBaseSchema
    .extend({
      source: z.enum(autoRunSources),
      ...
      metadata: autoRunMetadata.describe(
      ...
      artifacts: z.array(apiAutoRunArtifact),
      checks: z.array(autoRunCheck),
      outcome: z.enum(autoRunOutcomes).nullable(),
...
export type ApiAutoRun = z.infer<typeof apiAutoRunInterface>;

export const apiCreateAutoRunBody = z
  .strictObject({
    source: z.enum(autoRunSources).optional(),
    agent: z.string().max(50).optional(),
    metadata: autoRunMetadata.optional(),
  })
  .describe("Open an auto run. Everything it creates is appended afterwards");
...
export const apiUpdateAutoRunBody = z
  .strictObject({
    metadata: autoRunMetadata.optional(),
    checks: z.array(autoRunCheck).max(50).optional(),
    outcome: z.enum(autoRunOutcomes).optional(),
    failureReason: z.string().max(1000).optional(),
  })
  .describe("Record the outcome of an auto run");
...
export const apiAppendAutoRunArtifactBody = autoRunArtifact
