import { NamespaceValue } from "back-end/src/validators/shared";
...
): namespace is { enabled: boolean; name: string; range: [number, number] } {
  return "range" in namespace && !("ranges" in namespace);
}

/**
 * Check if a namespace is using the new format (multiple ranges)
 */
export function isNewNamespaceFormat(namespace: NamespaceValue): namespace is {
  enabled: boolean;
  name: string;
...
export function getNamespaceHashAttribute(
  ...
  if (isNewNamespaceFormat(namespace) && namespace.hashAttribute) {
