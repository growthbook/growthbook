...
- **SSO_CONFIG** - A JSON-encoded string that configures SSO (using OpenID Connect). It should be an object with the following keys:
  - `clientId` (string)
  - `implicitGrant` (boolean, optional) - defaults to false (PKCE flow)
  - `metadata` (object)
    - `issuer` (string)
    ...
    - `code_challenge_methods_supported` (array of strings) - Required unless `implicitGrant` is set to true
  - `emailDomain` (string, optional) - Allow auto-joining from a specified email domain
  - `extraQueryParams` (object, optional) - Dictionary of query params to be passed along with every OAuth call

For SSO, make sure the following callback URLs are whitelisted:

- `{APP_ORIGIN}/oauth/callback`
- `{APP_ORIGIN}/oauth/silent`
