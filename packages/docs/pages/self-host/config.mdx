...
params: ...
settings:
  # The different types of supported user ids
  userIdTypes:
    - userIdType: user_id
      description: Logged-in user id
    - userIdType: anonymous_id
...
ignoreNulls: false
# Which user id types are supported for this metric
userIdTypes:
  - user
...
Plus, you need to select a column for each user id type the metric supports.
...
Dimensions only have 4 properties: name, datasource, userIdType, and SQL. The SQL query must return two columns: the user id type and `value`.
