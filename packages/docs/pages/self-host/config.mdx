...
For count, duration, and revenue metrics, you can sum a numeric property:

```yml
# A "Revenue per user" metric
type: revenue
table: Purchases
# If a user has multiple "Purchases" events, this property will be summed together
column: event.properties.grand_total
```

For count metrics, we also support the equivalent of `COUNT DISTINCT` in sql:

```yml
type: count
# The event name
table: PDF Downloads
# Javascript expression that we can to count
column: event.properties.file
# Tell it do a distinct count
countDistinct: true
```
