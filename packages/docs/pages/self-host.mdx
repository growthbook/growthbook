...
In addition, you can specify your data sources and metrics in a `config/config.yml` file. This is entirely optional and meant for teams that prefer storing these settings in files vs defining them within the Growth Book UI (the defualt behavior).
...
datasources:
  warehouse:
    ...
    settings:
      queries:
        usersQuery: SELECT user_id, anonymous_id FROM identifies
        ...
metrics:
  signups:
    type: binomial
