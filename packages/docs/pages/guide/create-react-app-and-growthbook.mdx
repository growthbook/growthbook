...
<div class="bg-blue-50 py-3 mb-5 px-4 text-gray-600 border-l-8 border-blue-200 dark:bg-blue-800 dark:text-gray-50">
  ...
  <pre>if (feature.on) ...</pre>and then you run an A/B test within GrowthBook
  to turn the feature on for 50% of users and off for the other 50% (or whatever
  percentage you like).
</div>
...
trackingCallback in `src/index.js`, instead of doing a `console.log`, you could use Mixpanel, Rudderstack, Jitsu,
