...
```ts
  ...
  // The encryption key is stored in the DB as a base64 encoded string, and needs to be converted to an ArrayBuffer before calling SubtleCrypto's importKey() method, which returns a CryptoKey that can be consumed by the decrypt() method.
  ...
  async function decryptFeatures({ features, iv }) {
    // This can be found in your account under /settings/keys
    const key = await getKeyFromString(
      "o7AolAMiEgl9PvBS63O/pNDF8tvg9mxAt330FU71n9c="
    );

    // ciphertext and iv come in as base64 encoded strings and must be converted to ArrayBuffers
    const ciphertext = stringToBuffer(atob(features));
    iv = stringToBuffer(atob(iv));

    const decryptedFeatures = await crypto.subtle.decrypt(
      {
        name: "AES-CBC",
        iv,
      },
    ...
    const decodedFeatures = decoder.decode(decryptedFeatures);
  ...
  fetch("https://cdn.growthbook.io/api/features/<your api key>")
    ...
    .then(async (json) => {
        growthbook.setFeatures(
          JSON.parse(await decryptFeatures(json.encrpytedFeatures))
        );
      }
    })
    .catch(() => {
