...
- **experiment** (`Experiment` or `null`) - When source is "experiment", this will be the Experiment object used
- **experimentResult** (`ExperimentResult` or `null`) - When source is "experiment", this will be the experiment result
...
### inNamespace(id, namespace): boolean

This checks if a user id is within an experiment namespace or not.

The `namespace` argument is a tuple with 3 parts: name (string), start (float), and end (float).

1. Hash the id and namespace name
   ```ts
   n = hash(id + namespace[0]);
   ```
...
### getEqualWeights(numVariations): float[]
...
### getBucketRanges(numVariations, coverage, weights): [float, float][]
...
### chooseVariation(n, ranges): integer
...
### getQueryStringOverride(id, url, numVariations): null|integer
...
### public evalCondition(attributes, conditionObj): boolean
...
### private evalOr(attributes, conditionObjs): boolean
...
### private evalAnd(attributes, conditionObjs): boolean
...
### private getPath(attributes, path): any
...
### public getExperimentResult(experiment, variationIndex, inExperiment): ExperimentResult
...
### public feature(key): FeatureResult
...
### public run(experiment): ExperimentResult
