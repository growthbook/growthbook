...
3. If `isOperatorValue(condition)`
...
- `$in` - attributeValue in the conditionValue array
- `$nin` - attributeValue not in the conditionValue array
- `$all`
  1. If attributeValue is not an array, return false
...
There are 3 operators where attributeValue is an array:

- `$elemMatch` - return `elemMatch(conditionValue, attributeValue)`
- `$size`
  1.  If attributeValue is not an array, return false
...
- `$type` - return `getType(attributeValue) == conditionValue`
- `$not` - recursive rule. Return `!evalConditionValue(conditionValue, attributeValue)`
...
There are language-agnostic test suites stored as JSON files in `packages/sdk-js/test/cases/`. This covers things like hashing, feature rules, variation assignment, and evaluating conditions. The files are arrays of arrays. Each test case has the following structure

- `mongrule.json` - tests for `evalCondition`
  - name of the test case (string)
  - expected return value (boolean)
  - condition
  - attributes
  - whether or not a logged error is expected (boolean, optional, default false)
- `features.json` - coming soon
- `experiments.json` - coming soon
- `fnv.json` - coming soon
