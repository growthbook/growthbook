...
- **condition** (`Condition`) - Optional targeting condition
- **force** (`any`) - Immediately force a specific value (ignore every other option besides condition)
...
2. Loop through the feature rules (if any)
   ...
   2. If `rule.force` is set, return immediately (value = forced value, source = `force`)
...
Targeting conditions are defined using a subset of the MongoDB query syntax. This makes it human readable/writeable while still being relatively easy to parse.
...
Look at the source of the Javascript SDK (specifically [mongrule.ts](https://github.com/growthbook/growthbook/tree/main/packages/sdk-js/src/mongrule.ts)) for a reference implementation (~200 lines of code total).
