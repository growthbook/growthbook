...
Override rules are defined separately for each environment (dev and production). This way you can, for example, test an experiment rule in dev first before deploying it to production.
...
If you have multiple experiments that may conflict with each other (e.g. background color and text color), you can use **namespaces** to make the conflicting experiments mutually exclusive. Namespaces cover a range from 0 to 1 and experiments are allocated to subsets of this range. As long as two ranges do not overlap, a user will only ever be included in one of the experiments.

![Namespaces](/images/namespaces.png)

Before you can use this in an experiment, you must define your namespaces under Management -> Namespaces.

## Integration Options

There are few ways to integrate GrowthBook features into your application.

At a high level, the features you create in the GrowthBook UI can be rendered as a single JSON feature definitions file for each environment (dev and production). The contents of this JSON file must be passed into the SDK. Any feature flags evaluated before the JSON file loads will have a value of `null`, so it's important to load the features quickly.
...
**Note:** There are separate API keys for dev and production. Make sure you are using the right one for your environment.
