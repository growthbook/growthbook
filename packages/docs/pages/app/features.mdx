...
```js
// Either the user's name starts with "john" OR they are over 65 and have a kid who's a doctor
{
  "$or": [
    {
      "name": { "$regex": "^john" }
...
Force rules are most useful when you already know exactly what value you want to assign to each user.

### Rollout Rules

The other type of rule is a percentage rollout. This randomly splits users into buckets, assigns them different values, and tracks that assignment in your data warehouse or analytics tool.

Rollouts are most useful when you aren't sure which value to assign yet (A/B test) or you want to make sure a new feature doesn't break anything (gradually release).

Here's what a Percent Rollout rule looks like in the GrowthBook UI:

![Rollout rule](/images/rollout-rule.png)
...
If the total rollout values add up to less than 100%, the remaining users will skip the rule and fall through to the next matching one (or the default value) instead.

You can analyze the result of a Rollout the same way you would any experiment in GrowthBook.
