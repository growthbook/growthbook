...
During analysis, we use these events to calculate conversion windows for metrics. For example, if a user saw an experiment on day 1 and again on day 7, we might only include their metric conversions that happened within 3 days of each event (i.e. day 1-4 or day 7-11). All other metric conversions would be ignored (e.g. day 5 or 6). The exact length of the conversion window depends on metric-specific settings.

If you prefer to only use the initial assignment event instead, you can override this behavior on a per-analysis basis in the configuration settings.
