...
A user dimension is defined by a simple SQL query that returns two columns: a user id and `value`. The name of the user id column depends on which user id type the dimension is using.
