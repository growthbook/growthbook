...
The first is using our [Visual Editor](/app/visual) and the second is using one of our [SDKs](/lib) (for Javascript, React, PHP, Ruby, Python, Go, or Kotlin).
...
If you are using the SDKs to implement experiments, there are some additional steps you must take.

The SDKs never communicate with the GrowthBook servers. That means as soon as your deploy the A/B test code to production, people will start getting put into the experiment immediately and the experiment will continue until you remove the code and do another deploy.
