...
async function createOrUpdateLicenseMongoCache(
  ...
  let currentCache = mongoCache;
  if (mongoCache === undefined) {
    currentCache = await LicenseModel.findOne({ id: license.id });
  }
      ...
      if (
        ...
        if (!isAirGappedLicenseKey(key)) {
          ...
          if (
            ...
            // Use the cache
            // mongoCache.usingMongoCache = true;
...
export function getLicenseError(org: MinimalOrganization): string {
  ...
  if (licenseData.usingMongoCache && licenseData.dateUpdated) {
    let cachedDataGoodUntil = new Date(
      licenseData.dateUpdated.getTime() - 7 * 24 * 60 * 60 * 1000
    );
    if (
    ...
    if (new Date(licenseData.dateUpdated) < cachedDataGoodUntil) {
