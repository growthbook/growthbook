...
Evaluate the feature (for example with `getFeatureValue` or `isOn`) to enter a feature flag experiment; that is when `trackingCallback` runs. If they are not included, you still get a value and `trackingCallback` does not run.
