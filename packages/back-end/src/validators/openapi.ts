...
export const postFeatureValidator = {
  bodySchema: z.object({"id":z.string().describe("The feature key. Only letters, numbers, hyphens, and underscores allowed. No spaces. Valid examples: greeting, enable_dark_mode, meal-preferences"),"archived":z.boolean().describe("Whether the feature is archived or not").optional(),"description":z.string().describe("A description of the feature").optional(),"owner":z.string().describe("The name of the owner of this feature, for reference").optional(),"project":z.string().describe("Project ID").optional(),"valueType":z.enum(["boolean","string","number","json"]).describe("The type of the value"),"defaultValue":z.string().describe("JSON parsable default value"),"tags":z.array(z.string()).describe("A list of tags").optional(),"environments":z.array(z.object({"id":z.string().describe("Environment ID"),"enabled":z.boolean().describe("Whether the environment is enabled")}))}).strict(),
  querySchema: z.never(),
  paramsSchema: z.never(),
};
