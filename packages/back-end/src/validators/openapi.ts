...
export const apiSavedGroupValidator = z.object({ "id": z.string(), "type": z.enum(["condition","list"]), "dateCreated": z.string(), "dateUpdated": z.string(), "name": z.string(), "owner": z.string().optional(), "condition": z.string().describe("When type = 'condition', this is the JSON-encoded condition for the group").optional(), "attributeKey": z.string().describe("When type = 'list', this is the attribute key the group is based on").optional(), "values": z.any().superRefine((x, ctx) => {
    const schemas = [z.array(z.string()), z.array(z.coerce.number())];
  ...
  }).describe("When type = 'list', this is the list of values for the attribute key").optional(), "description": z.string().optional(), "passByReferenceOnly": z.boolean().describe("Whether the saved group must be referenced by ID rather than its list of items for performance reasons").optional() }).strict()
