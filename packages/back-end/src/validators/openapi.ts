...
export const apiSegmentValidator = z.object({ "id": z.string(), "owner": z.string(), "datasourceId": z.string(), "identifierType": z.string(), "name": z.string(), "query": z.string(), "dateCreated": z.string(), "dateUpdated": z.string() }).strict()
...
export const apiSavedGroupValidator = z.object({ "id": z.string(), "type": z.enum(["condition","list"]), "dateCreated": z.string(), "dateUpdated": z.string(), "name": z.string(), "owner": z.string().optional(), "condition": z.string().describe("When type = 'condition', this is the JSON-encoded condition for the group").optional(), "attributeKey": z.string().describe("When type = 'list', this is the attribute key the group is based on").optional(), "values": z.array(z.string()).describe("When type = 'list', this is the list of values for the attribute key").optional() }).strict()
...
export const putMemberRoleValidator = {
  bodySchema: z.object({ "globalRole": z.string().describe("The name of the global role. This can be one of GrowthBook's standard roles - read-only, collaborator, engineer, analyst, experimenter, or admin. Enterprise organizations who have defined custom roles can also specify the name of a custom role here.").optional(), "environments": z.array(z.string()).optional() }).strict(),
  querySchema: z.never(),
  paramsSchema: z.object({ "id": z.string() }).strict(),
};
