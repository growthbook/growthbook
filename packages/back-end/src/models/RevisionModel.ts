  ...
  async merge(id: string, userId: string, options?: { bypass?: boolean }) {
    ...
    return this.update(existing, {
      ...
      resolution: {
        action: "merged",
        userId,
        user: this.context.auditUser ?? undefined,
        dateCreated: new Date(),
      },
      activityLog: [
        ...this.cleanActivityLog(existing.activityLog),
  ...
  async createRequest(target: {
    ...
    return this.createWithVersionRetry(() =>
      this.create({
        ...
        // Structured actor snapshot — unlike authorId, this also captures
        // api_key and system actors (org-scoped API keys have no userId).
        author: this.context.auditUser ?? undefined,
        reviews: [],
        activityLog: [],
