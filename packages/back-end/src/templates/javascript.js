/* eslint-disable */
(()=>{
if(location.search.match(/\bgrowthbookVisualDesigner\b/)) {window.EXP_PLATFORM_ORIGIN="{{APP_ORIGIN}}";const s=document.createElement("script");s.async=true;s.src="https://unpkg.com/ab-designer@0.4.0/dist/ab-designer.cjs.production.min.js";document.head.appendChild(s);return}
const mutate=(()=>{
...
function injectStyles(css) {var s=document.createElement("style");s.innerHTML=css;document.head.appendChild(s); return function(){s.remove();}}
const config = window.GROWTHBOOK_CONFIG;
if(!config){return console.error("window.GROWTHBOOK_CONFIG must be defined");}if(config.enabled===false) return;
function generateAnonId() {const k="GROWTHBOOK_ANONID";try{let id=localStorage.getItem(k);if(!id){id=Math.floor(Math.random()*1000000);localStorage.setItem(k, id);}return id;} catch (e) {return "";}}
function hashFnv32a(str) {let hval=0x811c9dc5;const l=str.length;for(let i=0;i<l;i++){hval^=str.charCodeAt(i);hval+=(hval<<1)+(hval<<4)+(hval<<7)+(hval<<8)+(hval<<24);}return hval >>> 0;}
const uid=config.userId||config.anonId||generateAnonId(),g=config.groups||[],t=config.track||console.log; 
if(!uid){console.error("No userId or anonId set, skipping all experiments.");return}
function choose(k,w,f){if(f>=0)return f;const n=(hashFnv32a(uid+k)%1000)/1000;let c=0;for(let i=0;i<w.length;i++){c+=w[i];if(n<c)return i;}return -1;}
function included(o){if(o.g&&!o.g.filter((n)=>g.includes(n)).length)return 0;if(o.u&&!location.href.match(new RegExp(o.u)))return 0;return 1;}
let undo=[];function runUndo(){undo.forEach(u=>u&&u());undo=[];}
function run(k,v,o){if(!included(o))return;const i=choose(k,o.w,o.f);if(!v[i])return;undo=undo.concat(v[i]()||[]);t(k,i);}
...
runExperiments();
const q=window.growthbook;
window.growthbook={push:a=>{if(a==="refresh"){runUndo();runExperiments();}}}
q&&q.forEach(q=>{window.growthbook.push(q)});
})();
