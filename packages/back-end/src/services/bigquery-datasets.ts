...
import { fetch, getHttpOptions } from "back-end/src/util/http.util";
import { normalizeBigQueryApiEndpoint } from "./bigquery";
...
export async function listBigQueryDatasets(
  ...
  try {
    const client = new BigQuery({ projectId, apiEndpoint, credentials });
    ...
    throw new Error(
      "Could not fetch BigQuery datasets. Check the endpoint and service account credentials.",
    );
  } finally {
    clearTimeout(timer);
    controller.abort();
