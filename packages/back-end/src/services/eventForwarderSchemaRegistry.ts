import type { EventForwarderAvroRecordSchema } from "shared/event-forwarder-avro";
...
/**
 * Registers a new Avro schema version for the topic's value subject and returns the schema id.
 */
export async function registerEventForwarderSchema(
  topic: string,
  schemaRecord: EventForwarderAvroRecordSchema,
): Promise<number> {
  if (
    !SCHEMA_REGISTRY_URL ||
    !SCHEMA_REGISTRY_API_KEY ||
  ...
  const subject = encodeURIComponent(getEventForwarderSchemaSubject(topic));
  const schemaJson = JSON.stringify(schemaRecord);
  const url = `${trimTrailingSlash(SCHEMA_REGISTRY_URL)}/subjects/${subject}/versions`;
  ...
  if (!response.ok) {
    const message =
      ...
      `${response.status} ${response.statusText}`;
    throw new Error(
      `Failed to register schema for ${getEventForwarderSchemaSubject(topic)}: ${message}`,
    );
  }
  ...
  if (typeof schemaId !== "number" || schemaId <= 0) {
    throw new Error(
      `Schema registry returned an invalid schema id after register for ${getEventForwarderSchemaSubject(topic)}`,
    );
  }
