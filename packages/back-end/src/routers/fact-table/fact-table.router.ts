...
router.post(
  "/fact-tables/tracked-events/:datasourceId",
  validateRequestMiddleware({
    body: z.object({ schema: z.string() }).strict(),
    params: z.object({ datasourceId: z.string() }).strict(),
  }),
  factTableController.getFactTablesFromTrackedEvents
);

router.post(
  "/fact-tables/auto-tables",
  validateRequestMiddleware({
    body: z
      .object({
  ...
  factTableController.postAutoGeneratedFactTables
);
