...
import { queueCreateAutoGeneratedFactMetrics } from "../../jobs/createAutoGeneratedFactMetrics";
...
export const postAutoFactMetrics = async (
  ...
  const factTable = await getFactTable(context, id);

  if (!factTable) {
    throw new Error(`Could not find fact table: ${id}`);
  }

  if (
    !context.permissions.canCreateFactMetric({ projects: factTable.projects })
  ) {
    throw context.permissions.throwPermissionError();
  }
  ...
  try {
    await queueCreateAutoGeneratedFactMetrics(
      context.org.id,
      id,
      metricsToCreate,
      userObj
    ...
    throw new Error(
      `Unable to generate fact metrics automatically. Reason: ${e.message}`
    );
  }
};
