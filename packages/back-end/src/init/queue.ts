...
export async function queueInit() {
  ...
  const eventAgenda = getEventAgendaInstance();
  EventNotifier.register(eventAgenda);
  EventWebHookNotifier.register(eventAgenda);
  ...
  if (CRON_ENABLED) {
    await Promise.all([agenda.start(), eventAgenda.start()]);
  }
