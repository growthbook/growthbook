...
import createAutoGeneratedFactTables from "../jobs/createAutoGeneratedFactTables";
...
export async function queueInit() {
  ...
  await addExperimentResultsJob(agenda);
  await updateScheduledFeatures(agenda);
  await addMetricUpdateJob(agenda);
  ...
  await deleteOldAgendaJobs(agenda);
