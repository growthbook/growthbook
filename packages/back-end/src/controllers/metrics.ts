...
export async function deleteMetric(
  ...
  const metric = await getMetricById(id, org.id, readAccessFilter);
  ...
  await deleteMetricById(id, org, res.locals.eventAudit, readAccessFilter);
...
export async function cancelMetricAnalysis(
  ...
  const { org, readAccessFilter } = getOrgFromReq(req);
  const { id } = req.params;
  const metric = await getMetricById(id, org.id, readAccessFilter, true);
  if (!metric) {
...
export async function putMetric(
  ...
  await updateMetric(metric.id, updates, org.id, readAccessFilter);
