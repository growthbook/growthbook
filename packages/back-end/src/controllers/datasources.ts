...
import { deleteEventForwarderConfigForDatasource } from "back-end/src/services/eventForwarder/datasourceLifecycle";
import {
...
export async function deleteDataSource(
  ...
  await deleteDataSourceWithChecks(context, datasource);
...
export async function putDataSource(
  ...
  try {
    const { integration, eventForwarderWarning } =
      await updateDataSourceWithChecks(context, datasource, {
        name,
        type,
        params,
        settings,
    ...
    // Queued only once the permission checks above have passed
...
export async function postFeatureEvalDiagnostics(
  ...
  const featureObj = await getFeature(context, feature);
  if (!featureObj) {
    res.status(404).json({
      status: 404,
