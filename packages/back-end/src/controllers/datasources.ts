...
import {
  isConfluentEventForwarderSinkType,
  requireEventsForwarderFeature,
} from "back-end/src/services/eventForwarderFeatureGate";
...
export async function postDataSources(
  ...
  if (isConfluentEventForwarderSinkType(eventForwarderConfig?.sinkType)) {
...
export async function putDataSource(
  ...
  const isExplicitConfluentForwarderUpdate =
    eventForwarderConfig !== undefined &&
    isConfluentEventForwarderSinkType(eventForwarderConfig?.sinkType);
  const isConfluentCredentialSync =
    eventForwarderConfig === undefined &&
    !!params &&
    isConfluentEventForwarderSinkType(existingEventForwarderConfig?.sinkType);
  if (isExplicitConfluentForwarderUpdate || isConfluentCredentialSync) {
    await requireEventsForwarderFeature(context);
  }
...
export async function putEventForwarderForDataSource(
  ...
  if (
    isConfluentEventForwarderSinkType(
      parsed.data.eventForwarderConfig?.sinkType,
    )
  ) {
    await requireEventsForwarderFeature(context);
  }
