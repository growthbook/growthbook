...
export async function postGrowthBookDatasource(
  req: AuthRequest<null, { id: string }>,
  res: Response
) {
  console.log("DEBUG postGrowthBookDatasource running 1st query");
  // connect to postgres & generate databse
  // create user, make owner
  console.log("create user");
  await runPostgresQuery(
    {
      host: "localhost",
      database: "adnanchowdhury",
      user: "adnanchowdhury",
      password: "",
      port: 5432,
      ssl: false,
    ...
    `create user org_datasource_1234;`
    // create user org_datasource_1234;
    // alter user org_datasource_1234 with password 'test1234';
    // create database gb_datasource_1234;
    // ALTER DATABASE gb_datasource_1234 OWNER TO org_datasource_1234;
    // `
  );
  ...
  console.log("process dirname", __dirname);
  // console.log("DEBUG postGrowthBookDatasource running 2nd query");
  // // write dummy data
  runPostgresQuery(
    {
      host: "localhost",
      database: "gb_datasource_1234",
      user: "org_datasource_1234",
      password: "test1234",
      port: 5432,
      ssl: false,
      defaultSchema: "",
...
CREATE TABLE sessions (
  ...
  await new Promise<void>((resolve) => {
    fs.createReadStream(path.join(csvFolderPath, "experimentViews.csv"))
      .pipe(csv())
      .on("data", async (row: Record<string, string>) => {
        const query = `INSERT INTO experiment_viewed (
            userId, anonymousId, sessionId, browser, country, timestamp, experimentId, variationId
      ...
      .on("end", () => {
        console.log("generated experiment_viewed queries");
        resolve();
      });
  });
