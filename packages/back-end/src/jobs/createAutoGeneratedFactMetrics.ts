...
const createAutoGeneratedFactMetrics = trackJob(
  ...
  async (job: CreateAutoGeneratedFactMetricsJob) => {
    ...
    try {
      ...
      const {
        allFactMetricsToInsert,
        insertedFactMetrics,
      } = await context.models.factMetrics.createFactMetrics(
        factMetricsToCreate
      );

      for (const metricToCreate of allFactMetricsToInsert) {
        if (
          insertedFactMetrics.some(
            (insertedFactMetric) => insertedFactMetric.id === metricToCreate.id
          )
        ) {
          // the metric was added successfully - add to audit log
          await insertAudit({
            event: "factMetric.autocreate",
            entity: {
              object: "factMetric",
              id: metricToCreate.id,
            },
            organization: organizationId,
            dateCreated: new Date(),
            details: auditDetailsCreate(metricToCreate),
            user,
          });
        } else {
          // the metric wasn't added - log as error
          logger.error(`Failed to insert Fact Metric ${metricToCreate.name}`);
        }
      }
    } catch (e) {
      logger.error(
