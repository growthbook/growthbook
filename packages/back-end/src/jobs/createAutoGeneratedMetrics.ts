...
import { InsertMetricProps } from "shared/types/metric";
...
const createAutoGeneratedMetrics = async (
  ...
  const { datasourceId, organizationId, metricsToCreate } = job.attrs.data;

  const context = await getContextForOrgAdminByOrgId(organizationId);
