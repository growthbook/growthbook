...
export function getInsertMetricSourceCovariateDataLegacyQuery(
  ...
  // Scope FT discovery to this insert's target FT so a pipeline with multiple
  // cross-FT ratios that share a hub (e.g. `[A/B, A/C]`) doesn't trip the 2-FT
  // cap inside `getFactTablesForMetrics`. The other FTs' covariate data is
  ...
  // When the exposure id type is materialized in an aggregated fact table, align
  // the raw scan to daily grain so this fallback covers the same days the
  // pre-aggregated table would (start one calendar day early, end at the day
  // boundary). The per-row covariate window filter below is unchanged.
  const scanStartDate = params.alignLegacyScanToDailyGrain
    ? precedingUtcDayStart(source.minCovariateStartDate)
  ...
  return format(
    ...
    SELECT * FROM (
          ...
          SELECT
            ...
            ${metricData
              .map((m) => {
                ...
                // Role-aware FT comparison matches the schema and decides which
                // side this FT's cache holds. We can't use the
                // numerator/denominatorSourceIndex equality check here because,
                // with `targetFactTableId` set, sides whose FT isn't in the
                // bucket list fall back to index 0 and would spuriously match.
      ...
      SELECT
        ...
        ${metricData
          .map((m) => {
            // Project only the side(s) this cache materializes — matches the
            // schema and the __newCovariateValues projection above. Role-aware
            // FT comparison, not source-index equality.
            const includeNumerator =
              m.metric.numerator?.factTableId === params.factTableId;
            const includeDenominator =
              m.ratioMetric &&
              m.metric.denominator?.factTableId === params.factTableId;
            // Build the value column locally (running it through
            // `capCoalesceValue` so aggregate filters get applied; capping is
