...
`;
  ...
  async getAutoFactTablesToCreate(
    schemaFormat: AutoMetricSchemas, //MKTODO: Rename this?
    // should I include the datasourceId as well?
    existingFactTables: FactTableInterface[],
    datasourceId: string,
    schema?: string
  ): Promise<AutoFactTableTrackedEvent[]> {
    // How do I update this so it handles GA4 auto fact metrics gracefully?
    // On one hand, it feels wasteful to get all the trackedEvents knowing we're only going to use the `events_*` table
    // But, on the other, it ensures that the data source is hooked up and is tracking events correctly

    // getEventsTrackedByDatasource is also universal, so it doesn't feel like we should add crazy logic there
    ...
    //MKTODO: This logic needs to be cleaned up a bit and thought through - I think there are better ways to accomplish this
    if (schemaFormat === "ga4") {
      //TODO: Add logic here to see if this already exists - it's different from the logic below since that is based on the event
      return [
        {
          eventName: "events_*",
          displayName: "events",
          ...
          shouldCreate: true,
          alreadyExists: false,
          count: 0, //MKTODO: Update this to use an actual value
          hasUserId: true, //MKTODO: Update this to use an actual value
    ...
    return trackedEvents.map((event) => {
      const alreadyExists = existingFactTables.some(
        (factTable) =>
          ...
          factTable.eventName === event.eventName
      );
      return {
        ...event,
