...
`;
  ...
  async getAutoFactTablesToCreate(
    ...
    return trackedEvents.map((event) => {
      return {
        ...event,
        shouldCreate: true, // This should eventually be based on if the fact table already exists
  ...
  async getEventsTrackedByDatasourceUpdated(
    schemaFormat: AutoMetricSchemas,
    schema?: string
    ...
    return resultRows.map((result) => {
      ...
      const processedEventData: TrackedEventDataUpdated = {
        event: row.event,
  ...
  //MKTODO: Update this to use the same pattern as getAutoFactTablesToCreate, rename it to getAutoMetricsToCreate
  // Then I can update the method above to remove the `Updated` suffix
