...
//
// Validates the token via auth.test, then creates or updates a Slack Event
// Webhook with the workspace metadata and bot token.
//
...
async function run() {
  ...
  if (existing) {
    ...
    const created = await createEventWebHook({
      ...
      // Valid placeholder for bot-token delivery; auth.test returns the
      // workspace URL rather than an incoming webhook URL.
  ...
  console.log("Done — Slack notifications are connected for this org.");
  process.exit(0);
}
