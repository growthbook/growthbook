...
- **422** - Soft Warning - The request failed, but can be re-submitted with \`"ignoreWarnings": true\` in the request body to proceed anyway. Blocked publishes include a \`gates\` array, one entry per blocking gate, each carrying a uniform set of fields: \`type\`, \`severity\`, \`messages\`, \`override\` (the body flag that clears it — \`ignoreWarnings\` or \`skipSchemaValidation\` — or \`null\` when no flag applies), \`requiresPermission\` (a permission the override needs, or \`null\`), and \`resolution\` (the non-flag way out as a callable \`{ action, method, path }\` route, or \`null\`). So one response lists every way past every gate. A gate with \`override: null\` (approval required) clears by getting the revision approved, or implicitly for callers with the \`bypassApprovalChecks\` permission; a locked config clears by calling its \`resolution\` unlock route. On a SUCCESSFUL publish (200), if a gate that would have blocked was bypassed by the caller's authority, the response includes a \`bypassedGates\` array (\`{ type, outcome: "bypassed", via }\`, where \`via\` is \`ignoreWarnings\`, \`bypassApprovalChecks\`, or \`restApiBypassesReviews\`); the key is omitted when nothing was bypassed.
