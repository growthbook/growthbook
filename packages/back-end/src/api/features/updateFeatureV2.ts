...
const updateFeatureV2Spec = {
  ...updateFeatureV2Validator,
  surfaceInputWarnings: true,
};

export const updateFeatureV2 = createApiRequestHandler(updateFeatureV2Spec)(
  async (req) => {
    ...
    if (defaultValue != null || dvcProvided) {
      ...
      storedDefault =
        dvc !== null
          ? composeConfigBacking(dvc, patch, "Default value")
          : patch;
    }
    ...
    if (
      ...
      if (
        !req.context.permissions.canPublishFeature(
          { project: effectiveProject },
          Array.from(
            getEnabledEnvironments({ ...feature, ...updates }, orgEnvs),
          ),
        )
      ) {
        req.context.permissions.throwPermissionError();
