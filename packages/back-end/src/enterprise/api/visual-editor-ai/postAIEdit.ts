...
- \`generateImage\` — generate a single AI image and get a hosted URL. Call when the user asks to replace, set, or insert an image that does not exist yet (or any background-image). If the user ATTACHED an image this turn, do not generate — place the attachment's hosted URL from the Attached images block. The URL you get back can be placed directly into a mutation's \`value\` — as a \`src\` for <img>, inside a \`background-image: url(...)\` style, or as part of HTML markup for a new <img>. Pick the aspectRatio that matches where the image will appear (16:9 hero, 1:1 avatar, etc.). Call once per image; for multi-image requests (e.g. "build a carousel of 3 slides"), call multiple times. You are budgeted up to 3 image generations per turn.
