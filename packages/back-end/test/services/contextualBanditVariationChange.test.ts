...
describe("executeContextualBanditVariationChange", () => {
  ...
  it("removes a variation in explore: clears leaf weights left over from an earlier exploit stage", async () => {
    const cb = makeCb({
      variations: [v("v0", "0"), v("v1", "1"), v("v2", "2")],
      variationWeights: [
        { variationId: "v0", weight: 0.2 },
      ...
      currentLeafWeights: [
        {
          ...
          weights: [
            { variationId: "v0", weight: 0.1 },
            { variationId: "v1", weight: 0.8 },
            { variationId: "v2", weight: 0.1 },
          ],
        },
      ],
    });
  ...
  it("removes a variation in exploit with leaf weights: overall and every leaf re-sum to 1 over survivors (Luke B)", async () => {
    const cb = makeCb({
      ...
      variationWeights: [
        { variationId: "v0", weight: 0.1 },
        { variationId: "v1", weight: 0.2 },
        { variationId: "v2", weight: 0.3 },
        { variationId: "v3", weight: 0.15 },
        { variationId: "v4", weight: 0.25 },
      ],
      currentLeafWeights: [
        {
          leafId: 0,
          condition: { country: "US" },
          weights: [
            { variationId: "v0", weight: 0.05 },
            { variationId: "v1", weight: 0.15 },
            { variationId: "v2", weight: 0.4 },
            { variationId: "v3", weight: 0.1 },
            { variationId: "v4", weight: 0.3 },
          ],
        },
        {
          leafId: 1,
          ...
          weights: [
            ...
            { variationId: "v3", weight: 0.2 },
            { variationId: "v4", weight: 0.1 },
          ],
        },
      ],
    });
  ...
  it("recomputes leaf weights from a fresh read when a concurrent run bumps banditVersion", async () => {
    const cb = makeCb({
      stage: "exploit",
      variations: [v("v0", "0"), v("v1", "1"), v("v2", "2")],
      variationWeights: [
      ...
      currentLeafWeights: [
        {
          ...
          condition: { country: "US" },
          weights: [
            { variationId: "v0", weight: 0.8 },
            { variationId: "v1", weight: 0.1 },
            { variationId: "v2", weight: 0.1 },
          ],
        },
      ],
  ...
  it("keeps added arms pending and skips activation when a linked feature fails to take its values", async () => {
    const feature = makeFeature();
    getRefLinkedFeatureInfoMock.mockResolvedValue([linkedInfo(feature)]);
    updateRevisionMock.mockRejectedValueOnce(
  ...
  it("keeps an added arm pending and out of the weights when one of several linked features fails (multi-feature partial failure)", async () => {
    const feature1 = makeFeature({ id: "feature1" });
    const feature2 = makeFeature({ id: "feature2" });
    getRefLinkedFeatureInfoMock.mockResolvedValue([
      linkedInfo(feature1),
      linkedInfo(feature2),
    ]);
    getFeatureMock.mockImplementation(async (_ctx: unknown, id: string) =>
      id === "feature1"
        ? makeFeature({
            id: "feature1",
            rules: [
        ...
        : makeFeature({ id: "feature2" }),
    );
    updateRevisionMock.mockImplementation(
      async (_context, feature, revision, changes) => {
        if ((feature as FeatureInterface).id === "feature2") {
          throw new Error("revision write failed");
        }
        return { ...revision, ...changes } as FeatureRevisionInterface;
      },
    );
    const cb = makeCb({
      status: "running",
  ...
  it("activates pending arms on feature publish without the publisher's CB permissions", async () => {
    getFeatureMock.mockResolvedValue(
      makeFeature({
        rules: [
          cbRefRule({
