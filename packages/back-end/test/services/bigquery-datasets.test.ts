...
jest.mock("@google-cloud/bigquery", () => ({
  BigQuery: class {
    ...
    constructor() {
      this.baseUrl = `${mockDefaultEndpoint}/bigquery/v2`;
    }
  },
}));
...
describe("listBigQueryDatasets", () => {
  ...
  it("fetches datasets with authentication headers", async () => {
  ...
  it("preserves the SDK base URL and pages through encoded tokens", async () => {
    mockDefaultEndpoint += "/tenant";
    respond = (req, res) => {
  ...
  it("uses stored credentials when the submitted private key is blank", async () => {
    const params = mergeDataSourceParams("bigquery", credentials, {
      privateKey: "",
    });
