...
describe("testQueryValidity", () => {
  ...
  describe("contextual bandit queries", () => {
    const cbQuery = {
      id: "user_id",
      name: "CB Query",
      userIdType: "user_id",
    ...
    it("requires the contextual bandit columns when contextualBandit is true", async () => {
      mockDataSourceIntegration.getTestValidityQuery = jest
        .fn()
        .mockReturnValue("SELECT * FROM experiments");
      mockDataSourceIntegration.runTestQuery = jest.fn().mockResolvedValue({
    ...
    it("passes when the contextual bandit columns are present", async () => {
      mockDataSourceIntegration.getTestValidityQuery = jest
        .fn()
        .mockReturnValue("SELECT * FROM experiments");
      mockDataSourceIntegration.runTestQuery = jest.fn().mockResolvedValue({
    ...
    it("does not require the contextual bandit columns when contextualBandit is not set", async () => {
      const nonCbQuery = {
        ...cbQuery,
