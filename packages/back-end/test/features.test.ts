...
describe("Rollout Ramp Schedule", () => {
  ...
  //
  // Resulting windows (elapsed seconds):
  //   [   0,  600) -> 0.01
  //   [ 600, 1200) -> 0.05
  //   [1200, 1800) -> 0.10
  //   [1800, 2400) -> 0.25
  ...
  describe("getCurrentRampCoverage", () => {
    it("returns null for an undefined schedule (caller falls back to static coverage)", () => {
      expect(getCurrentRampCoverage(undefined, at(0))).toEqual(null);
    });

    it("returns null when startedAt is null (rule still in draft)", () => {
      const notStarted: RampSchedule = { ...uniformRamp, startedAt: null };
      expect(getCurrentRampCoverage(notStarted, at(0))).toEqual(null);
    });

    it("returns the first step's coverage at exactly startedAt", () => {
      expect(getCurrentRampCoverage(uniformRamp, at(0))).toEqual(0.01);
    });
    ...
    it("clamps to the first step when now < startedAt (clock skew safety)", () => {
      ...
      expect(getCurrentRampCoverage(uniformRamp, at(-30))).toEqual(0.01);
    });

    it("holds the current step's coverage for its full hold period", () => {
      // 1s into step 1 (window [600, 1200))
      expect(getCurrentRampCoverage(uniformRamp, at(601))).toEqual(0.05);
      // 1s before step 2
      expect(getCurrentRampCoverage(uniformRamp, at(1199))).toEqual(0.05);
    });
    ...
    it("advances at each step boundary (uniform schedule)", () => {
      ...
      for (const [elapsed, expected] of cases) {
        expect(getCurrentRampCoverage(uniformRamp, at(elapsed))).toEqual(
          expected,
        );
      }
    });
    ...
    it("graduates to 100% after the last step's hold period expires", () => {
      ...
      expect(getCurrentRampCoverage(uniformRamp, at(3600))).toEqual(1);
      // And forever after
      expect(getCurrentRampCoverage(uniformRamp, at(86400))).toEqual(1);
    });

    it("respects non-uniform hold periods", () => {
      // In the 2-min 1% smoke window
      expect(getCurrentRampCoverage(bakeRamp, at(60))).toEqual(0.01);
      // In the 3-min 10% window
      expect(getCurrentRampCoverage(bakeRamp, at(200))).toEqual(0.1);
      // Start of the 30-min 50% bake
      expect(getCurrentRampCoverage(bakeRamp, at(300))).toEqual(0.5);
      // Deep in the bake
      expect(getCurrentRampCoverage(bakeRamp, at(1800))).toEqual(0.5);
      // One second before graduating
      expect(getCurrentRampCoverage(bakeRamp, at(2099))).toEqual(0.5);
      // Graduated to implicit 100%
      expect(getCurrentRampCoverage(bakeRamp, at(2100))).toEqual(1);
    });
    ...
    it("handles a single-step schedule (one hold, then 100%)", () => {
      ...
      expect(getCurrentRampCoverage(single, at(0))).toEqual(0.1);
      expect(getCurrentRampCoverage(single, at(299))).toEqual(0.1);
      expect(getCurrentRampCoverage(single, at(300))).toEqual(1);
    });
    ...
    it("preserves step order as given (does not sort by coverage)", () => {
      ...
      expect(getCurrentRampCoverage(weird, at(50))).toEqual(0.5);
      expect(getCurrentRampCoverage(weird, at(150))).toEqual(0.1);
      expect(getCurrentRampCoverage(weird, at(200))).toEqual(1);
    });
  });

  describe("getNextRampUpdate", () => {
    it("returns null for an undefined schedule", () => {
      expect(getNextRampUpdate(undefined, at(0))).toEqual(null);
    });

    it("returns null when startedAt is null", () => {
      const notStarted: RampSchedule = { ...uniformRamp, startedAt: null };
      expect(getNextRampUpdate(notStarted, at(0))).toEqual(null);
    });

    it("returns the end of the current step's hold window while mid-step", () => {
      // 300s into step 0 (holds until 600)
      expect(getNextRampUpdate(uniformRamp, at(300))).toEqual(at(600));
    });

    it("returns the following boundary at an exact boundary", () => {
      // At 600 we've just entered step 1 (holds until 1200)
      expect(getNextRampUpdate(uniformRamp, at(600))).toEqual(at(1200));
    });
    ...
    it("respects non-uniform hold periods when computing the next boundary", () => {
      ...
      expect(getNextRampUpdate(bakeRamp, at(1000))).toEqual(at(2100));
    });
    ...
    it("returns the graduation boundary while in the last explicit step", () => {
      ...
      expect(getNextRampUpdate(uniformRamp, at(3500))).toEqual(at(3600));
    });

    it("returns null once graduated to the implicit 100%", () => {
      expect(getNextRampUpdate(uniformRamp, at(3600))).toEqual(null);
      expect(getNextRampUpdate(bakeRamp, at(2100))).toEqual(null);
      expect(getNextRampUpdate(uniformRamp, at(86400))).toEqual(null);
    });

    it("returns the first boundary when now < startedAt", () => {
      // Clamped to step 0; next boundary is end of step 0's hold
      expect(getNextRampUpdate(uniformRamp, at(-3600))).toEqual(at(600));
    });
    ...
    it("handles a single-step schedule (one boundary: the graduation)", () => {
      ...
      expect(getNextRampUpdate(single, at(0))).toEqual(at(300));
      expect(getNextRampUpdate(single, at(300))).toEqual(null);
    });
  });
});
