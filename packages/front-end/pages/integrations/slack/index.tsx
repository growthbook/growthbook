...
function buildSlackAppManifest({ appUrl }: { appUrl: string }): string {
...
function SlackManifestModal({ onClose }: { onClose: () => void }) {
  const manifest = useMemo(() => {
    ...
    return buildSlackAppManifest({ appUrl });
  }, []);
        ...
        >
          <Box style={{ maxWidth: 640, flex: 1 }}>
            ...
            <Text as="p" color="text-mid" mb="0">
              Connect Slack channels to GrowthBook notifications. Open a channel
              to configure what it&rsquo;s notified about.
            </Text>
          </Box>
