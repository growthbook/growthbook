 ...
 * the datetime picker (optional seconds; picking a day defaults to 00:00:00).
 *
 ...
 * day. Values are stored as `yyyy-MM-dd` (equality) or `yyyy-MM-dd'T'HH:mm:ss`
...
export function DateFilterInput({
  ...
  const dateFormat = dateOnly ? "yyyy-MM-dd" : "yyyy-MM-dd'T'HH:mm:ss";
  const parsedDate = value
    ? dateOnly
      ? getValidDateOffsetByUTC(value.slice(0, 10))
      : getValidDate(
          value.length > 10 ? value : `${value.slice(0, 10)}T00:00:00`,
        )
    : undefined;
