      ...
      <Flex align="start" direction="column" gap="4" mt="7">
        ...
        <Box width="100%">
          <Frame p="3" mb="0">
            ...
            <Text as="p" size="medium" mb="4" color="text-low">
              All changes to constants are tracked as revisions. Requiring
              approvals adds a review step before any change goes live.
            </Text>
            ...
            {hasRequireApprovals && (
              <>
                <Checkbox
                  id="toggle-require-approvals-constants"
                  label="Require approval to modify Constants"
                  description="When enabled, all changes to Constants must be reviewed and approved by another person before going live."
                  value={!!form.watch("approvalFlows.constants.0.required")}
                  setValue={(v) =>
                    form.setValue("approvalFlows.constants.0.required", v)
                  }
                />
                {!!form.watch("approvalFlows.constants.0.required") && (
                  <Flex direction="column" gap="3" mt="2" ml="5">
                    <Box mt="2">
                      <Text as="label" size="medium" weight="semibold" mb="2">
                        Require approval for
                      ...
                      <Flex direction="column" gap="2" align="start">
                        <Checkbox
                          id="toggle-constant-values"
                          label="Values and overrides"
                          value={true}
                          disabled={true}
                          setValue={() => undefined}
                        />
                        <Checkbox
                          id="toggle-constant-metadata-review"
                          label="Metadata changes (name, description, owner, projects, etc.)"
                          value={
                            form.watch(
                              `approvalFlows.constants.0.requireMetadataReview`,
                            ) !== false
                          }
                          setValue={(v) =>
                            form.setValue(
                    ...
                    <Checkbox
                      id="toggle-constant-reset-review-on-change"
                      label="Reset review on changes"
                      description="If a draft is modified after being approved, the approval is revoked and a new review is required before publishing."
                      value={
                        !!form.watch(
                          `approvalFlows.constants.0.resetReviewOnChange`,
                        )
                      }
                      setValue={(v) =>
                        form.setValue(
                    ...
                    />
                    <Checkbox
                      id="toggle-constant-block-self-approval"
                      label="Require approval from a non-editor"
                      description="Anyone who edited the draft is blocked from approving it. A separate reviewer must approve before publishing."
                      value={
                        !!form.watch(
                          `approvalFlows.constants.0.blockSelfApproval`,
                        )
                      }
                      setValue={(v) =>
                        form.setValue(
                    ...
                    />
                    <Checkbox
                      id="toggle-constant-autopublish-on-approval"
                      label="Allow approve & publish in one step"
                      description="Adds an 'Approve & Publish' option so reviewers with publish access can approve and publish a constant change together."
                      value={
                        !!form.watch(
                          `approvalFlows.constants.0.autopublishOnApproval`,
                        )
                      }
                      setValue={(v) =>
                        form.setValue(
