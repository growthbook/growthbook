...
import { buildConfigRevisionUrl } from "@/components/Revision/revisionUtils";
...
function revisionToRow(revision: Revision): ReviewRow {
  const snapshot = revision.target.snapshot as
    ...
    | undefined;
  const configName = snapshot?.name || revision.target.id;
  return {
    id: revision.id,
...
const ConfigReviews: FC = () => {
  ...
  const rows: ReviewRow[] = useMemo(
    () => revisions.filter(isConfigRevision).map(revisionToRow),
    [revisions],
  );
  ...
  const {
    ...
    items: reviewItems,
    localStorageKey: "configReviewsList",
    defaultSortField: "dateUpdated",
    defaultSortDir: -1,
    searchFields: ["configName", "authorId", "title", "authorDisplay"],
    searchTermFilters: {
  ...
  return (
    <Box mt="4">
      ...
      {isLoading ? (
        ...
        <Callout status="info">No drafts for configs.</Callout>
      ) : (
        <>
          <Table variant="list" stickyHeader roundedCorners>
            <TableHeader>
              <TableRow>
                <SortableTableColumnHeader field="configName">
                  Config
                </SortableTableColumnHeader>
                <TableColumnHeader
