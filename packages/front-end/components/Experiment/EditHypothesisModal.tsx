import { useForm } from "react-hook-form";
import { Box, Flex, Heading, Text } from "@radix-ui/themes";
import { BsStars } from "react-icons/bs";
import { useState } from "react";
import { PiArrowClockwise } from "react-icons/pi";
import {
  AISuggestionType,
  computeAIUsageData,
  formatAIRateLimitRetryMessage,
} from "shared/ai";
import { useGrowthBook } from "@growthbook/growthbook-react";
import Tooltip from "@/components/Tooltip/Tooltip";
import { useAuth } from "@/services/auth";
import Button from "@/ui/Button";
import Callout from "@/ui/Callout";
import { useAISettings } from "@/hooks/useOrgSettings";
import Markdown from "@/components/Markdown/Markdown";
import MarkdownInput from "@/components/Markdown/MarkdownInput";
import LoadingSpinner from "@/components/LoadingSpinner";
import OptInModal from "@/components/License/OptInModal";
import PremiumTooltip from "@/components/Marketing/PremiumTooltip";
import { useUser } from "@/services/UserContext";
import ModalStandard from "@/ui/Modal/Patterns/ModalStandard";
import track from "@/services/track";
import { AppFeatures } from "@/types/app-features";

interface Props {
  source: string;
  close: () => void;
  experimentId: string;
  initialValue?: string;
  mutate: () => void;
}

type CheckResult = {
  isCompliant: boolean;
  feedback: string;
  suggestion: string;
};

export default function EditHypothesisModal({
  source,
  close,
  experimentId,
  initialValue,
  mutate,
}: Props) {
  const { apiCall } = useAuth();
  const { aiEnabled, aiAgreedTo } = useAISettings();
  const [loading, setLoading] = useState<boolean>(false);
  const [error, setError] = useState<string | null>(null);
  const [checkResult, setCheckResult] = useState<CheckResult | null>(null);
  const [suggestionRevealed, setSuggestionRevealed] = useState<boolean>(false);
  const [aiAgreementModal, setAiAgreementModal] = useState<boolean>(false);
  const [revertValue, setRevertValue] = useState<string | null>(null);
  const [showModal, setShowModal] = useState<boolean>(true);
  const { hasCommercialFeature } = useUser();
  const gb = useGrowthBook<AppFeatures>();
  const hasAISuggestions = hasCommercialFeature("ai-suggestions");
  const form = useForm<{ hypothesis: string }>({
    defaultValues: {
      hypothesis: initialValue || "",
    },
  });

  const resetCheckState = () => {
    setCheckResult(null);
    setSuggestionRevealed(false);
    setRevertValue(null);
  };

  const checkHypothesis = async (type?: AISuggestionType) => {
    if (!aiAgreedTo) {
      setAiAgreementModal(true);
      // This needs a timeout to avoid a flicker if this modal disappears before the AI agreement modal appears.
      setTimeout(() => {
        setShowModal(false);
      }, 0);
      return;
    }
    if (!aiEnabled) {
      setError("AI is disabled for your organization. Adjust in settings.");
      return;
    }

    setError(null);
    setLoading(true);
    setSuggestionRevealed(false);
    const aiTemperature =
      gb?.getFeatureValue("ai-suggestions-temperature", 0.1) || 0.1;
    track("ai-suggestion", { source: "edit-hypothesis-modal", type });
    apiCall<{ data: { check: CheckResult } }>(
      `/ai/reformat`,
      {
        method: "POST",
        body: JSON.stringify({
          type: "experiment-hypothesis",
          text: form.watch("hypothesis"),
          action: "check",
          temperature: aiTemperature,
        }),
      },
      (responseData) => {
        if (responseData.status === 429) {
          setError(formatAIRateLimitRetryMessage(responseData.retryAfter));
        } else if (responseData.message) {
          setError(responseData.message);
        } else {
          setError("Error getting AI suggestion");
        }
        setLoading(false);
      },
    )
      .then((res) => {
        setCheckResult(res.data.check);
      })
      .catch(() => {
        // Error handling is done by the apiCall errorHandler
      })
      .finally(() => {
        setLoading(false);
      });
  };

  const hypothesis = form.watch("hypothesis");
  const suggestion = checkResult?.suggestion ?? "";
  const showSuggestionBlock = suggestionRevealed && !!suggestion;

  return (
    <>
      <ModalStandard
        trackingEventModalType="edit-hypothesis-modal"
        trackingEventModalSource={source}
        header={"Edit Hypothesis"}
        size="lg"
        open={showModal}
        close={close}
        submit={form.handleSubmit(async (data) => {
          if (suggestion) {
            track("experiment-hypothesis-saved-after-ai-suggestion", {
              aiUsageData: computeAIUsageData({
                value: data.hypothesis,
                aiSuggestionText: suggestion,
              }),
            });
          }
          await apiCall(`/experiment/${experimentId}`, {
            method: "POST",
            body: JSON.stringify({
              hypothesis: data.hypothesis,
            }),
          });
          mutate();
        })}
        ctaEnabled={initialValue !== hypothesis}
      >
        <MarkdownInput
          value={hypothesis}
          setValue={(value) => {
            form.setValue("hypothesis", value);
            if (checkResult) {
              resetCheckState();
            }
          }}
          placeholder="e.g Making the signup button bigger will increase clicks and ultimately improve revenue"
          showButtons={false}
        />
        <Box my="4">
          {!checkResult && !loading && (
            <Flex align="start" justify="start">
              {hasAISuggestions ? (
                <Tooltip
                  body={
                    aiEnabled ? (
                      "Check if your hypothesis matches the organization's formatting standards"
                    ) : (
                      <>
                        Org admins can set hypothesis formatting standards for
                        the organization in <strong>General Settings</strong>.
                      </>
                    )
                  }
                >
                  <Button
                    disabled={
                      (!aiEnabled && aiAgreedTo) ||
                      loading ||
                      hypothesis.trim() === ""
                    }
                    variant="soft"
                    onClick={() => checkHypothesis("suggest")}
                    stopPropagation={true}
                  >
                    <BsStars /> Check hypothesis
                  </Button>
                </Tooltip>
              ) : (
                <PremiumTooltip commercialFeature="ai-suggestions">
                  <Button disabled={true} variant="soft">
                    <BsStars /> Check hypothesis
                  </Button>
                </PremiumTooltip>
              )}
            </Flex>
          )}
          {error && (
            <Box my="4">
              <p className="text-danger">{error}</p>
            </Box>
          )}
          {loading && (
            <Box my="4">
              <Text color="gray">
                <LoadingSpinner /> Checking hypothesis...
              </Text>
            </Box>
          )}
          {!loading && !error && checkResult && checkResult.isCompliant && (
            <Box my="4">
              <Callout status="success">
                Hypothesis looks good — it matches your organization&apos;s
                formatting standards.
              </Callout>
              <Box mt="3">
                <Button
                  variant="ghost"
                  onClick={() => checkHypothesis("try-again")}
                >
                  <PiArrowClockwise /> Check again
                </Button>
              </Box>
            </Box>
          )}
          {!loading &&
            !error &&
            checkResult &&
            !checkResult.isCompliant &&
            !showSuggestionBlock && (
              <Box my="4">
                <Callout status="warning">
                  {checkResult.feedback ||
                    "This hypothesis could be improved to better match your organization's formatting standards."}
                </Callout>
                <Flex gap="2" mt="3">
                  {suggestion && (
                    <Button
                      variant="soft"
                      onClick={() => {
                        setSuggestionRevealed(true);
                        track("ai-suggestion-generate-compliant", {
                          source: "edit-hypothesis-modal",
                        });
                      }}
                    >
                      <BsStars /> Generate compliant version
                    </Button>
                  )}
                  <Button
                    variant="ghost"
                    onClick={() => checkHypothesis("try-again")}
                  >
                    <PiArrowClockwise /> Try Again
                  </Button>
                </Flex>
              </Box>
            )}
          {!loading && !error && showSuggestionBlock && (
            <Box my="4">
              <Flex align="center" justify="between" my="4">
                <Heading size="2" weight="medium">
                  Suggested Hypothesis:
                </Heading>
                <Flex gap="2">
                  <Button
                    variant="ghost"
                    onClick={() => checkHypothesis("try-again")}
                  >
                    <PiArrowClockwise /> Try Again
                  </Button>
                  {hypothesis !== suggestion && (
                    <Tooltip body="Overwrite content above with suggested content.">
                      <Button
                        variant="soft"
                        onClick={() => {
                          setRevertValue(hypothesis);
                          form.setValue("hypothesis", suggestion);
                          track("use-ai-suggestion", {
                            source: "edit-hypothesis-modal",
                          });
                        }}
                      >
                        Use Suggested
                      </Button>
                    </Tooltip>
                  )}
                  {revertValue && hypothesis === suggestion && (
                    <Tooltip body="Revert to previous content.">
                      <Button
                        variant="soft"
                        onClick={() => {
                          form.setValue("hypothesis", revertValue);
                          setRevertValue(null);
                          track("revert-ai-suggestion", {
                            source: "edit-hypothesis-modal",
                          });
                        }}
                      >
                        Revert
                      </Button>
                    </Tooltip>
                  )}
                </Flex>
              </Flex>
              <Box className="appbox" p="3" mb="2">
                <Markdown>{suggestion}</Markdown>
              </Box>
            </Box>
          )}
        </Box>
      </ModalStandard>
      {aiAgreementModal && (
        <OptInModal
          agreement="ai"
          onClose={() => {
            setShowModal(true);
            setAiAgreementModal(false);
          }}
        />
      )}
    </>
  );
}
