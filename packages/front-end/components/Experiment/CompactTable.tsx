...
const CompactTable: FC<{
  ...
  setRiskVariation: (v: number) => void;
}> = ({
  snapshot,
  experiment,
  metrics,
  ...
  const hasRisk =
    (!guardrail || riskVariation > 0) &&
    Object.values(variations[1]?.metrics || {}).filter(
      (x) => x.risk?.length > 0
    ).length > 0;

  const showChanceToBeatControl = !guardrail;

  const graphWidth = guardrail ? 300 : 500;

  return (
    <table className={`table experiment-compact aligned-graph w-auto`}>
      <thead>
        <tr>
          ...
          {experiment.variations.map((v, i) => (
            <th
              colSpan={i ? (showChanceToBeatControl ? 3 : 2) : 1}
              className="value"
              key={i}
            >
              {v.name}
        ...
        <tr>
          ...
          {experiment.variations.map((v, i) => (
            <React.Fragment key={i}>
              ...
              {i > 0 && (
                <>
                  {showChanceToBeatControl && <td className="empty-td"></td>}
                  <td className="p-0">
                    <div>
                      <AlignedGraph
                ...
                {experiment.variations.map((v, i) => {
                  ...
                  return (
                    <React.Fragment key={i}>
                      ...
                      {i > 0 && (
                        <td
                          colSpan={showChanceToBeatControl ? 1 : 2}
                          className="variation"
                        ></td>
                      )}
                    </React.Fragment>
