...
function activityEntry(
  ...
  switch (a.action) {
    ...
    case "scheduled-publish-updated":
      return {
        kind: "lifecycle",
        label: a.description || "scheduled this revision to publish",
        icon: <PiClockFill />,
        color: "violet",
      };
    case "scheduled-publish-canceled":
      return {
        kind: "lifecycle",
        label: a.description || "canceled the scheduled publish",
        icon: <PiClockFill />,
        color: "gray",
      };
    default:
...
export default function RevisionTimeline<T>({
  ...
  // When provided, the author of a comment gets inline edit/delete affordances.
  onEditComment?: (reviewId: string, comment: string) => Promise<void>;
  onDeleteComment?: (reviewId: string) => Promise<void>;
}) {
  const { users, getUserDisplay, userId } = useUser();
  const [editsExpanded, setEditsExpanded] = useState(false);
  // Entry ids whose "Details" diff is expanded.
  const [openDetails, setOpenDetails] = useState<Set<string>>(new Set());
  // The comment currently being edited inline, and the one pending deletion.
  ...
  return (
    <Box>
      {deleteId && onDeleteComment && (
        <ModalStandard
          trackingEventModalType="delete-revision-comment"
          open={true}
              ...
              {items.map((entry) => {
                ...
                return (
                  ...
                  >
                    ...
                    {editingId === entry.id && onEditComment ? (
                      <CommentComposer
                        initialValue={entry.body}
                        cta="Save"
                        autofocus
                        onSubmit={async (v) => {
                          await onEditComment(entry.id, v);
                          setEditingId(null);
                        }}
