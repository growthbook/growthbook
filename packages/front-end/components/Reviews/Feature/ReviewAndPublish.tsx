...
export default function ReviewAndPublish({
  ...
  const canAdminPublish = permissionsUtil.canBypassFlagApprovalChecks(
    feature,
    "feature",
  );
  ...
  // Non-member creators aren't org members, so they don't appear in the people
  // rows. An API key (often a PAT) renders with the standard author UI — avatar,
  // name, and an "API" badge — the same as authors everywhere else; a system
  // actor keeps its plain "Generated by" line. Without this an API-key-authored
  // revision showed no attribution at all.
  const apiKeyCreator =
    revision.createdBy?.type === "api_key" ? revision.createdBy : null;
  const systemCreator =
  ...
  const contributorIds =
    authorId && !contribIds.includes(authorId)
      ? [authorId, ...contribIds]
      : contribIds;
  const generatedByRow = apiKeyCreator ? (
    <Box mb="3">
