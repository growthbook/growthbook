...
function MetricExplorerContent() {
  ...
  const [submittedExploreState, setSubmittedExploreState] =
    useState<ExploreState>(draftExploreState);
  ...
  const handleUpdateSeries = (id: string, updates: Partial<ExploreSeries>) => {
    setDraftExploreState({
      ...draftExploreState,
      series: draftExploreState.series.map((s) =>
        s.id === id ? { ...s, ...updates } : s,
      ),
    });
  };
  ...
  const selectedSeries = draftExploreState.series.find(
