...
export const MockDefinitionsProvider: FC<{ children: ReactNode }> = ({
  ...
  const value = useMemo(() => {
    ...
    const getFactTableById = (id: string) => factTables.find(t => t.id === id) || null;
    const getFactMetricById = (id: string) => factMetrics.find(m => m.id === id) || null;
    const getDatasourceById = (id: string) => datasources.find(d => d.id === id) || null;
