...
**Requirements:** Node.js 24+, pnpm 10.33.4, Python 3.9+ with Poetry (for stats engine), Docker (for MongoDB).
...
- Shared: Do not use `.default()` on Zod schemas; use `defaultValues` in the BaseModel config instead.
...
2. **External REST API** (for customers): Handlers in `src/api/`, mounted at `/api/v2/` and `/api/v1/` (prefer v2 when both exist), API key Bearer auth, uses `createApiRequestHandler()`.
...
4. Bootstrap is legacy — do not use it in new code.

### Permission System

Three tiers: global (`manageTeam`, `manageBilling`), project-scoped (`manageFeatures`, `createMetrics`), and environment-scoped (`publishFeatures`, `runExperiments`).

- **Front-end:** `usePermissionsUtil()` hook with methods such as `canCreateFeature({ project })`.
...
- **TypeScript strict mode** — never use `any`; use `unknown` for untrusted data and validate it with Zod.
...
The detailed reference material lives under `.agents/guides/`. Read the relevant guide before changing that area:

- Project structure: `.agents/guides/project-overview.md`
- General development practices: `.agents/guides/development-guidelines.md`
