...
<Tip>
**Passing Saved Groups by reference**

Enterprise users can reduce payload size by passing Saved Groups by reference: each group is sent once in the payload instead of being copied into every rule that uses it. Edit your SDK Connection and set **Pass Saved Groups by reference** to **ID Lists only** or **All Saved Groups**. **All Saved Groups** needs a newer SDK version, such as JavaScript SDK 1.8.0 or later. If the setting isn't visible, your SDK version does not support it.
