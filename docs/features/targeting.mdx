...
To enforce scope on Feature Flags, enable **Enforce Saved Group Project scope** under **Settings → General → Saved Group Settings**. Every referenced group must then be unscoped or shared with every Project where the referencing rule can run. This includes the Feature Flag's primary Project and `targetingProjects` (or All Projects), narrowed by the rule's own Project scope. A rule delivered to All Projects requires unscoped groups.
...
Enabling enforcement does not rewrite existing targeting or remove groups from SDK payloads. Existing violations must be repaired before publishing an affected Feature Flag again. This setting applies to Feature Flag targeting; standalone experiment targeting is unchanged.
