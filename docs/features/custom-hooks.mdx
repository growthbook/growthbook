---
title: Custom Hooks
description: Run custom validation when a feature is published. Available on self-hosted GrowthBook only.
...
A warning or thrown error can include links in Markdown syntax, such as a link
to your internal guidelines. In the app, only `http://` and `https://` links are
shown as links, and they open in a new tab. A REST API client receives the raw
text. Links are also rendered in any user-entered text your hook adds to the
message, such as an experiment name.

```js
if (!feature.description) {
  addWarning(
    "Consider adding a description. See [our flag guidelines](https://wiki.example.com/flags)."
  );
}
```
