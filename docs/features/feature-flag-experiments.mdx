...
Feature flag experiments use an experiment override rule. Saving and publishing the rule makes it available to the SDK.
A user is enrolled when your application evaluates the feature (for example with `isOn` or `getFeatureValue`) and they
qualify. That is when `trackingCallback` runs so the exposure can be tracked in your data warehouse. If they are not
included, you still get the default (or the next matching rule), and `trackingCallback` does not run.

Here's what an Experiment rule looks like in the GrowthBook UI:

<Frame>
  <img src="/static/images/features/feature-experiment-rules.png" alt="Experiment rule UI" width="500" />
</Frame>
...
be included in the experiment. You can add multiple targeting conditions per rule, and you can add multiple rules per feature,
...
section. Like all attributes in GrowthBook, the user's value for this attribute must be defined in the SDK implementation.
You can read more about [targeting attributes](/features/targeting).
...
(or the default value) instead. 100% traffic means 100% of those evaluations, not 100% of all visitors.

You may want to run an experiment at a split percentage that weights the control group
...
Results only include people who got an exposure. Those exposures flow into your data warehouse via `trackingCallback`.
There is a link to view experiment results on the bottom of each experiment override rule. Read more about
