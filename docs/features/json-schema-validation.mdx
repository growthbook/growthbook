...
sidebarTitle: JSON Schema Validation
...
<Frame>
  <img src="/static/images/features/json-schema/validation-banner.png" alt="JSON Schema Validation" width="800" />
</Frame>
...
<Frame>
  <img src="/static/images/features/json-schema/validation-form.png" alt="JSON Schema Editor" width="800" />
</Frame>
...
<Frame>
  <img src="/static/images/features/json-schema/value-editor.png" alt="Simplified editing UI for simple schemas" width="800" />
</Frame>
...
The API's `skipSchemaValidation` option bypasses schema checks, but values must still be valid for the Feature Flag's value type. JSON values that GrowthBook can repair (for example, objects with unquoted keys) are saved as valid JSON. Values that cannot be parsed or repaired are rejected, including default values and rule variations. The JSON literal `null` is a valid value unless your schema disallows it.

Existing stored values are preserved when they are carried forward unchanged. Creating an empty draft, changing metadata, or toggling a Feature Flag does not require repairing legacy values first. New or changed JSON values must still pass type validation.
