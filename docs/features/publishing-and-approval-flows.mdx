...
description: Draft feature revisions, request review, resolve merge conflicts, and require approval before publishing.
...
![Draft Modal](/static/images/features/feature-publishing-2.png)
...
<Frame>
  <img src="/static/images/features/revision-dropdown-2.png" alt="Feature Revisions" width="800" />
</Frame>
...
<Frame>
  <img src="/static/images/features/resolve-conflicts-2.png" alt="Resolve Conflicts" width="800" />
</Frame>
...
The **Global** section below the rules holds settings that span every rule. One of them concerns [Targeting Projects](/features/environments#delivering-a-feature-to-multiple-projects):

- **Apply approval requirements from Targeting Projects** (on by default) — A Feature Flag delivered into other Projects must also satisfy those Projects' approval rules before publishing. A Targeting Project with a rule of its own must additionally be approved by one of its own reviewers, and those reviewers may review the draft. When off, only the flag's primary Project's rules and reviewers apply. Also available on the [REST API](/api#tag/settings) as `targetingReviewMode`.

![Approval flow organization settings](/static/images/features/approval-flows/org-settings.png)
...
<Frame>
  <img src="/static/images/features/approval-flows/request-approval-to-publish.png" alt="Requesting a review for feature changes" width="800" />
</Frame>
...
<Frame>
  <img src="/static/images/features/approval-flows/drafts-pending-review-list.png" alt="Pending review list in the Drafts tab" width="800" />
</Frame>

### Reviewing

A reviewer needs the **Review** permission for Feature Flags in the flag's
primary Project, covering the environments the draft changes (see
[Environment-scoped approvals](#environment-scoped-approvals)). When the flag is
delivered into a Targeting Project that has an approval rule of its own, that
Project's reviewers may also review the draft; their approval satisfies their
own Project's requirement, not the primary Project's. The person who
...
<Frame>
  <img src="/static/images/features/approval-flows/review-and-approve-button.png" alt="Review and Approve button on a feature" width="800" />
</Frame>

From here, you can see the diff between the currently published changes as well as the comments.

<Frame>
  <img src="/static/images/features/approval-flows/review-draft-changes-diff-2.png" alt="Diff view showing changes between published and draft versions" width="600" />
</Frame>
...
<Frame>
  <img src="/static/images/features/approval-flows/review-status-options.png" alt="Review status options: Comment, Request Changes, Approve" width="600" />
</Frame>

Once the changes are **Approved**, they can be published.

<Frame>
  <img src="/static/images/features/approval-flows/approved-ready-to-publish.png" alt="Approved draft ready to publish" width="800" />
</Frame>
...
approved it. If the flag is delivered into a Targeting Project with an approval
rule of its own, the draft also needs an approval from one of that Project's
reviewers. A user with **Bypass draft approvals** can publish without any of
these, and so can API keys when **REST API always bypasses approval
requirements** is on.

<Frame>
  <img src="/static/images/features/approval-flows/publish-with-admin-bypass.png" alt="Publishing approved changes with admin bypass option" width="600" />
</Frame>

Once published, a green "Live" badge confirms the version is published and active.

![Green badge indicating the published version is live](/static/images/features/approval-flows/revision-live-badge.png)
...
![Compare Revisions button](/static/images/features/approval-flows/compare-revisions-diff.png)
