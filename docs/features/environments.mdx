...
**Ownership stays with the primary project; delivery takes permission in the destination.** Who can edit and publish the feature is keyed on its primary project, and so is review, except that a strict Targeting Project's own reviewers may also approve. Adding a Targeting Project is a write to that project's SDK payloads, so it requires the **Target** permission (the `FlagsTarget` policy, also included in `FlagsFullAccess`) in the project being added. Targeting **all projects** requires that permission with no project restriction. Grant `FlagsTarget` on a project, for example via a team, to let flags owned elsewhere be delivered into it without giving anyone authority over that project's own flags. Removing a Targeting Project, or editing a feature that already targets one, takes nothing extra. A project can also refuse new targeting altogether with the **Allow targeting from other Projects** setting on its Roles & Permissions page (also the `allowTargeting` field on the [REST API](/api#tag/projects)); turning it off blocks new targeting and All Projects without touching flags that already target it.
