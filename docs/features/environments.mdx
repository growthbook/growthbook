...
**Ownership stays with the primary project; delivery takes permission in the destination.** Who can edit, review, and publish the feature is keyed on its primary project. Adding a Targeting Project is a write to that project's SDK payloads, so it requires the **Target** permission (the `FlagsTarget` policy, also included in `FlagsPublish` and `FlagsFullAccess`) in the project being added. Targeting **all projects** requires that permission with no project restriction. Grant `FlagsTarget` on a project, for example via a team, to let flags owned elsewhere be delivered into it without giving anyone authority over that project's own flags. Removing a Targeting Project, or editing a feature that already targets one, takes nothing extra.
