---
title: Swift (iOS)
description: Install the GrowthBook Swift SDK to evaluate feature flags and run A/B tests on iOS, macOS, tvOS, and other Apple platforms.
sidebarTitle: Swift (iOS)
slug: swift
---
...
How often those refreshes reach the network is capped by `ttlSeconds`. See [Cache freshness (TTL)](#cache-freshness-ttl).
...
Cached feature payloads are considered fresh for `ttlSeconds` (default: `60`). While the cache is fresh, feature fetches are served from disk and skip the network. Raise it to cut request volume:
...
This is a staleness gate, not a scheduler — the SDK does not poll in the background. Call `gb.refreshCache()` at natural points such as app foreground or navigation, and the TTL caps how often those calls actually reach the network.

<Note>
**`refreshCache()` respects the TTL**

`gb.refreshCache()` goes through the same freshness check, so it will not force a network request while the cache is still fresh. Lower `ttlSeconds` if you need refreshes to land sooner — there is no per-call override.

</Note>

With `backgroundSync: true`, SSE updates are applied as they arrive and are unaffected by `ttlSeconds`.
