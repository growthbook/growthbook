...
The SDK ships inline type hints ([PEP 561](https://peps.python.org/pep-0561/) `py.typed`), so mypy, pyright, and IDEs like VS Code (Pylance) and PyCharm check your GrowthBook calls with zero configuration. This also gives coding agents a feedback loop: wrong feature usage comes back as a checker error instead of shipping.
...
For dynamic keys (looping over feature names), the generated `FeatureKey` Literal is the escape hatch:
...
Regenerate the file whenever your feature list changes — for example as a CI step. Features whose value type can't be inferred (no `defaultValue` and no typed rules) fall back to `Any` and are listed in a warning at generation time.
