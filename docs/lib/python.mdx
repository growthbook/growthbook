---
title: Python SDK
description: Install the GrowthBook Python SDK to evaluate feature flags and run A/B tests in backends, scripts, and data applications.
sidebarTitle: Python
slug: python
---
...
_Requires **Python 3.9** or above_
...
# The parameter names matter: the SDK invokes this callback with keyword
# arguments (experiment=..., result=..., user_context=...)
...
async def on_experiment_viewed(experiment: Experiment, result: Result, user_context: UserContext):
    ...
    # Use whatever event tracking system you want
    user_id = result.hashValue or "anonymous"
...
```python
def legacy_tracker(experiment, result, user_context):
...
To run experiments on prompts or models and analyze them with Langfuse or Arize Phoenix data, tag each trace with the assignment in the form `gb:<experimentKey>:<variationKey>`. The Python SDK does not have a tracing plugin yet, so build the tag in your `on_experiment_viewed` callback and pass it to your tracing SDK (Langfuse `propagate_attributes(tags=...)` or OpenInference `using_attributes(tags=...)`) at the top of the traced request. Skip keys that contain `:`, since GrowthBook's exposure SQL splits the tag on that character.
...
Keep the `tags` list scoped to a single request so one user's assignments do not leak onto another user's trace. See [Experimenting on LLM Features](/integrations/llm-tracing) for the full setup, including matching the tracer's user id to the GrowthBook hash attribute.
...
result = await client.run(
    Experiment(key="my-test", variations=["blue", "green"]), user
)
```
...
```python
gb.get_feature_value("banner-text", "hello") + 1   # error: str + int
```

Misspelled keyword arguments to `Experiment` and `FeatureRule` are caught too:

```python
Experiment(key="my-test", variations=["a", "b"], weigths=[0.5, 0.5])  # checker error: typo
```
...
```python
def on_experiment_viewed(experiment, result, user_context):  # accepted
    ...

def wrong_names(exp, res, ctx):  # checker error when passed to the client
    ...
```

Passing an `async def` callback to the legacy synchronous client is also a checker error — only the async client schedules coroutines.
...
For compile-time checking of feature _keys_ and per-key value types — the equivalent of the JS SDK's `GrowthBook<AppFeatures>` — generate a typed client from your features JSON. The generator ships inside the SDK (no separate CLI needed):
...
await client.is_on("dark-mode", user)                     # OK
await client.is_on("dark-mod", user)                      # checker error: typo in feature key
await client.get_feature_value("max-items", "10", user)   # checker error: this feature is a number
```
...
gb.is_on("dark-mode")                     # OK
gb.is_on("dark-mod")                      # checker error: typo in feature key
gb.get_feature_value("max-items", "10")   # checker error: this feature is a number
gb.get_feature_value("banner-text", None) # Optional[str]: None when the flag isn't set
gb.eval_feature("dark-mode").value        # Optional[bool] instead of Any
```
...
for k in keys_from_your_feature_payload:
...
```bash
python -m growthbook.codegen --input features.json --output growthbook_features.py
git diff --exit-code growthbook_features.py  # fails if the committed file is stale
mypy .                                       # or: pyright
```
...
If you use the [typed-client generator](#strict-typing-generated-feature-keys), pass the same key as `--decryption-key` so it can read the encrypted payload.
