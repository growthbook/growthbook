...
description: Use the GrowthBook React Native SDK to evaluate feature flags and run A/B tests in iOS and Android apps, including targeting and tracking.
slug: react-native
...
There is an alternate synchronous version of init named `initSync`, which can be especially useful in SSR to prevent hydration mismatches. There are some restrictions/differences:
...
Then, tell GrowthBook to use this polyfill:
...
URL Redirect experiments are not supported in React Native at this time.
...
The JS SDK exports several implementations of this service for reference, although you will need to build your own to work in a React Native environment.
