...
Use `@openfeature/growthbook-provider` with `@openfeature/server-sdk` in Node.js. Use `@openfeature/growthbook-client-provider` with `@openfeature/web-sdk` in the browser. The two providers are not interchangeable: the web provider throws if you register it with the server SDK.
...
await OpenFeature.setProviderAndWait(new GrowthbookProvider(gbClientOptions));
...
await OpenFeature.setProviderAndWait(new GrowthbookClientProvider(gbContext));
...
The JavaScript examples use the Node.js server SDK, which accepts an invocation-level context. On the web SDK, call `OpenFeature.setContext(context)` instead of passing context into each evaluation.
