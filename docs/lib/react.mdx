---
title: React SDK
description: Use the GrowthBook React SDK to evaluate feature flags and run A/B tests in React, including client and server components.
sidebarTitle: React
slug: react
...
The React SDK also supports the [tracing plugin](/lib/js#tracing) for tagging LLM traces in Langfuse or Arize Phoenix with experiment assignments; see [Experimenting on LLM Features](/integrations/llm-tracing).
