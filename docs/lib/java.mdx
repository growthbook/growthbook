---
title: Java SDK
description: Install the GrowthBook Java SDK to evaluate feature flags and run A/B tests in JVM applications, including targeting attributes and tracking callbacks.
sidebarTitle: Java
slug: java
...
import CodeTabsAddDependencyPartial from "/snippets/partials/java/_code-tabs-add-dependency.mdx";
import CodeTabsGBContextBuilderPartial from "/snippets/partials/java/_code-tabs-gbcontext-builder.mdx";
import CodeTabsGBContextBuilderEncryptedPartial from "/snippets/partials/java/_code-tabs-gbcontext-builder-encrypted.mdx";
import CodeTabsGBContextConstructorPartial from "/snippets/partials/java/_code-tabs-gbcontext-constructor.mdx";
import CodeTabsUserAttributesPartial from "/snippets/partials/java/_code-tabs-user-attributes.mdx";
import CodeTabsUsingFeaturesPartial from "/snippets/partials/java/_code-tabs-using-features.mdx";
import CodeTabsTrackingCallbackPartial from "/snippets/partials/java/_code-tabs-tracking-callback.mdx";
import CodeTabsRunExperimentPartial from "/snippets/partials/java/_code-tabs-run-experiment.mdx";
import CodeTabsExperimentResultCallbackPartial from "/snippets/partials/java/_code-tabs-subscribe-experiment-run.mdx";
import CodeTabsExperimentResultSubscribeMultipleCallbackPartial from "/snippets/partials/java/_code-tabs-subscribe-multiple.mdx";
import CodeTabsFeaturesRepository from "/snippets/partials/java/_code-tabs-gbfeatures-repository.mdx";
import CodeTabsFeatureUsageCallback from "/snippets/partials/java/_code-tabs-feature-usage-callback.mdx";
...
[<Frame>![JitPack version badge for the GrowthBook Java SDK](https://jitpack.io/v/growthbook/growthbook-sdk-java.svg)</Frame>](https://jitpack.io/#growthbook/growthbook-sdk-java)
...
| `trackingCallback`     | `TrackingCallback`     | A callback that will be invoked with every experiment evaluation where the user **is** included in the experiment. See [TrackingCallback](#tracking-callback). To subscribe to all evaluated events regardless of whether the user is in the experiment, see [Subscribing to experiment runs](#experiment-run-callback). |
...
The `GBFeaturesRepository` will automatically refresh the features when the features become stale. Features are considered stale every 60 seconds. This amount is configurable with the `swrTtlSeconds` option, which also sets the interval of the background poll that drives the refresh.
...
Options options = Options.builder()
        .apiHost("https://cdn.growthbook.io")
        .clientKey("sdk-abc123")
        // Consider features stale after 10 minutes
        .swrTtlSeconds(600)
        .build();
...
<Warning>
**Re-read `getFeaturesJson()` per request**

`GBContext` holds the features JSON you pass it as a fixed snapshot. If you build a single `GBContext` at startup, refreshed features will never reach it no matter what `swrTtlSeconds` is set to. Either call `getFeaturesJson()` again when building each request's context, or use the [Enhanced Client](#enhanced-client-recommended), which handles this for you.
