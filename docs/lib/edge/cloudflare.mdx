...
description: Evaluate GrowthBook feature flags and Visual Editor experiments at the edge on Cloudflare Workers to cut flicker before HTML ships.
...
Regardless of your use case, our Cloudflare integration makes it easy to synchronize feature and experiment values between GrowthBook and Cloudflare's KV store. This eliminates the network request to the GrowthBook API, unlocking blazingly fast edge-side and client-side SDK performance.
...
<Tip>
**Get up to speed with Cloudflare Workers**
...
```bash Yarn
yarn add --save @growthbook/edge-cloudflare
```

```bash pnpm
pnpm add --save @growthbook/edge-cloudflare
```
...
In these cases, set the environment variable `PERSIST_UUID="true"` (via `[vars]` in your `wrangler.toml`). The Edge App will then include a `Set-Cookie: gbuuid=...` header on its response, ensuring the cookie is stored on the first request.
