...
description: Add a GrowthBook HTML script tag to any website or low-code platform to run feature flags and Visual Editor experiments.
sidebarTitle: HTML Script Tag
slug: script-tag
...
This option is quick to add. Visual Editor experiments run when the page loads and do not require code. Feature flag experiments still require you to evaluate the flag in code (for example with `isOn`). Adding the script does not enroll users in those experiments by itself.
...
- `data-tracking` - Set to `growthbook` to send experiment view events that already fired to GrowthBook's warehouse. This does not enroll users in an experiment.
...
When a user is included in an experiment, a tracking event is fired. There is built-in support for Segment.io (`analytics.js`), GA4 (`gtag`), and Google Tag Manager (`dataLayer`).
