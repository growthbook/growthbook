...
description: Use the GrowthBook Next.js adapter for Vercel Flags to evaluate feature flags and experiments in App Router and hybrid backends.
...
```bash Yarn
yarn add --save @flags-sdk/growthbook
```

```bash pnpm
pnpm add --save @flags-sdk/growthbook
```
...
You may access the underlying GrowthBook instance. Specifically, our Flags SDK adapter wraps the `GrowthBookClient` class. See the [JavaScript SDK](/lib/js). All user evaluation options (attributes, tracking callbacks, sticky buckets) are applied at the `userContext` level (not globally).
...
<Note>
**Vercel Edge Config limitations**

Vercel's Edge Config is subject to storage size limitations. If your SDK payload is excessively large, you may not be able to populate the your Edge Config. See [Vercel Edge Config limits](https://vercel.com/docs/edge-config/edge-config-limits).
