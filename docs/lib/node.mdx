---
title: Node.js
og:description: GrowthBook SDK for Node.js
slug: node
...
```bash Yarn
yarn add --save eventsource
```

```bash pnpm
pnpm add --save eventsource
```
...
As a general philosophy, we aim to keep the SDK 100% backwards compatible at all times. View the [Changelog](https://github.com/growthbook/growthbook/blob/main/packages/sdk-js/CHANGELOG) for a complete list of all SDK changes.
