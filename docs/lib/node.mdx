...
<Note>
**Streaming vs cache refresh vs `backgroundSync`**
...
A long-running process that never enables streaming, never sets `pollingInterval`, and never calls `refreshFeatures()` (directly or on a timer) will keep the payload from `init` until restart.
