---
title: Node.js SDK
description: Install the GrowthBook Node.js SDK to evaluate feature flags and run A/B tests in server-side JavaScript. Requires Node 18 or above.
slug: node
...
The tracing plugin records every experiment assignment as a tag of the form `gb:<experimentKey>:<variationKey>` so you can attach it to an LLM trace in Langfuse, Arize Phoenix, or any OpenTelemetry-based tool. GrowthBook's Langfuse and Phoenix data source integrations read the tags back out of the trace to compute exposures. See [Experimenting on LLM Features](/integrations/llm-tracing) for the full setup and the plugin options in the [JavaScript SDK docs](/lib/js#tracing).

Pass the plugin in the `GrowthBookClient` options. The shared client has no assignments of its own, so the plugin runs for each `createScopedInstance()` call and every scoped instance keeps its own tag set. Read the tags off the scoped instance with `getTracingTags()`.
...
app.post("/chat", async (req, res) => {
  const scoped = gbClient.createScopedInstance({
    attributes: { user_id: req.user.id },
  });

  // Evaluate flags first so the tags exist before the trace starts
  const promptLabel = scoped.getFeatureValue("support-prompt", "production");
  const tags = getTracingTags(scoped); // e.g. ["gb:support-prompt:prod-b"]

  // Hand `tags` (and req.user.id as the user id) to your tracing SDK
});
```
...
There is an alternate synchronous version of init named `initSync`, which can be useful in some environments. There are some restrictions/differences:
...
Evaluating the feature is what enrolls a user and fires `trackingCallback`. See [Feature Flag Experiments](/features/feature-flag-experiments).
...
The DevTools plugins are designed to work with both GrowthBookClient and GrowthBook (legacy) SDK instances. Your SDK must be in dev mode (`enableDevMode: true`) for the plugin to work.
...
- It snapshots `gb.logs` **at call time**. Call it _after_ you've finished evaluating every feature/experiment you want visible in DevTools for that request, not before.
