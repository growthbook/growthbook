export const SdkResources = ({ sdk }) => {
  ...
  const Row = ({ href, icon, children }) => {
    const rowClass =
      "mint-flex mint-items-center mint-gap-2 mint-p-3 mint-border-b mint-border-[#00002f26] dark:mint-border-[#d6ebfd30]";
    const iconSpan = (
    ...
    if (href) {
      return (
        <a
          ...
          target="_blank"
          rel="noreferrer"
          className={rowClass + " mint-text-inherit mint-no-underline hover:mint-text-inherit hover:mint-underline"}
        >
          {iconSpan}
          {children}
        </a>
  ...
  return (
    <section className="grid mb-4">
      ...
      {packageRepos.map((repo) => (
        <Row key={repo.name} href={repo.url} icon={<Package />}>
          {repo.name}
        </Row>
      ))}

      {examples.map(({ url, name }) => (
        <Row key={name} href={url} icon={<FileCode />}>
          {name}
        </Row>
      ))}
