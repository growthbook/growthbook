...
Custom Hooks can validate different types of resources. Today they support **features** (and feature revisions) and **saved groups**.
...
- **Global** — runs for every resource of that type.
- **Project** — runs only for resources in the selected projects.
- **Feature** / **Saved Group** — runs only for a single feature or saved group.

Global and project-scoped hooks are managed under **Settings → Custom Hooks**. Resource-scoped hooks are created and managed from the resource's **Validation** tab (on the feature or saved group detail page) by anyone who can edit that resource; that tab also lists any global and project hooks that apply to it.
...
Called whenever a saved group is about to be created or updated (including when a saved group revision is published). Receives the full saved group object as input.

Example: Require every saved group to have a description.

```js
if (!savedGroup.description) {
  throw new Error(
    "Saved groups must have a description."
  );
}
```

Example: Warn if a saved group isn't scoped to any project.

```js
if (!savedGroup.projects || savedGroup.projects.length === 0) {
  addWarning(
    "Consider scoping this saved group to at least one project."
  );
}
```

Example: Enforce a naming convention for saved groups.

```js
if (!/^[a-z0-9-]+$/.test(savedGroup.groupName)) {
  throw new Error(
    "Saved group names must be lowercase, using only letters, numbers, and dashes."
  );
}
```
