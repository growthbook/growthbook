...
- `status` — `"approved"` or `"changes-requested"`. A verdict that was retracted no longer appears, and all verdicts reset when a new review cycle starts (review re-requested or recalled).
