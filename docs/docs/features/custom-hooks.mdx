...
Internal bookkeeping writes skip this hook. For example, when GrowthBook flips the flag that tracks whether an experiment has visual changes or URL redirects, your validation rules are not run, so those actions never fail because of an unrelated validation rule.

Example: Require at least one contextual attribute, and require a linked ticket when the "checkout" attribute is selected. This validates one multi-select Custom Field against another.
