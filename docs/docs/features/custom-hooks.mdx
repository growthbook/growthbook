...
Called whenever a [config](/features/configs) is created or updated. Receives the config (its fields, staged `value`, a `lineage` object, and `isHookTarget` / `hookTargetKey`) as input. `config.value` is a parsed JSON object — read its keys directly, no `JSON.parse` needed. Since a config-scoped hook also runs for descendants, `config.isHookTarget` is `true` only for the exact config the hook is pinned to.
