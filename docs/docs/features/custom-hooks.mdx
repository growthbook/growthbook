...
Called whenever a [config](/features/configs) is created or updated. Receives the config (its fields plus staged `value`) as input.
...
- `config` — the config's published content: `key`, `name`, `project`, staged `value`, `schema`, and lineage (`parent`/`extends`).
...
Example: Block publishing a value that exceeds a limit.
