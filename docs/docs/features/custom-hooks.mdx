...
Managing a Global, Project, or Config-scoped hook takes the **Custom Hooks** permission (`manageCustomHooks`, an admin-level policy). A hook scoped to a single Feature Flag or Experiment instead takes edit rights on that resource, so a flag's owner can manage its validation without org-wide hook access.
