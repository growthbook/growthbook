...
In order to use namespaces, simply create a new namespace or modify an existing one in _Experimentation → Namespaces_ in the GrowthBook UI's left navigation bar. It's essential to ensure that all experiments within the namespace use the same hash attribute (assignment attribute).
...
You can create and manage archetypes in the _Archetypes_ page under the SDK Configuration.
