...
ID Lists are limited to 1 MB.

![Saved Group UI, focusing on adding an ID list](/images/saved-groups-id-list.png)

:::note
Using multiple, large ID Lists may impact performance as it increases the SDK payload. It's recommended to keep ID Lists as concise as possible.
:::
