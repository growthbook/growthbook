...
If you need support for something more advanced (e.g. nested `AND`s, `OR`s, and `NOT`s), you can enter targeting conditions with JSON instead.

![Advanced JSON targeting conditions](/images/targeting-advanced.png)

The JSON structure is based on the MongoDB Query syntax. The following operators are supported:
