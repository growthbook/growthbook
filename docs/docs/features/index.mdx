...
### Requiring Approvals on Feature changes

You are able to require draft feature flags to have a review before publishing.
The only excption to this rule is if you are an admin you can Overide the review process from the reqest review modal to publish without a review.
To set your organization to require reviews:

1. Go to [General Settings](/settings) in growthbook
2. Under the features section turn on **Require approval to publish changes**
3. All Draft features will now require review before publishing
