...
A constant has a base **value** plus optional **per-environment overrides**. When a flag is built for an environment, the constant resolves to:

```
environmentValues[environment] ?? value
```

So a constant with a value of `https://api.example.com` and a `staging` override of `https://staging-api.example.com` resolves to the staging value when building the staging payload, and the base value everywhere else.

A constant must define at least a value or one environment override.
