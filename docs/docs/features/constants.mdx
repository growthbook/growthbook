...
**Own keys are a deep (targeted) patch.** When an own key and the merged base are both objects, they merge **recursively, key by key** — you restate only the leaves you want to change and inherit the rest. For example, extending a base of `{ "retry": { "timeouts": { "connect": 1000, "read": 5000 } } }` with own keys `{ "retry": { "timeouts": { "read": 8000 } } }` resolves to `{ "retry": { "timeouts": { "connect": 1000, "read": 8000 } } }` — `connect` is inherited, not dropped. Two things replace wholesale rather than merging:

- **Arrays and scalars** — a patched array or scalar replaces the inherited value outright (arrays are never merged element-by-element).
- **`null`** — writing `null` sets the value to `null`; it does **not** delete the inherited key.

A referenced chunk is always applied **whole**: an `$extends` reference (or a nested object that itself uses `$extends`) drops in as a complete unit — the deep patch never reaches inside it. A constant's value can itself reference other constants; they're resolved recursively.
