...
**Own keys are a deep (targeted) patch.** When an own key and the merged base are both objects, they merge **recursively, key by key** — you restate only the leaves you want to change and inherit the rest. For example, extending a base of `{ "abr": { "levels": { "low": 1500, "high": 12000 } } }` with own keys `{ "abr": { "levels": { "high": 16000 } } }` resolves to `{ "abr": { "levels": { "low": 1500, "high": 16000 } } }` — `low` is inherited, not dropped. Two things replace wholesale rather than merging:
