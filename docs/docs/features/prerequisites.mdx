...
- **Deterministic** states (**live** and **not live**) are applied to your features within the GrowthBook.
...
1. Inline prerequisites can be applied a feature's [override rule](/features/rules) level, and can be environment-specific. This comes with the added benefits of feature draft releases and approvals.
...
- **Deterministic** states (**live** and **not live**) are calculated using your evaluation condition, which is **_not_** limited to `is true`. As before, no run-time evaluation of prerequisites is required in the SDK.
  ...
  - **Not live** prerequisites will always block their dependent rules or experiments.
