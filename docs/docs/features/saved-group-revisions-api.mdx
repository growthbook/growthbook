...
When your org [requires approvals for saved groups](/features/publishing-and-approval-flows#saved-group-approval-flows), publishing a draft is blocked unless the revision is in `approved` status, the caller has the `bypassApprovalSavedGroups` permission on every target project, or the org has enabled **REST API always bypasses approval requirements**.
...
1. **Per-call bypass** — `POST /saved-groups` and `POST /saved-groups/{id}` accept `bypassApproval: true` and write directly to the live saved group, skipping the revision flow entirely. The caller must hold `bypassApprovalSavedGroups` on every target project.
2. **Per-user bypass on publish** — `POST .../revisions/{version}/publish` succeeds for callers with `bypassApprovalSavedGroups` on the saved group's projects, even if the revision is not `approved`.
