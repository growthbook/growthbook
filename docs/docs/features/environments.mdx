...
To use multiple environments in the same codebase, use environment variables to set a dynamic key (e.g., `GROWTHBOOK_CLIENT_KEY='sdk-abc123'`) and reference the variable in your code. Depending on your framework, some environment variables aren't exposed on the front-end unless given an appropriate prefix (e.g., `NEXT_GROWTHBOOK_CLIENT_KEY='sdk-abc123'` for Next.js client code).
