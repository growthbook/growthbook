...
| **SDK impact**  | Each SDK connection maps to one environment  | SDK connections can be scoped to a project to reduce payload size; features and rules can also target projects to scope _delivery_ (advanced — see below) |
