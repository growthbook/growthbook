...
Anyone with the Review permission for feature flags (see [Permissions](/account/user-permissions)) can serve as a reviewer, except the user who created the request.
...
Anyone with permission to publish feature flags can publish changes once they are marked as **Approved**. Users with the bypass-approvals permission can skip the review requirement by selecting the checkbox at the top of the modal.
...
When approvals are enabled, the **Review & Publish** button on a saved group draft becomes **Request Approval to Publish**. After you submit, reviewers see the request in the saved group's revision list with a **Pending Review** status and can open the **View Approval Request** modal to view the diff, leave comments, and choose **Comment**, **Request Changes**, or **Approve**. As with features, anyone with the Review permission for saved groups can review — except the user who created the request when **Require approval from a non-editor** is enabled.

Once approved, anyone with permission to publish saved groups can publish the revision. Users with the saved-group bypass-approvals permission on every project the group belongs to can publish without going through review.
