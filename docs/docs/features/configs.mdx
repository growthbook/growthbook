...
When you genuinely need to publish anyway, acknowledge the block: in the app, confirm the warning; over the REST API, pass `?ignoreWarnings=true` on the publish request. Turning the guard _off_ for a config requires the **bypass-approvals** permission.
