...
A config's value can also interpolate [string constants](/features/constants) with `{{ @const:key }}` inside any string field, exactly as in feature values — including constants with per-environment overrides, which is how an environment-varying value enters an environment-agnostic config.
...
Hooks are scoped Global, Project, or Config, and a config-scoped hook can opt to cover the whole family via **Include descendant configs**. Manage them from a config's **Validation** tab or under **Settings → Custom Hooks**.
...
When you genuinely need to publish anyway, a user with the **bypass-approvals** permission can override the block (over the REST API, pass `?ignoreWarnings=true` on the publish request). Turning the guard off for a config also requires that permission.
...
Configs and their revisions are fully manageable over the [REST API](/api/#tag/configs) — create, update, archive, delete, list references, fetch the full lineage, see where each key is used, import/export value and schema, and drive the revision/approval flow programmatically. Webhook events (`config.*`, including `config.revision.*`) fire on every change, modeled after the feature and constant events.
