...
The canonical form is a **MongoDB-style condition** — the same condition syntax as feature targeting rules — extended with `{ "$ref": "path" }` to compare against another field of the value instead of a literal. The REST API accepts any of three formats on write and normalizes to the canonical form (responses always return the MongoDB-style object):

- **MongoDB-style condition** — `{ "buffer.target_seconds": { "$lte": { "$ref": "buffer.max_seconds" } } }`, with the full condition operator set (`$eq`, `$ne`, `$lt`/`$lte`/`$gt`/`$gte`, `$exists`, `$and`/`$or`/`$not`/`$nor`, …).
- **CEL-style expression string** — `"buffer.target_seconds <= buffer.max_seconds"`: comparisons (`==`, `!=`, `<`, `<=`, `>`, `>=`), `&&`, `||`, `!`, and parentheses; a bare identifier on the right-hand side references another field.
- **JSONLogic object** — `{ "<=": [{ "var": "buffer.target_seconds" }, { "var": "buffer.max_seconds" }] }`, with `and`/`or`/`!`.

**Nested fields are supported.** A dot-separated path like `buffer.target_seconds` reaches into nested objects — as the tested field and as a `$ref`/`var` target alike. A path that doesn't exist evaluates as `null` rather than erroring.
