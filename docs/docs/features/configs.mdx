...
- **Off (warn mode)** — violations surface as a bypassable warning; the API proceeds when the request body sets `"ignoreWarnings": true`.

Setting `"skipSchemaValidation": true` in the request body skips validation rules together with schema validation; it is only honored for callers with org-wide bypass authority.
