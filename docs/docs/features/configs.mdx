...
Schema errors, validation-rule failures, and **downstream schema breaks** (a change that makes a dependent config or config-backed feature value violate its schema) are **validation-class** failures — they block a publish and are **not** cleared by `"ignoreWarnings": true` (that flag only acknowledges soft warnings, like a running experiment reading a value or dependents dropped by an archive). To force past one, a caller with org-wide bypass authority (the **bypass-approvals** permission) sets `"skipSchemaValidation": true` in the request body; it is ignored for anyone else. The editor also surfaces these inline as you edit — the failing field rows are flagged so you see them before publishing.

The **Block publishing on JSON schema errors** organization setting (on by default) still governs the strictness of the standalone per-request value check on non-publish writes; validation-class publish failures are enforced regardless.
