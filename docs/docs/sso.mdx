...
SSO is available on GrowthBook Cloud or Self-hosted via OpenID Connect. To enable SSO on your self hosted GrowthBook
...
On GrowthBook Cloud, organization admins can set up and edit their SSO connection directly under **Settings** → **Members**.
Select your identity provider, then enter the **CLIENT_ID**, **CLIENT_SECRET**, and (for some providers) the **TENANT_ID**
or **BASE_URL**. You can use the instructions below to get these values from your provider. To associate email domains with
your SSO connection (so users signing in with a matching email can auto-join your organization), contact your account
representative or [hello@growthbook.io](mailto:hello@growthbook.io) — domains must be verified by GrowthBook first.
