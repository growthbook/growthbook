...
### 1. Enable DataDog Real User Monitoring (RUM)
...
GrowthBook with [DataDog RUM](https://www.datadoghq.com/product/real-user-monitoring/). If using a broswer RUM library version < v5.17.0 you'll need to enable additional flags as explained [here](https://docs.datadoghq.com/real_user_monitoring/guide/setup-feature-flag-data-collection/?tab=browser#setup).

### 2. Report GrowthBook feature flag evaluations to DataDog

In the `trackingCallback` function add the following line:

```js
 trackingCallback: (experiment, result) => {
    datadogRum.addFeatureFlagEvaluation(experiment, result);
  }
```
...
### Build a DataDog Dashboard & Monitor
...
#### Update Feature Flag via GrowthBook REST API.
