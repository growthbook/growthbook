...
Once you're in the code editor, locate the `theme.liquid` file and add the following snippet to the bottom of the file, just before the closing `</body>` tag. Be sure to replace the `data-client-key` with the `Client Key` you located above.
