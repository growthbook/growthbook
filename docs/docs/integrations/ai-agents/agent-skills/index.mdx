...
description: "Run the full feature-flag and experiment lifecycle from your AI coding agent. Install the GrowthBook Agent Skills for Claude Code, Cursor, and other agents. No MCP server required."
...
GrowthBook Agent Skills let you run the entire feature flag and experiment lifecycle from your AI coding agent (create a flag, roll it out, run an experiment, read the results, and clean up afterward) without leaving your editor and **without running an MCP server**.

Each skill is a playbook your agent follows: it knows the GrowthBook REST API, the right order of operations, and the guardrails (draft → review → publish, safe rollouts, two-step deletes) that keep changes safe. Skills work in Claude Code, Cursor, Codex, and other agents that follow the [Agent Skills](https://agentskills.io) standard.

{/* TODO: add a short demo video or screenshot of a skill running (e.g. flag-create end to end). Place media under /static/images/integrations/agent-skills/ or /static/videos/. */}
...
```
  ...
  <TabItem value="other" label="Cursor, Codex, and others">

Install globally with the [Agent Skills](https://agentskills.io) CLI:
...
The library has 23 skills. The feature-flag skills are documented in detail on **[Feature flag skills](/integrations/ai-agents/agent-skills/feature-flags)**, and the experiment skills on **[Experiment skills](/integrations/ai-agents/agent-skills/experiments)**.

**Setup**

- `gb-setup`: configure and validate your API credentials.

**[Feature flags](/integrations/ai-agents/agent-skills/feature-flags)**

- _Revision lifecycle:_ `flag-revisions`, `flag-review`, `flag-publish`
- _Operations:_ `flag-create`, `flag-metadata`, `flag-default-value`, `flag-toggle`, `flag-prerequisites`, `flag-cleanup`
- _Rules:_ `flag-rules`, `flag-targeting`, `flag-schedule`, `flag-ramp`, `flag-monitoring`, `flag-experiment`
- _Discovery:_ `flag-search`, `flag-graph`

**[Experiments](/integrations/ai-agents/agent-skills/experiments)**

- `experiment-brainstorm`, `experiment-design`, `experiment-launch`, `experiment-analyze`, `experiment-stop`
...
- [Agent Skills vs. the MCP Server](/integrations/ai-agents): which to use
- [GrowthBook REST API](/app/api): what the skills call under the hood
