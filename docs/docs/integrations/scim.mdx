...
<CommercialFeature feature="scim" description="SCIM requires Single Sign-On (SSO) to be enabled. Please refer to our [SSO documentation](https://docs.growthbook.io/sso) for more details on setting up SSO. Cloud users should contact their Account Representative for SSO configuration. Currently, GrowthBook only supports Okta and Azure AD/Microsoft Entra ID as identity providers." />
