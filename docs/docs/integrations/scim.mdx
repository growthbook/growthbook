...
GrowthBook does support the ability to optionally [define a role when provioning a user](#how-to-define-user-role-when-provisioning). Project-level permissions are not supported when provisioning a user via SCIM. That must be done via the GrowthBook application.
...
**Can I define a Groups role when I push the group to GrowthBook?**

While Okta does support the ability define custom attributes for a Group like it does for a User, Okta doesn't pass these custom attributes to GrowthBook when creating or updating a group. As some other identity providers might support this, GrowthBook will honor the `growthBookRole` attribute if it is passed in the SCIM payload when creating or updating a group.
