...
If self-hosting GrowthBook on a VPN or behind a firewall, you will need to make sure to whitelist [Okta's IP addresses](https://support.okta.com/help/s/article/list-of-ip-addresses-that-should-be-allowlisted-for-inbound-traffic?language=en_US) to ensure the SCIM API calls can hit GrowthBook's SCIM server.
...
#### Okta: How to define user role when provisioning
...
1. First, verify that your GrowthBook organization is on an enterprise plan with SSO enabled.

![](../../static/images/integrations/azure/growthbook-entra-scim-add-enterprise-application.png)

2. Log in to your Azure account, select **Microsoft Entra ID**, and in the left-nav select **Manage** > **Enterprise Applications** > **+ New Application**

3. Here, you can select **+ Create your own application**, add an app name, and select the **Integrate any other application you don't find in the gallery (Non-gallery)** option.

![](../../static/images/integrations/azure/growthbook-entra-scim-create-application.png)

3. Once created, you can select the box to **Provision User Accounts**

![](../../static/images/integrations/azure/growthbook-entra-scim-setup-provisioning.png)

4. Here, you can click the CTA to **Get Started**

![](../../static/images/integrations/azure/growthbook-entra-scim-provisioning-empty-state.png)

5. Then, you can change the Provisioning Mode to **Automatic** before adding the Base URL and your OAUTH Bearer Token

![](../../static/images/integrations/azure/growthbook-entra-scim-add-credentials.png)
...
If self-hosting GrowthBook on a VPN or behind a firewall, you will need to make sure to whitelist Azure AD's / Entra ID's IP addresses to ensure the SCIM API calls can hit GrowthBook's SCIM server.

:::

7. After adding your credentials, click **Test API Credentials** to ensure they are valid. If they pass the test, click **Save**.

8. Once saved, the **Mappings** section should become available. Here you can enable/disable syncing of groups and users. The two screenshots below show the mappings necessary - these need to match exactly.

![](../../static/images/integrations/azure/growthbook-entra-scim-user-mapping.png)

![](../../static/images/integrations/azure/growthbook-entra-scim-group-mapping.png)

:::note

Syncing groups is optional. When a group is synced with GrowthBook, it will create a team in your GrowthBook organization and add any members in the group to the team. This is handy as you can give a team a global role, and each user will inherti that role. With Azure AD / Entra ID, all members of the group will be synced to GrowthBook.

:::

![](../../static/images/integrations/azure/growthbook-entra-scim-start-provisioning.png)

9. Now, you can begin adding users and groups, before clicking the **Start Provisioning** button.

## Frequently Asked Questions

**What features are supported with SCIM?**

Currently, GrowthBook supports provisioning and deprovisioning users and groups.

**What identity providers are supported?**

At present, GrowthBook only officially supports Okta and Azure AD / Microsoft Entra ID, but we are actively working on adding support for additional identity providers. Given the standarized nature of SCIM, its possible other identity providers will work, but we do not have official guides at this time.
...
If you use Okta and you notice that some users that you expect to be in your GrowthBook team from your identity provider are not being added in GrowthBook, double check that those users are assigned to the GrowthBook SCIM application in your identity provider. With Okta only users that are both assigned and withing the group will be synced to the corresponding team. With Azure AD / Entra ID, the user only needs to be in a group, and the group assigned to the application in order to be provisioned.
...
**Why can I not change a user's name via my Identity Provider?**

A user in GrowthBook can belong to many organizations, so no single organization has the permission to update a user's name, as that would affect all organizations the user is a member of.

**Can I define a user's `growthbookRole` via Azure AD / Microsoft Entra ID?**

It should be possible by adding a custom attribute and mapping it accordingly, but we do not have an official guide on that at this time.
...
Given that only a user's `growthbookRole` can be updated, we suggest de-provisioning the user, updating their email address, and then re-provisioning the user. This will create a net-new user within GrowthBook with the updated email address.
