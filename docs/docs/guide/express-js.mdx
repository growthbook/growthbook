...
Now lets install the GrowthBook JavaScript SDK
...
Lets re-start the Node process and make sure you see the "GrowthBook initialized" message in the console. Since we are using a `.env` file, we need to modify the command to tell Node.js to load it.
...
<img
  src="/images/guides/express-js-1-create-feature.png"
...
<img
  src="/images/guides/express-js-2-targeting-rule.png"
...
After publishing, restart the Node process once more and refresh the page. You should now see the message in Spanish!
...
We showed here how to use a simple feature flag in an Express.js app and target individual users. This barely scratches the surface of what you can do with GrowthBook.
...
View the [full Node.js docs](/lib/node) for more information on all of the options available in the GrowthBook SDK, including streaming updates, persistant caching, and more.
