...
This guide walks you through using GrowthBook with Google Analytics 4 (GA4) to track and report on the impact of your experiments
on your business. There are a few parts to this, connecting GA4 to BigQuery, connecting GrowthBook to BigQuery, and then configuring
GrowthBook to track correctly with GA4.
...
With the data source connected, you can use the GrowthBook SDK run A/B tests defined in the GrowthBook UI, and send the
experiment events to GA4. The best way to do this is to use the GrowthBook SDK. Once implemented, SDK will automatically
send the experiment exposure event to GA4, and you can use the GrowthBook UI to create your experiment and track the results.
...
(ga\_). There are a few ways to grab this Id from your code. One of the easiest ways with Javascript is using the following code:
...
If the attribute you used for targeting an experiment (split by) is not set or is null when the SDK assigns a user, then
the feature will return null or off and the `trackingCallback` will not be called. For JS/React SDKs you can use our
[Chrome developer tool](https://chrome.google.com/webstore/detail/growthbook-devtools/opemhndcehfgipokneipaafbglcecjia) to help you debug

:::

Implementing the experiment variations can be done with code, using the feature flags, or by using our visual editor.
