...
On first load, the cookie may not be available, so you can either wait for it, use the js way above, or set your own ID that can be used for immediate assignment.
...
If you are using GA4 via GTM, adding custom events via the dataLayer will not automatically trigger GA4 events. You will need to create a custom trigger in GTM to listen for these events and then fire the appropriate GA4 event. See below.
...
By default, custom events are not sent to GA4 and must be manually configured in the GTM dashboard first. This section
describes all the steps required to get custom events tracking correctly in GA4 via GTM.
