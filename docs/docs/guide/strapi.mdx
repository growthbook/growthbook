...
For this tutorial, we'll create create a Product collection with a [Dynamic Zone](https://docs.strapi.io/dev-docs/backend-customization/models#dynamic-zones) field that holds Product Card components. On our frontend, we'll show 3 shoe product cards and use GrowthBook to A/B test which shoe is displayed in the third spot.
...
Next, create a new collection called **Product**. Add a Dynamic Zone field called `products` that uses the `productCard` component. Dynamic Zones let us create multiple variations of content within a single collection - perfect for A/B testing different versions.
...
The next step is to create a feature flag in GrowthBook. With feature flags, we can toggle content on and off, implement advanced targeting, or use the flags to run A/B tests. In this example, we'll create a feature flag for the **Product** collection we just created in Strapi.
...
2. `variations`: The content variations fetched from Strapi, as part of a collections Dynamic Zone.
3. `featureFlagKey`: The feature [flag key from GrowthBook](#create-feature-flags-and-ab-tests-in-growthbook).
...
export default function getVariation<T>({
  ...
  return (variations?.[result] as T) ?? null;
}
```

### Update page.tsx

The last part of the process is to update `app/page.tsx` to use the variation returned by the `getVariationHelper`.
...
:::tip
To measure the success of your experiments, make sure to track relevant metrics (like clicks or conversions) using your analytics platform. See our guide on [setting up analytics with GrowthBook](https://docs.growthbook.io/app/datasources).
:::

## Summary

In this tutorial, you've built a complete A/B testing system that combines the power of Strapi's content management with GrowthBook's experimentation capabilities!
...
- [Set up advanced targeting](https://docs.growthbook.io/features/targeting) for your experiments and feature flags.
