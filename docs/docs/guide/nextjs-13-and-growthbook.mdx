---
title: GrowthBook and Next.js 13 (beta)
description: This guide walks you through using GrowthBook with Next.js 13 (beta).
sidebar_label: Next.js 13 (beta)
slug: nextjs-13-and-growthbook
---

# GrowthBook and Next.js 13 (beta)

This document is a guide on how to add GrowthBook feature flags to your Next.js application. It assumes you are starting from scratch, and are using the new 'app directory' pattern introduced in Next 13.

If you already have a NextJS application, you can skip to step 2.
...
Then we can modify the generated React code to work with the Next.js framework.

You'll first need to create a **client component** that will contain the GrowthBookProvider. Create a file `app/GrowthBookProvider.tsx` with the following contents:
...
Once created, we'll need to include this component in our `app/layout.tsx` page. Open the file and add the following:
...
We can now add another client component called **WelcomeBanner** and conditionally render a welcome message based on the state of the feature.

Create a new component `app/WelcomeBanner.tsx` with the following contents:
...
And then put your welcome message somewhere on the page (in `app/page.tsx`).

First import the banner:

```ts
import WelcomeBanner from "./WelcomeBanner";
```

Then insert into the JSX (above the Next.js Logo):
...
In the hard-coded targeting attributes we set in `app/GrowthBookProvider.tsx`, we designated ourselves as an internal employee. Let's use this attribute to turn on the feature for the whole internal team:
...
<img
  ...
  alt="NextJS app with feature"
  style={{ width: 500, margin: "0 auto" }}
/>

If you change employee to false in `app/GrowthBookProvider.tsx`, you should see the welcome message disappear.
...
In the targeting attributes in `app/GrowthBookProvider.tsx`, make sure employee is set to false. That will ensure you skip the first rule we made and fall into the second rollout rule.
...
Try changing the user id in the targeting attributes in `app/GrowthBookProvider` to a few different random strings and see what happens. You should notice about half of the time the welcome message shows up and half of the time it doesn't.
