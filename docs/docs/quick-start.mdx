...
You can use GrowthBook for Feature Flags, Experiment Analysis, or both, and you're welcome to skip to section that
...
Once you're in the GrowthBook app, you need to add an SDK endpoint to allow GrowthBook to communicate to your app.
...
## Step 2. Integrate GrowthBook with your application
...
or you can read the full documentation [here](/lib).
...
## Step 3. Add one additional event

To analyse the results of an AB test, you'll need to track which users were exposed to which variation. GrowthBook
itself does not collect raw events. Instead, GrowthBook works with most popular event trackers. This actually makes
implementation easier as you can use all your existing events that you're already tracking. You will need to just add
one additional event to track when a user was exposed to an experiment. You should see a `trackingCallback` method in
...
popular event trackers [here](/event-trackers).

:::note **Don't have an event tracker yet?**

Most companies we speak to have Google Analytics installed, which will [work great](/event-trackers/GA4-google-analytics).
If you don't use GA, or are not testing on a website, we recommend using something like RudderStack, Jitsu or Segment.
...
We support all the popular SQL data warehouses such as Snowflake, Redshift, Databricks, and even Mixpanel.
...
the data source for your event tracker [here](/event-trackers).
...
presented with a list of data warehouses and event trackers. Select the one you're using, or click on a custom, and
follow the instructions to connect to your data warehouse. You can find guides on how to set up the data source for your
specific data warehouse [here](/warehouses).

## Step 5. Add a metric or two

GrowthBook needs to know what metrics you want to track for your experiments. You can add metrics in the GrowthBook UI,
...
You can read more about metric types [Metrics](/app/metrics).

## Step 6. Add attributes to GrowthBook
...
You are now ready to create features and add experiment rules, or use the visual experiment editor.
