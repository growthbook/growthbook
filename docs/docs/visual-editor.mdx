...
All you need to get started is the [GrowthBook Chrome Extension](/tools/chrome-extension) [installed](https://chrome.google.com/webstore/detail/growthbook-devtools/opemhndcehfgipokneipaafbglcecjia) on your Chrome Browser. Once you've added it to your Chrome browser, you need to add the API key (from Settings -> API Keys). If you are using Cloud, the API Host will be `https://api.growthbook.io`.
