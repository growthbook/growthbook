...
If you do decide to use a framework like React or Vue, be sure to see our [section on SPA support below](#spa-support).
...
If you also plan to use Feature Flags, we recommend doing a full custom integration with our [JavaScript](/lib/js) or [ReactJS](/lib/react) SDKs. This integration requires the help of a developer to set up.
...
If you only plan on using the Visual Editor or do not have access to a developer, you can use our pre-built script tag instead.

First, create an SDK Connection in GrowthBook and note your **Client Key**.

Then, add the script tag below to your website and replace `YOUR_CLIENT_KEY_HERE` with your actual client key:

```html
<script async data-client-key="YOUR_CLIENT_KEY_HERE"
...
All of this default behavior is customizable by defining a `window.growthbook_config` object BEFORE you load the script tag:

```html
<script>
window.growthbook_config = {
  trackingCallback: (experiment, result) => {
    customTrack("Viewed Experiment", {
...
Alternatively, you can customize some settings by adding `data-*` attributes to the pre-built script tag directly. Here's an example of overriding the API host and setting a decryption key:
...
### Content Securit Policy (CSP) Changes

No matter which method you use to integrate GrowthBook, if your website uses a Content Security Policy (CSP), there are some additional changes you'll need to make to your website.

#### connect-src

If you have the `connect-src` directive defined in your website's CSP, you'll need to include the URL of your GrowthBook API Host in order for your website to load experiments from GrowthBook's servers.

```
Content-Security-Policy: connect-src <GROWTHBOOK_API_HOST>;
```

If you use GrowthBook Cloud, use our global CDN: https://cdn.growthbook.io

```
Content-Security-Policy: connect-src https://cdn.growthbook.io;
```
