...
While the experiment is still in a draft state, there are two ways to preview the variations. Until an experiment is moved out of the "draft" state and started, these are the only two ways you can preview the variations.
...
### Click the "Preview" link in the GrowthBook dashboard

On the Experiment's detail page, scroll down to the "Implementation" section. Click on each variation's "Preview" link as shown in the screenshot below.

![Preview link in Experiment detail page](/images/preview-link-for-visual-editor-changes.png)

### Add a querystring to the target URL
...
If you are unable to see the Preview as expected, follow the troubleshooting steps provided in the Knowledge Base article called [Why is the Preview feature not working for the Visual Editor?](https://docs.growthbook.io/kb/visual-editor#preview-links-not-working).
