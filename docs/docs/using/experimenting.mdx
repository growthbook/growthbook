...
GrowthBook supports two SRM detection methods: a standard **chi-squared test** (default) and a **sequential Bayes factor test** that provides anytime-valid detection under continuous monitoring. The sequential method processes daily traffic counts incrementally and is especially useful for long-running experiments where you check results frequently. You can read more in the [SRM Detection documentation](/statistics/srm).
