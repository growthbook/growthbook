...
For larger organizations, it can helpful to provide some org-specific messaging to your team around processes for setting up experiments and features. GrowthBook gives enterprise users the capability to inject markdown into key pages in the app so that you can do just that. We've also surfaced certain Handlebars variables on each page so that you can leverage the power of [Handlebars templates](https://handlebarsjs.com/guide/#simple-expressions) to customize your messaging depending on the context.
