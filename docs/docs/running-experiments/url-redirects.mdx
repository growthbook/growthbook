...
URL Redirect tests require integrating one of our [SDKs](/lib) into your application. Currently, URL Redirect tests are only supported in our [Script Tag](/lib/script-tag), [Javascript](/lib/js), and [ReactJS](/lib/js) SDKs. Support for other SDKs is coming soon. Verify URL Redirect tests are enabled for your SDK Connection by going to **SDK Connections**, clicking **Edit**, and checking that "Enable URL Redirect experiments" is toggled on.
