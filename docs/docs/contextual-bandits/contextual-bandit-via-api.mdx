...
Required fields are `name`, `trackingKey`, `datasource`, `contextualBanditQueryId`, `variations`, `goalMetrics`, and `contextualAttributes`. Other fields (`hashAttribute`, `fallbackAttribute`, `hashVersion`, `disableStickyBucketing`, `segment`, `queryFilter`, `activationMetric`, `decisionMetric`, `minUsersPerLeaf`, `maxLeaves`, etc.) are optional and fall back to defaults.
