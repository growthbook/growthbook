...
**Note**: Make sure to use the domain `api.growthbook.io`, not `cdn.growthbook.io`.
2. Configure the CDN to cache requests to `/api/features/*` (and deny all other routes except `/sub/*`)
...
**Important**: Make sure to block all other routes besides `/api/features/*` and `/sub/*`. Doing this will greatly increase security by limiting the potential attack surface.
