...
:::tip
Our official Helm chart to deploy GrowthBook on Kubernetes is distributed as a OCI artifact via [ghcr.io](ghcr.io/growthbook/charts/growthbook):
...
- **Minikube:**
  ...
  ```bash
  echo "127.0.0.1 growthbook.192.168.49.2.sslip.io" | sudo tee -a /etc/hosts
  echo "127.0.0.1 api.192.168.49.2.sslip.io" | sudo tee -a /etc/hosts
  ```
