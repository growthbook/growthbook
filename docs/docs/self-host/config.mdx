...
segments:
  visitors:
    ...
    owner: Sample User
    sql: |-
      SELECT userid as user_id, timestamp as date, country
      FROM pages
      WHERE country='US'
    userIdType: user_id
```
...
Segment support for the config.yml was added in December 2022. If you are using the config.yml file and have previously created segments stored in MongoDB, in order to access,
