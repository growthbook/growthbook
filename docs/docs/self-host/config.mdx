...
The other option is to create a `config.yml` file. In the Docker container, this file must be placed at `/usr/local/src/experimentation-analysis/config/config.yml`. Below is an example file:
