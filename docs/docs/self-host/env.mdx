...
- **SDK_PAYLOAD_REFRESH_STALE_TRACKING_ENABLED** - When "true", REST API writes mark affected SDK connections stale and a background job rebuilds them on the job server instead of refreshing inline. Default off. UI-triggered refreshes always run immediately. Useful for orgs with very frequent API updates to reduce load on request-serving instances. Requires at least one instance running background jobs (i.e. without `CRON_DISABLED`), and makes SDK payload propagation for API writes eventually consistent (typically a few seconds).
