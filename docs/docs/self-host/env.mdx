...
- **DISABLE_TELEMETRY** - We collect usage telemetry to help us improve GrowthBook. This includes a hashed organization ID, your user ID, IP address and derived geolocation, user agent, and app version. Set to "true" to disable.
...
- **SDK_PAYLOAD_REFRESH_DEBOUNCE_MS** - Debounce SDK payload cache refreshes per organization by coalescing rapid, back-to-back writes into a single rebuild. Default `0` (disabled — refreshes run immediately). When set to a positive number of milliseconds, this delay is end-to-end: SDK clients (including the GrowthBook Proxy's SSE stream) won't see a change until at least this long after the triggering write. Useful if an organization makes very frequent feature/experiment updates (e.g. via the API) and you want to reduce the resulting load on MongoDB.
- **SDK_PAYLOAD_REFRESH_MAX_WAIT_MS** - When `SDK_PAYLOAD_REFRESH_DEBOUNCE_MS` is set, force a refresh at least this often during continuous writes, instead of letting the debounce push the refresh back indefinitely. Default `5000`. Set to `0` to disable and debounce with no maximum wait. Has no effect when `SDK_PAYLOAD_REFRESH_DEBOUNCE_MS` is `0`.
