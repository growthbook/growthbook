...
If you want to use the AI features in GrowthBook, you will need to configure one of the supported AI providers: OpenAI or Ollama. These providers enable features such as generating experiment descriptions, checking hypothesis validity, and detecting similar experiments. See more on the [AI Integrations page](/integrations/ai).

### OpenAI

To use OpenAI, set the `OPENAI_API_KEY` environment variable to your OpenAI API key. This is required for the AI features to work.
...
To use Ollama, set the `OLLAMA_BASE_URL` environment variable to the base URL of your Ollama instance. This is required to enable Ollama as the AI provider.
