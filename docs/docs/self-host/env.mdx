...
:::info Unique ports
The back-end and the front-end **must run on unique ports**.

The front-end has its own set of `/api` routes so currently we do not support running both the API and the front-end on the same port.
:::
...
Ensure you are rebuilding the container after changing any environment variables, e.g. `docker-compose up --build`. See the [docker compose build docs](https://docs.docker.com/engine/reference/commandline/compose_build/) for more information.
