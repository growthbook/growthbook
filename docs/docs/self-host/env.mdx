...
- **SDK_PAYLOAD_REFRESH_STALE_SWEEP_SECONDS** - Track SDK payload staleness for writes made via the external REST API instead of always refreshing immediately: the affected SDK connections are marked stale, refreshed right away if the organization was otherwise quiet, and otherwise left for a periodic sweep (this many seconds apart) to pick up together with anything else pending. Default `0` (disabled — refreshes always run immediately). Refreshes triggered from the GrowthBook UI are never delayed, regardless of this setting. Useful if an organization makes very frequent feature/experiment updates via the API and you want to reduce the resulting load on MongoDB, without adding latency to isolated one-off writes.
