...
- **APP_FEATURE_DEFAULTS** - GrowthBook uses feature flags to control parts of its own app, fetched from GrowthBook's CDN. Set this to a JSON object of feature keys to values (e.g. `{"my-flag": true}`) to pin specific flags for your instance. Pinned flags win over fetched values, and if your deployment has no outbound network access the remaining flags safely evaluate to their fallback values (`false`/`null`).
- **DISABLE_APP_FEATURE_FETCH** - Set to "true" to prevent the back-end from fetching GrowthBook's own app feature flags from `cdn.growthbook.io` (e.g. for air-gapped networks). Flags then resolve to `APP_FEATURE_DEFAULTS` or their fallback values.
