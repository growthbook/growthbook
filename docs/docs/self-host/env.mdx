...
- **DISABLE_API_ROOT_PATH** - We include a default path in the API server to expose certain settings. Set to "true" to disable.
- **API_ROOT_HIDE_API_HOST** - The value for the API_HOST variable is included in the payload from the default path. Set to "true" to omit.
- **API_ROOT_HIDE_APP_ORIGIN** - The value for the APP_ORIGIN variable is included in the payload from the default path. Set to "true" to omit.
- **API_ROOT_HIDE_BUILD_INFO** - Details about the build in-use are included in the payload from the default path. Set to "true" to omit.
