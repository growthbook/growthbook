...
- **SDK_PAYLOAD_REFRESH_STALE_TRACKING_ENABLED** - Track SDK payload staleness for writes made via the external REST API instead of always refreshing immediately: the affected SDK connections are marked stale and a background job rebuilds them on your job server (see the "Jobs Server" scaling setup above) rather than inline on whichever instance received the request. Set to "true" to enable — default off (refreshes always run immediately, inline). Refreshes triggered from the GrowthBook UI are never delayed, regardless of this setting. Useful if an organization makes very frequent feature/experiment updates via the API and you want to reduce the resulting load on MongoDB and keep that work off your request-serving instances, without adding latency to isolated one-off writes.
