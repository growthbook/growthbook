...
- **APP_FEATURE_DEFAULTS** - GrowthBook uses feature flags to control parts of its own app. Self-hosted deployments don't fetch these flags from GrowthBook Cloud, so back-end flags evaluate to their fallback values (`false`/`null`). Set this to a JSON object of feature keys to values (e.g. `{"my-flag": true}`) to override specific flags for your instance.
