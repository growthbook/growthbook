...
- **Official Metrics** — metrics managed by GrowthBook or imported from a [managed source](/metrics/metrics).
...
Saving a query requires the **Save SQL Explorer Queries** commercial feature (Pro+). Saved queries can be:
...
| Save SQL Explorer queries       | Create SQL Explorer Queries (and Pro+ plan)        |
| Save explorations to dashboards | Create / Update General Dashboards (and Pro+ plan) |
