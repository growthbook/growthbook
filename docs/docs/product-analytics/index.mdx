...
Both surfaces read from the same [data sources](/warehouses), [fact tables, and metrics](/metrics/metrics) you already use for experimentation, so there's no separate event pipeline or instrumentation to set up.
...
2. Define [fact tables and metrics](/metrics/metrics) — these power the Metric and Fact Table explorers, and they're what dashboard blocks visualize.
