...
A leading `v` in a version string should be ignored and segments are separated by `.` and `-` characters. Segments should be compared alphanumerically with digit-only segments left-padded with zeros.
