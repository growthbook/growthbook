...
**Latest spec version: 0.5.3 [View Changelog](#changelog)**
...
7.  Apply filters and namespace
    ...
    2.  Else if `experiment.namespace` is set, return if not in range
        ...
        ```
    3.  Else if `experiment.include` is set, return if evaluates to false
8.  If `experiment.condition` is set, return if it evaluates to false
    `ts if (!evalCondition(context.attributes, experiment.condition)) { return getExperimentResult(experiment); } `
    8.1 Apply any url targeting based on experiment.urlPatterns, return if no match
9.  Choose a variation
    1.  If a sticky bucket value exists, use it.
        ...
        2. If the found sticky bucket variation is blocked (key doesn't exist in `experiment.meta`) and `experiment.excludeBlockedBucketUsers` is set, then skip enrollment
...
13. Build the result object
    `ts result = getExperimentResult(experiment, assigned, true, n); `
...
- **v0.5.3** 2024-01-02
  ...
  - New `fallbackAttribute`, `disableStickyBucketing`, `bucketVersion`, `minBucketVersion`, `excludeBlockedBucketUsers` properties of FeatureRules
  - New `fallbackAttribute`, `disableStickyBucketing`, `bucketVersion`, `minBucketVersion`, `excludeBlockedBucketUsers` properties of Experiments
