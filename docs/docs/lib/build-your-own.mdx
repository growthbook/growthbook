...
### unbiasedHash(seed: string, value: string): float

New hashing algorithm that fixes bias issues with the original `hash` function. Still uses the same FNV function under the hood.
...
1. Loop through filters array
   ...
   3. Determine the bucket for the user
      ```ts
      n = unbiasedHash(filter.seed, hashValue);
      ```
...
### private isIncludedInRollout(featureKey: string, hashAttribute: string | null, range: BucketRange | null, seed: string | null, coverage: float | null): boolean
...
2. Loop through the feature rules (if any)
   ...
   3. If `rule.force` is set
      1. If not `includedInRollout`, skip this rule and continue to the next one
         ```ts
...
- **v0.4.0** 2023-02-24
  - New `unbiasedHash`, `inRange`, `isIncludedInRollout`, and `isFilteredOut` helper methods
  - New `range`, `ranges`, `meta`, `filters`, `seed`, `name`, `tracks`, and `phase` properties of FeatureRules
  - New `ranges`, `meta`, `filters`, `seed`, `name`, and `phase` properties of Experiments
