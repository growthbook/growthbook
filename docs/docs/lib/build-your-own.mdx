...
**Latest spec version: 0.4.2 [View Changelog](#changelog)**
...
- Segments should be compared alphanumerically from the left-most segment
  - Digit-only segments should be left-padded (e.g. with a space or a `'0'`) so that they have the same number of characters, e.g. when comparing segment `2` with `10`, the comparison should be `'02' === '10'` so that 10 is recognized as greater than 2.
