...
If your GrowthBook plan supports it, you can enable your Go SDK for
streaming updates. This allows you to publish updates to feature
definitions via the GrowthBook web application and for them to be
immediately available in your Go applications using the Go SDK. (The
updates are sent using [server-sent
events](https://en.wikipedia.org/wiki/Server-sent_events)).

In the Go SDK, streaming updates are handled by an autonomous
goroutine that is managed by the SDK code. All you need to do is to
call the `LoadFeatures` method with a `FeatureRepoOptions` options
value with a `true` value for the `AutoRefresh` field. The feature
values retrieved from the Go SDK will then (up to communications
delays) be the latest values published in the GrowthBook web
application.


## Tracking Experiments and Feature Usage

The Go SDK supports mechanisms for tracking the results of experiments
and feature usage. The `Options` value supports two kinds of
callbacks:
...
your event tracking or analytics system of choice. This callback is
implemented using an `ExperimentTracker` interface with a single
`Track` method. The `Options` type has an `ExperimentTracker` field
that you can assign one of these interface values to, and the `Track`
method of the interface will called whenever an experiment is run.

There is a little bit of complexity here, because in most cases, you
will not want to record *every* run of an experiment, but only
*unique* experiments, based on hash values and variation IDs. This
requires some sort of a cache to ensure that there are no repeated
calls to the tracker for the same experiment. In server-side
applications, horizontal scaling means that it is often not acceptable
to use a simple in-process cache for this sort of thing: you need to
use Redis or some other distributed caching system. The choice of
caching system is very dependent on your individual application, so
the Go SDK does not impose any requirements on your caching layer.

We recommend that you write a caching layer to wrap your
`ExperimentTracker` or implement the caching directly within your
`ExperimentTracker` implementation. For development use, we provide a
simple `SingleProcessExperimentTrackingCache` cache implementation.

:::warning Warning
The `SingleProcessExperimentTrackingCache` implementation is
thread-safe, but is not appropriate for use in a multi-process
setting. If you use it in such a setting, your experiment tracking
statistics will not be correct, because your tracker will record the
same experiment in each server process. You need to use a distributed
cache in this case.
:::

The simplest possible usage of experiment tracking then looks
something like this, using a simple callback (the `ExperimentCallback`
type is used to wrap a simple callback to give an `ExperimentTracker`
value) and a simple single-process caching implementation (to be used
only in development!):

```go
func TrackCallback(ctx context.Context,
  experiment *growthbook.Experiment, result *growthbook.ExperimentResult) {
...
...
  ...
  tracker := growthbook.NewSingleProcessExperimentTrackingCache(
    &growthbook.ExperimentCallback{TrackCallback},
  }
  
  client := growthbook.NewClient(&growthbook.Options{
    ...
    ExperimentTracker: tracker,
    ...
  })
...
A similar mechanism is provided for tracking feature usage. This
tracker is called any time a feature result is generated which can be
used for collecting statistics about feature values, rules and so on.

The Go SDK mechanisms to support this parallel the features for
experiment tracking:

- `FeatureUsageTracker`: an interface with a single `OnFeatureUsage` method;
- `FeatureUsageCallback`: a wrapper for a simple callback to implement
  the `FeatureUsageTracker` interface;
- `SingleProcessFeatureUsageTrackingCache`: a simple single-process
  feature usage cache for development use.

:::warning Warning
The considerations about caching in multi-process server
implementations apply for feature tracking, just as they do for
experiment tracking!
:::
...
`Client.GetFeatureValue`, which takes a default value, and just
returns the feature value. For example, this code evaluates the result
of a feature and returns the feature value, defaulting to "blue" if
...
`FeatureValue`, `FeatureRule`), but the most common use case (apart
from loading features directly from GrowthBook) is likely to be
ingesting feature definitions from JSON data using Go's standard
...
JSON definitions using `json.Unmarshal`. Passing an `Experiment` value
to the `Run` method of the `Client` type will run the experiment,
returing an `ExperimentResult` value that contains the resulting
feature value. This allows users to run arbitrary experiments without
providing feature definitions up-front.
...
A common use case is to download the feature definition from the
GrowthBook SDK endpoints, and parse them into a feature map that can
be passed into the GrowthBook `Options`.

The shape of the GrowthBook response is defined in the Go SDK using
the `FeatureAPIResponse` type:
...
We can retrieve JSON data from GrowthBook and deserialize it into
a `FeatureAPIResponse` value:
...
var featuresResponse growthbook.FeatureAPIResponse
...
// Create a growthbook.GrowthBook instance using the decoded features.
gb := growthbook.NewClient(nil).WithFeatures(featuresResponse.Features).

// Evaluate a feature.
result, err := gb.EvalFeature("feature", userAttributes)
```

All of the types that might need to be deserialized from JSON suport
Go's standard `json.UnmarshalJSON` interface.

:::tip Recommendation
In the most common use case, where you use the `Client.LoadFeatures`
method to load features from GrowthBook, all of the deserialization
and installation of features is handled for you by the Go SDK. We
recommend this approach for most users.
:::

## Code Examples

- [Server example using `LoadFeatures` <ExternalLink/>](https://github.com/growthbook/examples/tree/main/go-example)
- [Server example using Echo framework
  <ExternalLink/>](https://github.com/growthbook/examples/tree/main/go-echo-example)
...
which can be created from JSON data by deserializing using Go's susing the `ParseFeatureMap`
