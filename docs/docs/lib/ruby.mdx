...
2. When creating the context for the first time, features are fetched and cached in the Rails cached. Subsequent calls use the cached version until the cache expires.
