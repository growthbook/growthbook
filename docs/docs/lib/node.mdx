...
For RSCs (React server components) or SSR (server-side rendering), pass in the readonly request cookies and search params to the plugin. In Next 15+, these may need to be awaited.
