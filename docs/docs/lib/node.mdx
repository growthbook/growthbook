...
Note the component named `<DevtoolsLogger>`. We will discuss how and why to implement this below.

For middleware implementations or API routes, pass the `request` object to `devtoolsNextjsPlugin` instead of searchParams and requestCookies.
...
In an Express.js context, in order to apply SDK overrides from DevTools, pass the request object into the `devtoolsExpressPlugin` plugin.
...
app.use((req, res, next) => {
  ...
  res.set("x-gbdebugEvents", getDebugEvent(req.growthbook, "express"));
