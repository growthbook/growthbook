...
In order for a user to enter a feature flag experiment, you need to evaluate the feature in your application (for example with `getFeatureValue` or `isOn`). That is also when `trackingCallback` runs so you can record the exposure in your analytics.
