...
Starting in version 1.3.0, we offer a new `GrowthBookMultiUser` class that can be re-used across requests. In our testing, this is up to 3x faster than creating a new GrowthBook instance for every request, but is a little more complex to set up.

First, create and instantiate your singleton:
...
Then, you can use this in any request by supplying a `UserContext` object to any of the feature evaluation methods.
...
app.get("/", (req, res) => {
  ...
  if (gb.isOn("my-feature", userContext)) {
    // Do something
  }
})
```
...
You can specify `trackingCallback` or `onFeatureUsage` either globally in your GrowthBookMultiUser class or per-request in your user context.
...
### Multi-User Sticky Bucketing

To use sticky bucketing with a multi-user instance, you must apply sticky buckets to your user context. This is an async operation since for some services (e.g. Redis), it may require a db lookup. Because of this, we recommend only doing this once per request and re-using the user context if you need to evaluate multiple feature flags.
...
app.get("/", (req, res) => {
  ...
  const userContext = await gb.applyStickyBuckets({
    attributes: {
      id: req.user.id
    }
  }, stickyBucketService);
