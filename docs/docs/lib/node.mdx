...
await client.init({timeout: 3000});
```
...
if (client.isOn("my-feature", userContext)) {
  console.log("My feature is on!");
}

// String, Number, or JSON flags
const value = client.getFeatureValue("my-string-feature", "fallback", userContext);
console.log(value);
```

### Express Middleware

Instead of constantly defining and passing `userContext` around your app, we recommend creating a "scoped" instance of GrowthBook in a middleware instead.
...
The `GrowthBookMultiUser` class was introduced in SDK version 1.3.0 to improve performance in back-end environments by up to 3x. It does this by re-using the core instance across many different user requests.
...
The `GrowthBook` class is identical to the one used in client-side environments. Check out the [/lib/js](Client-side JavaScript SDK docs) for more info on all of the settings and methods available.
...
If you pass an `apiHost` and `clientKey` into the GrowthBookMultiUser constructor, it will handle the network requests, caching, retry logic, etc. for you automatically.
...
```ts
const client = new GrowthBookMultiUser().initSync({
  payload: {
    features: {...}
  }
});
...
await client.init({ streaming: true });
```
...
You can specify this globally in your GrowthBookMultiUser class and/or per-request in your user context.
...
const newLogin = req.growthbook.isOn("new-signup-form");
```
...
app.use((req, res, next) => {
  ...
  const userContext = await client.applyStickyBuckets({
    attributes: {
      id: req.user.id
    }
  }, stickyBucketService);
...
const color = req.growthbook.getFeatureValue("button-color", "blue");

// You can manually specify types as well
// feature.value will be type `number`
const feature = req.growthbook.evalFeature<number>("font-size");
...
const client = new GrowthBookMultiUser<AppFeatures>({
  ...
});
```
...
```ts
// feature.value will be type `number`
const feature = client.evalFeature("font-size", userContext);
console.log(feature.value);

// Typos will cause compile-time errors
client.isOn("buton-color", userContext); // "buton" instead of "button"
```
...
Sometimes there are global attributes that apply to all users. For example, the ip of your server. These can be specified on the GrowthBookMultiUser instance and will be merged with any attributes in the user context.
...
Like with the `trackingCallback`, this can be defined on either the GrowthBookMultiUser instance or as part of the user context. When defined on the GrowthBookMultiUser instance, a 3rd argument with the user context will be sent so you can identify which user evaluated a feature.
...
const result = client.evalFeature("my-feature", userContext);
...
Instead of declaring all features up-front in the context and referencing them by ids in your code, you can also just run an experiment directly. This is done with the `runInlineExperiment` method:

```js
// These are the only required options
const { value } = client.runInlineExperiment({
  key: "my-experiment",
...
const holdout = client.runInlineExperiment({
  key: "holdout",
...
if (!holdout.value) {
  const res = client.runInlineExperiment({
    key: "my-experiment",
    variations: ["A", "B"]
  }, userContext)
}
