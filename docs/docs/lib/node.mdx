...
`setPolyfills({ EventSource: require("eventsource") })` (without destructuring) works only on `eventsource` v2 and below. On v3+ it passes the module object rather than the constructor, which prevents streaming from starting. If streaming is enabled but features never update, check this first — the SDK logs a warning when it detects that streaming could not be started.
