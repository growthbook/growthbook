...
You can use TypeScript to varying degrees of strictness based on your preference:

- Basic support with type inference
- More strict support with strongly-typed feature definitions
...
### Strongly-typed feature definitions

As of version `0.22.0` you can use strongly-typed feature definitions, which will allow you to pass a typed feature definition as a generic argument when creating the `GrowthBook` instance.

When providing typed feature definitions, the following methods are strictly typed:
...
```ts
// This can be generated with the CLI
type MyAppFeatures = {
  dark_mode: boolean;
...
const features = {
  // Features from the SDK endpoint, as described above
};
...
const growthbook = new GrowthBook<MyAppFeatures>(context);

const featureResult = growthbook.evalFeature("greeting");
const shouldUseDarkMode = growthbook.getFeatureValue("dark_mode", false);
```

When using strongly typed feature definitions when creating your `GrowthBook` instance, you will get the following benefits:

- You will only be allowed to pass in known keys to the typed methods
- `getFeatureValue(key, defaultValue)` will find the reference for the provided key in the `AppFeatures` passed into the generic parameter when constructing your `GrowthBook` instance. This means if you try to pass in a string value into `defaultValue` when the known type should be a boolean, you will get a type error.

![Screenshot of TypeScript support in an IDE that shows hints offering suggested keys for valid types for the method getFeatureValue](../../static/images/lib-js-typescript-01.png)
_Screenshot of TypeScript support in an IDE that shows hints offering suggested keys for valid types for the method `getFeatureValue(key, defaultValue)`._

![Screenshot of TypeScript support in an IDE that shows an error when trying to use an unknown key in the method evalFeature](../../static/images/lib-js-typescript-02.png)
_Screenshot of TypeScript support in an IDE that shows an error when trying to use an unknown key in the method `evalFeature(key)`._

#### Generating strictly-typed feature definitions with the CLI

To generate strictly-typed feature definitions for your project with the CLI, please see the [documentation for the Command Line Interface](/tools/cli).

You can see an example project that implements the GrowthBook CLI to generate type definition and uses those generated type definitions [here <ExternalLink />](https://github.com/growthbook/examples/tree/main/vanilla-typescript).
