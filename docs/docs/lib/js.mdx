...
const userEmail = sha256( user.email + salt );

// hashing an secureString[] attribute
const userTags = user.tags.map( tag => sha256( tag + salt ) );
...
function sha256(str) {
  ...
  // In a modern browser context, `crypto.subtle` is available (although calls are asynchronous)
  // Alternatively, a reasonable synchronous library is https://www.npmjs.com/package/crypto-js
