...
In order to run A/B tests, you need to set up a tracking callback function. This is called every time a user is put into an experiment and is needed to track the exposure event in your analytics system (Segment, Mixpanel, GA, etc.). For more details refer to [Date Source Configuration](/app/datasources). 
