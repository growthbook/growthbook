...
The JS SDK contains a built-in method `setEncryptedFeatures` that automatically decrypts the encryptedFeatures before calling `setFeatures`.

```ts
// Load features from the encrypted SDK Endpoint and decrypt them
fetch("https://cdn.growthbook.io/api/features/<your api key>")
  .then((res) => res.json())
  .then(async (json) => await growthbook.setEncryptedFeatures(json.encryptedFeatures, <your encryption key>));
  .catch((e) => console.log("Failed to set GrowthBook feature definitions", e));
```

**Note: If you're not in a browser environment (E.G Node, React Native), you will need to pass in a SubtleCrpyto implementation as the 3rd argument when calling `setEncryptedFeatures`. **

```ts
const { subtle } = require('node:crypto').webcrypto;

growthbook.setEncryptedFeatures(encryptedString, encryptionKey, subtle);
  // Load features from the encrypted SDK Endpoint and decrypt them
  fetch("https://cdn.growthbook.io/api/features/<your api key>")
    .then((res) => res.json())
    .then(async (json) => await growthbook.setEncryptedFeatures(json.encryptedFeatures, <your encryption key>, subtle));
    .catch((e) => console.log("Failed to set GrowthBook feature definitions", e));
```
