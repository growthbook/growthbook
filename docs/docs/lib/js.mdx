...
app.use(function(req, res, next) {
  ...
  req.growthbook.init({timeout: 1000})
    .catch((e) => console.error("Failed to load features from GrowthBook", e))
    .finally(() => next());
});
```
...
You can update the payload at any time by calling `setPayload(newPayloadJSON)` and there is also a `getPayload()` method, which is useful in hybrid apps where you want to hydrate the client with data from the server.

Note: you don't need to specify `clientKey` or `apiHost` on your GrowthBook instance unless you want to enable streaming (see below) or call `refreshFeatures()`.
...
In addition to the above, we highly recommend switching from `loadFeatures()` to the new `init()` method. The `init()` method solves many pain points including easier error handling and more control over caching and streaming. See the Loading Features section above for more info.
