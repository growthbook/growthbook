...
Use Vue's `provide` function to make GrowthBook available to your components.
...
// Define the GrowthBook instance
let gbInstance

try {
  // Wrap the GrowthBook instance in a reactive object and use streaming for real-time reactivity
  gbInstance = reactive(
    new GrowthBook({
  ...
  console.log('Error initializing GrowthBook:', e)
}

// Share the provider type with other components
export const gbKey = Symbol('gbKey') as InjectionKey<typeof gbInstance>
const app = createApp(App)

// Define the provider and pass in the GrowthBook instance
...
- [Composition API example <ExternalLink />](https://github.com/growthbook/examples/tree/main/javascript-vue-composition)
- [Options API example <ExternalLink />](https://github.com/growthbook/examples/tree/main/javascript-vue-options)
