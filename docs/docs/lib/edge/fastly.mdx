...
# Fastly Compute Edge App & SDK <Pill>beta</Pill>
...
You can enable URL Redirects on edge by setting environment variable `RUN_URL_REDIRECT_EXPERIMENTS` to `"edge"` or `"everywhere"`. Additionally if your redirect is cross-domain (e.g. redirection from "https://public.mysite.io" to "https://newsite.io/"), you must also set `RUN_CROSS_ORIGIN_URL_REDIRECT_EXPERIMENTS` to `"edge"` or `"everywhere"`. See ([environment variables](#environment-variables)) for more information.
