...
Convenient solutions this problem are realized through [CloudFlare KV<ExternalLink />](https://developers.cloudflare.com/kv/reference/how-kv-works/), an on-edge key-value store which we can leverage for persistent payload caching. There are 2 levels of KV integration available:
...
2. Alternately, you can eliminate most of these network requests by using CloudFlare KV as a just-in-time payload cache.
