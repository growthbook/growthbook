...
You may wish to either customize front-end tracking or switch to edge tracking (or use both concurrently if running hybrid edge + front-end experiments). In general, tracking on an edge or backend environment helps eliminate measurement delta and experimental bias. However, using the default injected front-end tracking reduces biases and deltas when compared with solely using a front-end SDK.
