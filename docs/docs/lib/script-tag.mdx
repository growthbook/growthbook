...
This option is quick and straightforward to use. Visual Editor experiments do not require coding knowledge — they run when the page loads. Feature Flag experiments still require you to evaluate the flag in code (for example `isOn`). Adding the script does not enroll users in Feature Flag experiments by itself.
...
- `data-tracking` - Set to `growthbook` to send experiment view events that already fired to GrowthBook's warehouse. This does not evaluate flags or enroll users.
...
When a user is included in an experiment, a tracking event is fired. Loading the script does not fire this by itself. There is built-in support for Segment.io (`analytics.js`), GA4 (`gtag`), and Google Tag Manager (`dataLayer`).
...
You can use Feature Flags with this SDK, but it does require some manual coding work. The script on the page is not enough. A user enters a Feature Flag experiment only when you evaluate the flag, as in the example below:
...
By default the `gbuuid` cookie is **host-only**, so it is not readable on other subdomains. If your site spans multiple subdomains (for example `www.example.com` and `app.example.com`) — or if you run **redirect experiments that send users to another subdomain** — the visitor will get a brand new `gbuuid` on the new subdomain and be re-bucketed. This creates duplicate anonymous ids for the same person, which shows up as inflated user counts and a Sample Ratio Mismatch (SRM) in your analytics/warehouse.

To keep one stable id across all subdomains, scope the cookie to your parent domain with `data-uuid-cookie-domain` (or `window.growthbook_config.uuidCookieDomain`):

```html
<script async
  data-client-key="YOUR_CLIENT_KEY"
  data-uuid-cookie-domain=".example.com"
  src="https://cdn.jsdelivr.net/npm/@growthbook/growthbook/dist/bundles/auto.min.js"
></script>
```

When set, this also scopes the sticky bucketing cookies (`data-use-sticky-bucket-service="cookie"`) to the same domain, so sticky assignments follow the shared id across subdomains.

Note that cookies cannot be shared across completely different registrable domains (e.g. `example.com` and `example.co.uk`). For that case, pass the id through the redirect URL and initialize the destination with `data-uuid` / `window.growthbook_config.uuid`.
