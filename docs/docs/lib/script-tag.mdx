...
If you need low level access to the GrowthBook SDK instance for any reason, you can pass a callback function to a `window.growthbook_queue` array. This will be called as soon as the GrowthBook SDK instance is ready and works before or after you add the script tag.
...
window.growthbook_queue.push((gb) => {
  ...
  gb.setRenderer(applyFeatureFlags);
});
</script>
```
