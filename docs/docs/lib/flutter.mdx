...
```
growthbook_sdk_flutter: <latest_version>
```
...
attrs.putIfAbsent("id", () => 123);
attrs.putIfAbsent("env", () => "dev");
attrs.putIfAbsent("betaUser", () => true);

final gb = await GBSDKBuilderApp(
  apiKey: "<API_KEY>",
  attributes: attrs,
  growthBookTrackingCallBack: (gbExperiment, gbExperimentResult) {},
  hostURL: '<GrowthBook_URL>',  // The hostURL should be end with `/` ex. `https://cdn.growthbook.io/`. 
).initialize();
...
exp.variations = <String>["control", "variation"]
final result = gb.run(exp)
