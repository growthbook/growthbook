...
The React SDK contains a built-in method `setEncryptedFeatures` that automatically decrypts the encryptedFeatures before calling `setFeatures`.
...
**Note: If you're not in a browser environment (E.G React Native), you will need to pass in a SubtleCrpyto implementation as the 3rd argument when calling `setEncryptedFeatures`. **
