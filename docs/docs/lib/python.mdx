        ...
        Options(
            ...
            tracking_plugins=[
                GrowthBookTrackingPlugin(
                    # Use the region you selected when setting up your Managed Warehouse
                    # (defaults to https://us-east-1.gb-ingest.com; https://eu-west-1.gb-ingest.com for EU)
                    ingestor_host="https://us-east-1.gb-ingest.com",
...
gb = GrowthBook(
    ...
    plugins=[
        GrowthBookTrackingPlugin(
            ...
            # Additional callback for custom analytics
            additional_callback=my_custom_tracker,
            # Number of events to accumulate before flushing (optional, default 10)
            batch_size=5,
...
def mixpanel_tracker(experiment, result, user_context):
    """Send events to Mixpanel (Fire-and-Forget)"""
    loop = asyncio.get_running_loop()
    loop.create_task(mixpanel.track_async(
        user_context.attributes.get('id'), "Experiment Viewed", {
            'experiment_id': experiment.key,
...
client = GrowthBookClient(
    Options(
        ...
        tracking_plugins=[
            GrowthBookTrackingPlugin(additional_callback=my_custom_tracker)
        ]
    )
)
...
For a complete Django integration guide covering middleware, DRF permissions, async Channels, and Celery tasks, see the [Django SDK docs](/lib/django).
