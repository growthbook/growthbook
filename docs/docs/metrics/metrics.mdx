...
These metric slices are defined in your [Fact Table](#fact-tables) and can be selectively enabled per each Fact Metric. When a auto slice is added to a Fact Metric, it is automatically added to all experiments which use the metric. This allows for a standardized deep-dive analyses across your experiments.
