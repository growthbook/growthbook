...
:::note
Metric dimensions are only available for Enterprise customers.
:::
...
4. Click "Enable Dimensions" to turn on metric dimensions for this metric.

When you run an experiment, you will now have the option to split revenue by product type.
If you click the arrow next to "revenue" on the results page, you will see results split by product type. You can also pin a dimension result so that it stays in the results table even when the metric is collapsed or you return to the page.
