...
Rather than creating and maintaining a separate revenue metric for each product type, metric slices permit you to automatically create distinct revenue metrics for each product type. These drilldown metrics are also sometimes referred to as "metric dimenensions" in the experimentation community.

Metric slices are enabled for Fact Metrics. We offer two key ways to slice your metrics: Auto Slices and Custom Slices.

### Auto Slices

These metric slices are defined in your [Fact Table](#fact-tables) and can be selectively enabled per each Fact Metric. When using auto slices in your Fact Metric, each slice analysis (a dimension + value pair) will materialize across **all** experiments which use the metric. This allows for a standardized deep-dive analyses across your experiments.
...
Custom slices can be created using any string column in your Fact Table which is not an identifier; custom slicing is not limited to auto-slices columns.

<MaxWidthImage maxWidth={700}>
  <img src="/images/metrics/custom-slices.png" alt="Adding custom slices" />
</MaxWidthImage>

### Viewing and pinning metric slices
...
If you click the arrow icon next to "revenue" on the experiment results table, you will see these slices expand under the base metric row (revenue), each slice materialized as its own sub-row.

You may "pin" these metric slice rows so that they always render on the results table, even when the metric is collapsed or you return to the page.
...
:::note Note: Dimensions versus Slices
