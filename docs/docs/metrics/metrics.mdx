...
sidebar_label: Metrics and Fact Tables
slug: /app/metrics
---
...
The primary way to create metrics in GrowthBook is with **Fact Tables** where you write SQL to pull a set of rows and then define multiple metrics on top of those rows. There is also a [legacy method](/metrics/legacy) where each metric has its own self-contained SQL. Fact Tables are easier to use, faster to query, and more powerful, so we highly recommend using them for all new metrics.

If you learn best by example, you can jump straight into [Metric Examples and Use Cases](/metrics/examples).
...
Fact Tables work great for raw event streams, modeled tables, and pre-aggregated tables. Check out [Metric Examples and Use Cases](/metrics/examples) for more info on supporting these different data schemas.
...
It's common to have an important "type" column that you use to build out metrics. For example, if your Fact Table is a raw event stream, there will be a column like `event_type` to differentiate between a page view, button click, purchase, etc..
...
When you check this box, GrowthBook will run a quick query looking for the top 100 values for this column in the past week. This will be used to power an auto-complete field in the UI when creating a metric.
...
You can add **Row Filters** to limit which rows are considered. For example, adding a `Mobile` filter, you can see what percent of users completed a purchase on a mobile device.

You can also add a **User Filter** to decide which users to include AFTER aggregating all of their rows together. For example, you can filter on `Count of Rows >= 3` to only include users who have at least 3 matching rows.
