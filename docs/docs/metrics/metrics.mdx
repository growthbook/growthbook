...
:::note
Metric slices are available for Enterprise customers only.
:::
...
Metric slices are enabled for Fact Metrics. Here are step-by-step instructions:

1. Navigate to your Fact Table that has the right columns. Ensure the Fact Table SQL is returning the relevant column. If it isn't you can click "Edit SQL" on the Fact Table page, add your column (e.g. `product_type`) to the SELECT statement, and then press "Confirm Changes". Only string columns can be used.

<MaxWidthImage maxWidth={700}>
  <img src="/images/metrics/metric-dim-config.png" alt="Configuring a column for metric slicing" />
</MaxWidthImage>

2. With your column (`product_type`) in the Fact Table SQL, you now edit the column property in the "Columns" table. Click the edit icon next to your column, check the "Enable Auto Slices" checkbox in the window that pops up, and add the slices you want to split your metric by. We run a query to populate the top values for you, and you can update the values found in the data by clicking the "Refresh" button.
   Levels here could be "apparel", "equipment", and more.
...
You can also pin an auto-sliced result so that it stays in the results table even when the metric is collapsed or you return to the page.
...
All users contribute to a metric slice.  
This provides the most accurate inference, as trying to filter down the denominator can cause bias issues if you allow users to switch between different dimension levels.
To get revenue for French users on French products, you need to apply a unit filter for country and then look at the metric slice breakdown for French products.
