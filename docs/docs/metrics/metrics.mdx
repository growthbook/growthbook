...
There are 5 types of metrics you can create within GrowthBook - Proportion, Retention, Mean, Ratio, and Quantile.
...
- `COUNT(DISTINCT DATE(timestamp))` - the distinct number of days user completed the qualifying actions in your fact table (e.g. for a count of active days per user)
...
Rather than creating and maintaining a separate revenue metric for each product type, metric slices permit you to automatically create distinct revenue metrics for each product type. These drilldown metrics are also sometimes referred to as "metric dimensions" in the experimentation community.
