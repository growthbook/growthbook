...
Capping (also known as winsorization) limits how extreme aggregate unit (e.g. user) values can be. **Upper-tail** capping ensures values are not above a ceiling; **lower-tail** capping ensures values are not below a floor. You can use either or both on **Fact Metrics** analyzed with SQL data sources. (Legacy Metrics and Mixpanel support upper-tail capping only.) So in the revenue example, if the upper cap was \$100, the \$5000 purchase will still be counted, but the aggregated value for that user will be capped at \$100 and will have a much smaller effect on the results. Another way to think about this is that you are slightly biasing your results by truncating extreme values, but you are reducing variance to prevent the outsized effect of outliers.

There are two mechanisms for each tail in GrowthBook:

**1. Absolute capping** - if set above zero on the upper tail, all aggregated user values are capped at that maximum. On the lower tail, values below the floor are raised to that minimum.

**2. Percentile capping** - when set to between 0 and 1, GrowthBook uses that quantile of the unit-level aggregates in the current analysis to set the bound. Upper tail: values above that quantile are reduced to it (same behavior as before). Lower tail: values below that quantile are raised to it. If both tails use percentile capping, the lower percentile must be less than the upper percentile (e.g. 5th percentile floor and 95th percentile ceiling).

You can choose to ignore zeros when computing percentile bounds (separately for upper and lower). That excludes user-level zeros from the percentile calculation, which helps when many users are zero and you do not want the cap or floor driven by the mass at zero.

Because percentile bounds depend on the data in your experiment, they can differ from run to run as new data arrives. To see the values used: on the Experiment Results tab, open the three-dot menu and choose "View Queries". Percentile upper bounds appear in `main_cap_value` (and `denominator_cap_value` for ratio metrics); percentile lower bounds appear in `main_cap_value_lower` and `denominator_cap_value_lower` when lower-tail percentile capping is enabled.
