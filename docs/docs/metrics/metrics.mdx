...
For example, you may want to see revenue across different device types.
Rather than creating and maintaining a separate revenue metric for each device type, metric dimensions permits you to automatically create distinct revenue metrics for each device type.

Create metric dimensions as follows:

1. ensure your Fact Table has a column that corresponds to the dimension by clicking the "Edit SQL" button on the Fact Table page, add "device_type" to the select statement, and then press "Confirm Changes". Metric dimensions are available only for Fact Metrics.
2. after adding "device_type" to the select statement, click the "Edit Column" icon next to "device_type" in the "Columns" table. Check the "is dimension" checkbox and add the dimension values you want to split revenue by.
   Dimension values here could be "mobile", "desktop", and "tablet".
3. find the Fact Metric "revenue" and click on it.
4. On the "Metric Dimensions" card click "Enable Dimension Analysis".

When you run an experiment, you will now have the option to split revenue by device type.
If you click the "plus" icon under "revenue" on the results page, you will see results split by device type.

User dimensions and experiment dimensions are attributes specific to a user.
...
In a dimension analysis, all of Romain's spend will be attributed to France.
If you create metric dimensions, only the French products purchased will be attributed to French revenue, and only the German products purchased will be attributed to German revenue.
