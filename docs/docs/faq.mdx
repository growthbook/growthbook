...
There are cases where 3rd party identifiers will not load in time, or may cause a flash. In these cases, generating your own identifier can load features much more quickly. This id will often align one to one with the other identifiers, so does not need to be passed outside the SDK. Please be aware of any cookie policies. Note, this technique is already included with our no-code SDK.
