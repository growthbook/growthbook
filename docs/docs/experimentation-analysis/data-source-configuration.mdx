...
### Data Pipeline Settings (beta)

:::note

Pipeline mode is only available for Enterprise customers, only works with for BigQuery and Snowflake datasources, and is currently in limited release while in (beta). Please contact us to join the beta.

:::

For experimenters who have multiple metrics per experiment and have large experiment assignment sources, GrowthBook can greatly improve the performance of your queries if you enable **Pipeline Mode**.
...
To enable Pipeline Mode, follow these steps:

1. (strongly recommended, but optional) Create a dedicated dataset (BigQuery) or schema (Snowflake) to which GrowthBook will write temporary tables. This will keep your data warehouse clean and ensure that we are only writing to a dedicated space.
2. Grant permissions to create tables to the role connecting GrowthBook to your warehouse. Feel free to limit that role to have create tables permission only in the dataset/schema that you created in step 1, but the role will also need to select data from that dataset/schema as well.
3. Navigate to your Data Source in GrowthBook and scroll down to "Data Pipeline Settings"
4. Click "Edit" and enable pipeline mode, set the destination dataset/schema to your new dedicated GrowthBook dataset/schema from step 1, and set the number of hours you will retain our temporary tables. We recommend at least 6 hours and the default is 24. For Snowflake we recommend leaving the value at 24.
