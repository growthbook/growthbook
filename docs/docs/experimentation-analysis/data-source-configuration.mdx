...
Default data sources can have one of their assignment queries be set as default. If set, then new experiments that are using this datasource will use the specified assignment query by default. These settings can later be changed in the experiment settings.
