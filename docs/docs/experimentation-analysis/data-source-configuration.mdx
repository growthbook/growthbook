...
##### Identifier Join Tables - Multiple identifiers

For cases when you'd want to use multiple identifiers, it's recommended you use the `CONCAT` function to synthesize a single new identifier for joining.

```
SELECT CONCAT(company_id, user_id) as company_user
```

Then use `company_user` as an identifier throughout the app.
