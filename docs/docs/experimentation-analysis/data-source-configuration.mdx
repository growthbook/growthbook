...
Pipeline mode is only available for Enterprise customers, is currently in beta, only works with for BigQuery datasources.
...
1. (strongly recommended, but optional) Create a dedicated dataset or schema to which GrowthBook will write temporary tables. This will keep your data warehouse clean and ensure that we are only writing to a dedicated space.
2. Grant write permissions on that dedicated dataset to your GrowthBook service account in BigQuery
