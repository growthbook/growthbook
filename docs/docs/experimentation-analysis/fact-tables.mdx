...
When using GrowthBook with certain event trackers, we may be able to generate Fact Tables for you automatically by identifying the unique events tracked by your event tracker. This is currently only supported for a few event trackers (listed below), but we are working to expand this list.

If you are using one of the supported event trackers and would like to see what Fact Tables we can create for you, head to the `Fact Tables` page in GrowthBook and select the `Auto Generate Fact Tables...` button.
