...
With Fact Tables, you write SQL once for an event (for example, an `orders` table) and from that you can quickly and easily create a bunch of related metrics - for example, `Revenue per User`, `Average Order Value`, `Items per Order`.

These metrics can be used as Goals and Guardrails in Experiments.

## Fact Table SQL

First, you have to write SQL that tells GrowthBook how to query your Fact Table. GrowthBook expects you to have a few pre-defined columns with specific names, but you can select as many additional columns as you need to build out metrics.

Here's an example:

```sql
SELECT
  -- Need a column named `timestamp`, when the event happened
  purchase_date as timestamp,
  -- Need columns for each of the supported identifier types
  -- This depends on your data source, but the below 2 are the most common
  user_id,
  anonymous_id,
  -- Anything else you select here can be used for metrics and filters
  amount,
  coupon_code,
  device_type
FROM
...
When you create a Fact Table, we run a simple `LIMIT 1` query and inspect the returned data to determine which columns you've selected and what data types they are.
...
Only numeric columns can be used to create Metrics, so you might need to CAST your data if they are a different shape. You can use any column, including non-numeric ones, to create Filters.

#### Number Formats

For numeric columns, you can also specify a number format, which controls how the metric is displayed on the front-end. Possible values are `currency` (uses the default currency specified in your organization settings) and `time:seconds`.

## Filters

Filters are reusable SQL snippets to filter the rows in the Fact Table. These let you create easily create metric variants. For example, `Purchase`, `Mobile Purchase`, and `Desktop Purchase`
...
If a metric uses this Filter, GrowthBook will add it to the WHERE clause automatically.

## Metrics

There are 3 types of supported metrics today - **Proportion**, **Mean**, and **Ratio**.
...
For example, if you have an Orders Fact Table, you can create a Proportion Metric to see what percent of users purchased something.

You can optionally add Filters to limit which rows are considered. For example, adding an `Is Mobile` filter, you can see what percent of users completed a purchase on a mobile device.
...
For the metric value, there are 2 types of aggregations you can do. These are calculated for each user in the experiment, then averaged to get the value per variation.

- `COUNT(*)` - Number of rows in the Fact Table
- `SUM(column)` - Total of a numeric column in the Fact Table

The denominator for Mean Metrics is always the number of users in an experiment. For example, if your experiment variation has 100 users and there are 500 matching rows in the Fact Table, a Mean Metric with `COUNT(*)` would have a value of `5.0`.
...
- `COUNT(*)` - Number of rows in the Fact Table
- `COUNT(DISTINCT 'User Identifier')` - Number of experiment users
- `SUM(column)` - Total of a numeric column in the Fact Table
...
By default, Fact Metrics are not restricted by a conversion window. Once a user views an experiment, any subsequent row in the Fact Table for that user is included up until the experiment is stopped.

You can enable conversion windows to restrict how far we look in the future for matching Fact Table rows. For example, if you specify a conversion window of **3 days** and a user is first exposed to the experiment on Jan 1 at 9:00, only rows between Jan 1, 9:00 and Jan 4, 9:00 will be included in the analysis.
