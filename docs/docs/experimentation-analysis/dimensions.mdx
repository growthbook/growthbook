...
<img
  src="/images/configure-dimensions.png"
  alt="Configure Dimensions Option"
...
SELECT
  ...
  experiment_id,
  variation_id,
  CASE WHEN browser LIKE '%Chrome%' THEN 'Chrome'
...
You can use SQL template variables in dimension queries just like you can with metrics (excluding metric specific template variables like `eventName` and `valueColumn`). See the [SQL Template Variables](/app/metrics#sql-templates) page for more details.
