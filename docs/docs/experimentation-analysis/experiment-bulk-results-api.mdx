...
Bearer auth with a Personal Access Token or secret key that can read the experiment's Project.

```bash
curl -G 'https://api.growthbook.io/api/v1/experiments/exp_abc123/bulk-results' \
