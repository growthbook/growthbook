...
- Event-level quantile metrics are supported for BigQuery, which stores event-level histograms for each user. This and provides approximate inference, as small effects may be undectable.
