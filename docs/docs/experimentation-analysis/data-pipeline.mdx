...
Both strategies do not access any additional data beyond what a non-pipeline run would access, and the only analysis differences are that incremental refresh still is missing a few features (activation metrics, quantile metrics, ratio metrics with different numerator and denominator fact tables) and that CUPED uses a fixed pre-experiment window rather than a user-specific pre-exposure window.

::::note

Availability may differ by warehouse and strategy. If you don't see an option in your datasource, please contact support.
...
Therefore, this pipeline relies all data in your experiment assignment source and fact tables are added in chronological order; in other words, a full refresh will be needed if there is any new data added to these sources with an older `timestamp` then the max timestamp seen on the last GrowthBook update.
