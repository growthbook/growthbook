...
For experimenters who have multiple metrics per experiment and have large experiment assignment sources, GrowthBook can greatly improve the performance of your queries if you enable **Pipeline Mode**, writing intermediate tables back to your warehouse and re-using those across metric analyses in an experiment. While it depends somewhat on your warehouse cost structure, pipeline mode will almost certainly provide substantial savings.
...
Incremental refresh is only available for BigQuery. Please reach out to support if your warehouse is not supported.

### How it works

On each experiment update, GrowthBook only scans new data in your warehouse to keep the amount of data consumed in each update to a minimum. On each update, GrowthBook materializes several tables in your warehouse to ensure old data does not need to be scanned again, and to reduce the computation needed in future analyses.
...
- Dramatic reduction in the cost and waiting time when running on-demand dimension analyses, as GrowthBook can use the existing materialized `Experiment Units Table` and `Units-Date Metrics Table`s to get dimension breakdowns rather than rescanning data in your warehouse.
...
- While in Beta, some analysis tools (activation metrics, quantile metrics, ratio metrics with different numerator and denominator fact tables, dimension breakdowns in Experiment Dashboards) are not supported.
...
- Any changes to fact table sources require using a full refresh of all of the data to take effect, but we are working on improvements to ensure that only the data that need be restated is restated.
