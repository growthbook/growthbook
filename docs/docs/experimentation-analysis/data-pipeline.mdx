...
- Dramatic reduction in the number of rows scanned per update, as we only have to scan your experiment assignment queries and fact tables for data in partitions that are on or after the last timestamp seen in the previous update.
- Dramatic reduction in the cost and waiting time when running on-demand dimension analyses, as we can use the existing materialized `Experiment Units Table` and `Units-Date Metrics Table`s to get dimension breakdowns rather than rescanning data in your warehouse.
