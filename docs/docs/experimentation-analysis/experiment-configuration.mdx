...
**Fallback attribute** (Sticky Bucketing enabled only) - If the hash attribute is not present, you can also bucket by a fallback attribute, such as an anonymous or session id. With Sticky Bucketing enabled, once the user logs in and is assigned a hash attribute, their bucket will be "sticky" and will not change, even if the fallback attribute is later lost.
