...
Each rule is made up of one or more conditions. A condition checks whether some, all, or none of your goal or guardrail metrics are statistically significant in a given direction. There are four directions, and the direction you choose also controls _when_ the condition is evaluated:

- **Stat Sig Good** / **Stat Sig Bad**: regular statistical significance. "Stat Sig Bad" is evaluated at all times, so harm can be detected early. "Stat Sig Good" only triggers a decision once the experiment is ready to make one (its target power has been reached, or sequential testing is enabled). This avoids declaring a winner before you have collected enough data.
- **Super Stat Sig Good** / **Super Stat Sig Bad**: significance at a stricter threshold (Frequentist engine: p-value < 0.001; Bayesian engine: chance to win > 99.9%). These are evaluated at all times, including before the target power is reached. Use **Super Stat Sig Good** when you want to ship early on a strong positive signal — early shipping requires an explicit Super Stat Sig Good condition, so a regular "Stat Sig Good" rule will never ship before the experiment is ready.

Guardrail conditions only support the harmful (Stat Sig Bad) direction.

### Target Minimum Detectable Effects

The **Target Minimum Detectable Effects** (MDEs) are the smallest effects you want to be able to reliably detect before making a decision. For example, suppose 10% is your target MDE value (10% is the GrowthBook default). If your confidence intervals range from 10 to 20\%, then you have reached your desired precision. Once this desired precision is reached, your experiment has collected enough data for the decision criteria to be applied and a ship, rollback, or review decision is given for your experiment. Note: a decision can be given before the target MDE is reached. Harmful results (Stat Sig Bad) are always evaluated, so a rollback can be recommended early; results that are significant at the stricter super stat sig threshold (Super Stat Sig Good / Super Stat Sig Bad) are also evaluated early; and if you have sequential testing enabled, regular results become valid to act on before the target MDE is reached.
...
| Ship now         | This status appears when your decision criteria to Ship Now have been met for at least one variation AND either (1) the goal metrics have reached their target MDEs, (2) Sequential Testing is enabled in our Frequentist engine, OR (3) the deciding rule uses a Super Stat Sig Good condition met at the strict threshold (Frequentist engine: p-value < 0.001; Bayesian engine: CTW > 99.9\%). Before the experiment is ready (and without sequential testing), a variation only ships early via an explicit Super Stat Sig Good condition. | Ship the variation that fits the above criteria!                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
| Roll back now    | This status appears when your decision criteria to Rollback Now have been met for all test variations. Harmful conditions (Stat Sig Bad and Super Stat Sig Bad) are evaluated at all times, so a rollback can be recommended before the target MDEs are reached.                                                                                                                                                                                                                                                                               | Stop the experiment and keep the status quo!                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| Ready for review | This status appears when your decision criteria to Review have been met for any variation AND the goal metrics have reached their target MDEs. Review is only surfaced once the experiment is fully powered, not for premature results.                                                                                                                                                                                                                                                                                                        | Consider the trade-offs between shipping, rolling back and iterating, or continuing to run the experiment in these ambiguous cases.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
