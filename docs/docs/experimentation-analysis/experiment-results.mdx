...
We use the frequentist engine with sequential testing disabled to calculate the p-values for the pre-exposure mean imbalance, with a significance threshold of 0.001. We adjust for testing multiple metrics using a Bonferroni correction. We calculate the p-values for the pre-exposure mean imbalance using a two-sample t-test on the absolute difference in means.
