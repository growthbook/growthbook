...
Time series showing the cumulative results of your experiment though a given experiment update are available in-line in the results section. They use a history of experiment updates (either manually triggered or from automatic refreshes) to show how the results have evolved over time. To read more about time series, see the [Experiment Time Series](/app/experiment-time-series) documentation.
