...
sidebar_label: Experiments (Results)
...
<img
  src="/images/results-tooltip.png"
  alt="Results Tooltip"
...
These difference types can have slightly different statistics in the case of the Frequentist engine, because for Absolute and Scaled Impact differences, we do need to account for the uncertainty in estimating $\mu_c$ as we do with Relative difference types (which forces us to use a delta method derived variance to properly handle).

Furthermore, with CUPED enabled in the Frequentist engine, you may find that the changes are not exactly the same as the difference in raw variation averages, due to CUPED adjusting those averages under the hood to reduce variance.
...
:::tip
To get access to the health tab, you need to enable us to run one additional query per experiment analysis. You can do this by enabling "Run traffic query by default" under Settings > General.
:::

The health tab provides you with more insights on the traffic to your experiment over time and across dimensions.

<img
  src="/images/health-tab.png"
  alt="Experiment Health Tab"
  style={{ width: 800, margin: "0 auto" }}
/>

**Experiment Traffic** - A plot of experiment units by the first date they were exposed to the experiment. We do return a health check for that individual day in a tooltip, but beware, long running experiments can frequently have a few days that looks imbalanced when in reality it is a false positive.
...
We also provide you with a dimension chooser that will provide dimension specific splits and traffic analysis automatically when you run your experiment. You can select certain Experiment Dimensions to automatically pre-compute when you refresh your overall results so that you don't need to compute dimension metric results just to see the traffic breakdown.

<img
  src="/images/health-dim.png"
  alt="Health Dimension Splits"
  style={{ width: 600, margin: "0 auto" }}
/>

By default, we do not include any dimensions in the health tab. This is because if a dimension is too high cardinality, we would be unable to process the returned data and show you meaningful results. Ideally, you can define your Experiment Assignment Queries such that you only return a few values for each dimension.

If you have some dimensions that are have only a few, meaningful different values (e.g. you modify your query so that you only return "Edge", "Safari", "Chrome", "Firefox", "Other" for a `browser` column), then you can navigate to your Experiment Assignment Query, click edit, and add a dimension to be computed for your health tab like so:

<img
  src="/images/edit-experiment-assignment-query.png"
  alt="Experiment Assignment Query"
  style={{ width: 600, margin: "0 auto" }}
/>

<img
  src="/images/add-traffic-dim.png"
  alt="Adding Browser as a Traffic Dimension"
  style={{ width: 600, margin: "0 auto" }}
/>
