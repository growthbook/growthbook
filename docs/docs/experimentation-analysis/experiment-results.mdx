...
- **CUPED and regression adjustment** are always off for funnel metrics and their steps.
- **Metric slices** are not supported. The Slices tab is replaced by the Funnel tab.
- **Activation metrics** cannot be funnel metrics.
- **Incremental Refresh** does not support funnel metrics while the incremental pipeline is in beta.
