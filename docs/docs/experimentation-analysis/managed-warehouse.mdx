...
2. Any other attribute marked as an **Identifier** under **SDK Connections** &rarr; **Attributes** (for example `account_id`) is automatically exposed as a top-level column on your fact tables and available for experiment assignment.
