...
Bandits optimize towards winning variations by making frequent changes to variation weights. Therefore it is important to ensure that your SDK implementation is able to receive updates in a timely fashion.
...
- Get a decent mix of different units into your experiment (e.g. running a 8 hour Exploration Window may capture only users in particular time zones before it begins updating).
