...
of an experiment. The same  and override rules apply exactly the same to client-side experimentation.
...
It's important to note that none of these implies that GrowthBook exposes a mechanism known as [sticky bucketing](/app/sticky-bucketing) without further configuration. It's all
about the consistent evaluation of variations based on the particular experiment settings.
