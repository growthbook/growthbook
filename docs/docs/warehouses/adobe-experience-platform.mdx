...
- **Per-license session and concurrency limits.** Your Adobe license caps how many sessions and concurrent queries you can run. GrowthBook defaults `maxConcurrentQueries` to 1, which matches the standard Query Service entitlement. Raise it in the connection settings only if your Adobe license allows more concurrent queries.

## 5. Current limitations

A few capabilities are off for Adobe Experience Platform at this moment. Please reach out to support if any of these are a blocker for you and your team.
