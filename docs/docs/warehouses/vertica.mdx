...
Create a user in Vertica for the purpose of connecting GrowthBook to your Vertica warehouse. You can re-use an existing user, but using a dedicated user is recommended to keep access control simple and limited to necessary permissions.

This user needs to:

- have SELECT permissions in the schema that houses your experiment exposure and metric data.
...
Once you select "Vertica" you will enter the credentials to connect GrowthBook to your warehouse. We need a host that GrowthBook can connect to, a port, and the user credentials from Step 1.
...
You can additionally use SSL to validate your connection.
