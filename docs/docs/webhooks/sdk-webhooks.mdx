...
Failed SDK webhooks are attempted one additional time after a 5 second delay. If still failing, the particular SDK payload event will be skipped. Future SDK payload events will start fresh with a new attempt.
