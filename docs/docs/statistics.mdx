...
fits their use case best under their organization settings. The current frequentist engine computes simple two-sample t-tests for differences in means,
but future extensions to implement sequential testing, variance reduction, and more are possible.
