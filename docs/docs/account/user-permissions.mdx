...
Enterprise organizations using GrowthBook have the ability to create teams - When creating a team, you can define a global role and project-level roles similar to how you can for users, but, once a team is created, you can add multiple users to a team, and once a user is added, they will inherit all of the permissions granted by the team. This can be especially handy when paired with [GrowthBook's SCIM integration])(https://docs.growthbook.io/integrations/scim), to automate provisioning and de-provisioning users. 
