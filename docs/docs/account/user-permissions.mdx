...
Within GrowthBook, not all resources are project-specific. For example, `Attributes`, `Saved Groups`, `Namespaces`, `Environments`, `Presentations`, and `Ideas` all live at the organization level. This means that all users, regardless of role, will be able to view these resources.
