...
Each GrowthBook user needs an email address, and you can select what global permissions you want to assign and select per-project overrides (if any). For example, a new user could be a `collaborator` by default for all projects, but on the `mobile` project, they could be an `experimenter` so they can manage all feature flags and experiments assigned to that project. 
