...
[Sample Ratio Mismatch (SRM)](/statistics/srm): A Sample Ratio Mismatch (SRM) in GrowthBook is a health check that warns you when the actual traffic split between experiment variations significantly deviates from the expected split, indicating potential issues with the experiment configuration or implementation. GrowthBook supports both chi-squared and sequential Bayes factor detection methods.
