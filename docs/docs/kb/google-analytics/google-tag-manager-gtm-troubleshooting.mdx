...
Ensure the `trackingCallback` is correctly implemented and that the required dataLayer variables and triggers in GTM have been configured properly by reviewing [Using GrowthBook with Google Tag Manager (GTM)](https://docs.growthbook.io/guide/google-tag-manager-and-growthbook#4-tracking-via-datalayer-and-gtm).
...
If you have verified your GTM setup and still see discrepancies, use GTM's Preview mode to ensure the `experiment_viewed` event is being pushed to the dataLayer and that your GA4 Event Tag is firing as expected.

Also, check GA4's real-time reports to confirm event receipt ([more details in our GTM setup guide](https://docs.growthbook.io/guide/google-tag-manager-and-growthbook#testing-your-setup)).
...
If you've confirmed that the `trackingCallback` is implemented and firing, GA4 is loaded, and you are still not seeing `experiment_viewed` events, we recommend logging the trackingCallback execution to check for errors. Add logging statements through the GrowthBook-related code in your application, then check in your browser's Developer Tools Console to further debug the issue.