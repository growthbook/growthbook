...
Official Resources are protected resources that **can only be modified by users with appropriate permissions**. These resources display an "Official" badge in the app to indicate their protected status. This feature is useful for resources that are externally managed or need to remain consistent ac
...
At this time, only `Legacy Metrics` can be marked or unmarked as "Official" in the app. We are actively working on adding this feature to other resource types.
