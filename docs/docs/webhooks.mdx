...
For instance, if you select `Project Apollo` as a project filter, only events related to `Project Apollo` (and any other selected project) will trigger your webhook. If the projects filter is empty, your webhook will fire for all events on any project.
