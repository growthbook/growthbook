...
[**Experiment Analysis**](/experimentation-analysis/experiments) queries your data warehouse for raw experiment data and runs it through our stats engine to produce a report. No raw user-level events or PII are ever sent to GrowthBook. We only get back aggregate info instead (sums, sums of squares, etc.).
...
All features for an environment are packaged together into a single JSON file. All you need to do is pass this into our [SDKs](/lib) along with [user targeting attributes](/features#targeting-attributes).

Read more about [features here](/features).

## Experiment Analysis

GrowthBook needs to connect to your [Data Source](/experimentation-analysis/connecting-to-your-data-warehouse) in order to query experiment results. We support all of the popular SQL data warehouses in addition to Mixpanel and Google Analytics. GrowthBook is extremely flexible and can support almost any schema structure with a little bit of configuration.

![Data warehouses supported by GrowthBook](/images/GrowthBook-supported-DB-766.png)

Once connected to a data source, you need to build a re-usable library of [Metrics](/experimentation-analysis/metrics). Metrics are what your experiments are trying to improve. Metrics are defined via SQL (if your data source supports it) or a simple query builder. The following types are supported:
...
Once these are metrics are set up, you can import experiments and start analyzing the results. GrowthBook uses a [Bayesian statistics engine](/experimentation-analysis/statistics/overview) to determine the probability that a variation is better than the control, as well as how much better it is and how risky it is to stop the experiment now.
...
Read more about [experiments here](/experimentation-analysis/experiments).
