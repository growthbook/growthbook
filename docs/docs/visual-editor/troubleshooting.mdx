...
1. **`Include Visual Experiments` toggle is off** on the SDK Connection. Visual experiments are stripped from the SDK payload unless this is enabled. Go to **SDK Configuration → SDK Connections**, edit the connection your site uses, and turn it on.
2. **The SDK isn't loaded yet** when the variation tries to render. Custom JS variations need to wait for the target element — use the `waitFor` helper from [Manual mode → Custom JavaScript](/app/visual/manual-mode#custom-javascript).
3. **URL targeting doesn't match.** The current URL doesn't satisfy the experiment's URL targeting rule. Check the rule and verify with the Debug Panel.
4. **The experiment is still a draft** and the SDK Connection doesn't have **Include Drafts** enabled. Draft experiments aren't delivered by default.
...
## Dark mode UI is light (or vice versa)

The side panel's theme can be toggled in the gear menu. If it's stuck:

- **Reload the side panel.** Right-click in the side panel → Reload.
- **Reinstall the extension** if the theme persists across reloads. The theme is stored in extension local storage; reinstalling clears it.

The side-panel theme is independent of your GrowthBook web UI theme — they don't sync.

## Drag and drop didn't reposition the element

- **Drop target wasn't valid.** Look for the blue drop indicator. If you don't see one as you drag, you're hovering over a region that isn't a valid drop zone.
- **Absolutely-positioned elements** (`position: absolute`, `fixed`) are repositioned with explicit offsets rather than DOM moves. The "Undo" affordance reverts the offsets.
- **Container has `display: contents` or other unusual layout** — drag targets need a real layout to anchor to.
...
See [Permissions](/account/permissions) for the full permission model.
