...
1.  **[Sample Ratio Mismatch](/statistics/srm)** (SRM) detects when the traffic split doesn't match what you are expecting (e.g. a 48/52 split when you expect it to be 50/50). GrowthBook supports both a standard chi-squared test and a sequential Bayes factor test that provides anytime-valid detection under continuous monitoring.
