...
Here we document the technical details behind GrowthBook mid-experiment power calculations. We make the following assumptions:
...
Below we outline different approaches for both Bayesian and frequentist inference.
...
1. there is a constant treatment effect throughout the experiment;
2. average new user traffic counts over the most recent 7 days equals the new user traffic counts going forward;
3. the experimenter stops the experiment after achieving a statistically significant result.
...
Below we describe GrowthBook's [production Bayesian model](/statistics/details#bayesian-engine).
...
<img
  src="/images/statistics/power-decreasing.png"
