...
Mid-experiment power estimates the number of additional days needed so that each goal metric could detect a target lift (default of 1\%) with at least 80% probability.
We combine uncertainty estimates from the experiment results with the target lift to estimate the number of additional days needed.
Suppose revenue is your single goal metric, and we estimate that in 7 days you would be able to detect a 1\% lift for each goal metric with at least 80\% probability.
Then the estimated number of additional days displayed is 7.
If you have opted into selecting a maximum duration for your experiment, then the number of additional days left will never be more than the maximum duration minus the current length of the experiment.

The second purpose is to warn you if your experiment is low-powered.
For each goal metric within a variation, GrowthBook estimates the probability the estimate will be statistically significant.  
If any of these estimates are less than 10\%, then GrowthBook will display a low power warning on the Health tab.
...
Note that even if your goal metrics are statistically significant, you may see a low-powered warning.  
This is because the power estimates are based upon the target lift, and the target lift may be smaller than the observed estimates.

We emphasize that number of days left is a projection, and power estimates are estimates.  
These values are predicated upon the following assumptions:

1. that the target lift $\Delta$ is the true lift;
...
We believe that mid-experiment power estimates can still be useful for providing guidance, even if these assumptions may not hold.  
For example, if the projected days remaining is short, if your treatment continues to affect logged-in users already included in the experiment, then the power estimate provided may be biased low.
...
In both cases, even rough estimates can help you decide whether or not to continue the experiemnt.  
Pre-experiment power analysis is also based upon assumptions that may not hold (e.g., correctly specified lift).  
Pre-experiment power results can still be used to decide whether the experiment should be run, and if so, what the sample size should be.
In summary, rough estimates can be helpful for deciding whether to stop or continue the experiment.
