...
## What is mid-experiment power, and what is it used for?

Mid-experiment power calculates the probability of observing a statistically significant result at the end of the experiment, given the data observed so far in the experiment.

GrowthBook uses mid-experiment power estimates for two purposes, the first of which is to help determine experimental duration.
Mid-experiment power estimates the number of additional days needed so that each goal metric has at least 80\% chance of attaining statistical significance.
If this is less than the maximum duration minus the current length of the experiment, then GrowthBook displays the standard number of additional days needed.
Otherwise, GrowthBook displays the maximum duration minus the current length of your experiment.

For example, suppose the maximum duration is 6 weeks (which is the default), and the current length of the experiment is 2 weeks.
Then the standard number of additional days needed is 7 \* (6 - 2) = 28.
Suppose revenue is your single goal metric, and we estimate that in 7 days you will have at least 80% power for revenue.
Then the estimated number of additional days displayed is 7.

The second purpose is to warn you if your experiment is low-powered.
For each goal metric within a variation, GrowthBook estimates the probability of observing a statistically significant result at the end of the experiment, given the data observed so far in the experiment.  
If any of these estimates are less than $2\alpha$, then GrowthBook will display a low power warning on the Health tab.
If mid-experiment power is low and the cost of running your experiment is high, then you may want to stop the experiment early, as the additional information gained from running the experiment longer may not be worth the cost.
Alternatively, if some of your goal metrics are not low-powered and the cost of running your experiment is low, then you may want to continue the experiment.

We emphasize that number of days left is a projection, and power estimates are estimates.  
These values are predicated upon the following assumptions, which may not hold:

1. there is a constant treatment effect $\Delta$ throughout the experiment, that is equal to two times the minimum effect size for the metric;
2. average new user daily traffic counts over the most recent 7 days equals the new user daily traffic counts going forward; and
3. the treatment effect estimate for users already in the experiment does not change during the experiment (i.e., logged in users have no additional behavior during the experiment).

While these assumptions may not hold, we believe that mid-experiment power estimates can still be useful for providing guidance.
For example, if your goal metrics are close to statistical significance, if your treatment continues to affect logged-in users already included in the experiment, then the power estimate provided may be biased low.
Alternatively, if new users added over the last 7 days are not representative of new users going forward, then the power estimate provided may be biased high.
Pre-experiment power analysis is a tool used to decide whether the experiment should be run, and if so, what the sample size should be.
These decisions are based upon the assumption of an effect size which is unknown.
Mid-experiment power is similarly based upon assumptions that may not hold, but can still be useful for providing guidance.

Mid-experiment power technical details can be found [here](/statistics/midexperiment-power-technical).
...
Each engine arrives at these values differently. Below we describe high-level technical details of our implementation. All technical details except for mid-experiment power can be found [here](/statistics/power-technical).
