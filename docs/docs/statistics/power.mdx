...
GrowthBook uses mid-experiment power estimates for two purposes.  
First, GrowthBook estimates the number of additional days needed to run the experiment.
The standard number displayed is the maximum duration minus the current length of your experiment.
...
If this number is less than the standard number, we show the estimated number of additional days needed.
...
For each goal metric, GrowthBook estimates the probability of observing a statistically significant result at the end of the experiment, given the data observed so far in the experiment.  
...
2. average new user traffic counts over the most recent 7 days equals the new user traffic counts going forward; and
3. the treatment effect estimate for users already in the experiment does not change during the experiment (i.e., logged in users have no additional behavior during the experiment).
   While these assumptions may not hold, we believe that mid-experiment power estimates can still be useful for providing guidance. For example, if your goal metrics are close to statistical significance, if your treatment continues to affect logged-in users already included in the experiment, then the power estimate provided may be biased low. Alternatively, if new users added over the last 7 days are not representative of new users going forward, then the power estimate provided may be biased high. Pre-experiment power analysis is a tool used to decide whether the experiment should be run, and if so, what the sample size should be. These decisions are based upon the assumption of an effect size which is unknown. Mid-experiment power is similarly based upon assumptions that may not hold, but can still be useful for providing guidance.
