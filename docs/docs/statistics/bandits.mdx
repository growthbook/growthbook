...
Multi-armed bandits (MABs) are an experimental design where the variation weights evolve during the experiment. This is in contrast to standard GrowthBook A/B tests, where the variation weights do not change during the experiment.

## When should I run a multi-armed bandit experiment?

Multi-armed bandits are helpful when: 1) you want to quickly identify the best arm with respect to a single metric; 2) you want to minimize costs during the experiment; and 3) you have 5 or more variations.

## How do GrowthBook multi-armed bandits work?

Suppose you have 10 variations in your experiment, and your key metric is revenue. In the first part of the experiment (the exploratory phase), traffic is randomly assigned to variations with equal probability. After the exploratory phase, GrowthBook estimates the probability that a given variation has the highest mean revenue for each variation. During the exploit phase, variations with larger revenue receive larger percentages of user traffic. After collecting more data, we update the weights again. We repeat this cycle multiple times. You can specify the amount of time between updates (see [FAQ](#faq)).
...
Second, a MAB can often identify the best arm more quickly than an A/B test. If you have 10 variations, you may be able to quickly determine that several result in low revenue. Sending traffic to the higher revenue variations both saves money and provides more statistical power for distinguishing the winner.

However, bandit results can be biased, please see [here](#bias). This bias does attenuate as we collect more data. If your goal is unbiased estimation, standard experiments with a fixed runtime are the gold standard. You will have to balance the potential benefits of a bandit (i.e., likelier to identify the best arm, reducing the cost of experimentation), against the risk of biasing your results.

## How do you determine how much traffic to send to each variation?

GrowthBook employs Thompson sampling, which allocates traffic proportionally to the probability that an arm yields the largest revenue. By sending some traffic to arms that are less likely to have the highest revenue, Thompson sampling _explores_ all variations when searching for a winner. Thompson sampling _exploits_ information about which arms are likely to be best by sending more traffic to arms that have resulted in the largest revenue so far.

Sending more traffic to larger revenue arms saves money.
...
In practice, GrowthBook ensures that each variation has at least 1% of traffic (roughly), in case user behavior changes over time.

## How do I run a bandits experiment?

To run a bandit experiment, go to "Experiments" on the left side-bar, and click "Bandit Experiments". Select the "Add Experiment" button.

When creating your experiment, most of the fields are the same as those described in [setup of a standard A/B test](/app/experiment-configuration). We list key differences below.

First, you will need to specify a decision metric, which is the metric used to determine how much traffic to send to each variation. In our example above, revenue was the decision metric.

Second, you will need to select the duration of the burn-in period, during which each variation weight is equal (e.g, if there are 10 variations, then each variation receives 10% of traffic).

Finally, you will need to indicate [how frequently you want your variation weights updated](#what-should-my-update-cadence-be).

## What should my update cadence be?

Your update cadence defines the period of time between variation weight updates.

You should pick the cadence that satisfies your logistical constraints while balancing: 1) the amount of information gleaned from the experiment; and 2) the cost of experimentation.
