...
8. Are quantile estimates available via [incremental refresh](/app/data-pipeline#incremental-refresh-recommended-in-beta)? Yes - for BigQuery, which stores event-level histograms for each user. This provides approximate inference, as small effects may be undectable. If the control and treatment quantiles are different but within the same histogram bin, the estimate will go to 0 as the sample size increases, due to histogram rounding.
