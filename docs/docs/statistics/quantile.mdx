...
:::note
Quantile Testing is a GrowthBook Pro and Enterprise feature. It is not compatible with Mixpanel, Redshift, or MySQL integrations.
:::
...
A quantile treatment test tries to estimate this difference. You interpret the interval above as “There is a 95% chance that the difference in P99 latencies across the groups is in (-9ms, -5ms)”.
...
1. Navigate to your [Fact Table](/app/fact-tables) and select "Add Metric".
2. Select “Quantile” from “Type of Metric”.
3. Toggle “Group by Experiment User before taking quantile” if you want to compare quantiles across variations at the user granularity, after summing row values at the user level. The default is at the event granularity.
...
Let $\hat{Y0}_{n\nu}$ be the $\nu^{\text{th}}$ sample quantile for control, and let its associated variance be $\hat{v}_{0}$.
Analogously define $\hat{Y1}_{n\nu}$ and $\hat{v}_{1}$ for treatment.
Given these terms, our relative effect estimate is $\hat{\tau} = (\hat{Y1}_{n\nu} - \hat{Y0}_{n\nu}) / \hat{Y0}_{n\nu}$ if $\hat{Y0}_{n\nu}\ne 0$ and is 0 otherwise.
We calculate $v_{\tau}$, the variance of $\hat{\tau}$, by using the delta method formula above.

In summary, our quantile treatment effect estimate for control is

For our Bayesian engine, we simply use a normal/normal conjugate prior to construct a Bayesian credible interval, which in turn can be used for risk estimates, probability that $\tau$ is positive, etc. We put a normal prior on the true quantile treatment effect, i.e., $\tau \stackrel{}{\sim}\mathcal{N}\left(\mu_{0}, \sigma_{0}^{2}\right)$. Then $\hat{\tau}|\tau \stackrel{}{\sim}\mathcal{N}\left(\tau, s^{2}\right)$ , so the posterior distribution for the treatment effect is $\tau|\hat{\tau} \stackrel{}{\sim}\mathcal{N}\left(\Omega^{-1}\omega, \Omega^{-1}\right),$ where the precision $\Omega = 1/\sigma_{0}^{2} + 1/s^{2}$ and $\omega = \mu_{0}\sigma_{0}^{2} + \hat{\tau}/s^{2}$.
We place a flat prior over the effect size. We combine the
