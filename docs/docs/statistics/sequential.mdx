...
In the above, $\hat{\mu}$ is our estimated uplift, $\hat{\sigma}$ is the estimated variance of the uplift, $\alpha$ is our significance level (defaults to 0.05), and $N$ is the sum of the two variation's sample sizes.
