...
:::info Sequential Testing for metric analysis is only implemented for the Frequentist statistics engine. GrowthBook also offers a separate [Sequential SRM Detection](/statistics/srm#sequential-srm-test) method that works with any statistics engine.
:::
