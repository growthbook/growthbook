...
- Ratio metrics that are not [Fact Metrics](/app/metrics#fact-tables) and where the denominator is a count, revenue, or duration metric
...
However, you can see that there is an icon showing that CUPED is disabled for Average Order Value (because it is a ratio metric but not a fact metric).
CUPED is applied to the Fact Metric version of this metric (in this test dataset the adjustment is small).
...
<MaxWidthImage maxWidth={300} border>
  ![Metric Icon showing CUPED Disabled Reason](/images/statistics/cuped-icon-ratio.png)
</MaxWidthImage>

If you mouse over the metric name itself, you get the final CUPED status for each metric (on/off, number of lookback days).

<MaxWidthImage maxWidth={300} border>
  ![Metric Tooltip with CUPED status](/images/statistics/cuped-tooltip-ratio.png)
</MaxWidthImage>
