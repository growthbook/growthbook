...
CUPED + post-stratification is available to Pro and Enterprise customers.
...
Regression adjustment combined with post-stratification can bring additional accuracy gains in three ways.

Most importantly, we estimate average lift across all users by weighting the strata-specific (e.g. country-specific) lifts. If the strata strongly correlate with the metric of interest, then the variance we estimate will not be inflated by the differences in metric behavior across strata. In other words, if average user revenue is high in the US and UK, but low in France, then more of the variance in the lift can be attributed to the experiment rather than the differences in revenue behavior across countries.
...
Finally, we can estimate country-specific lifts by running CUPED for each country individually. Estimating strata-specific relationships in pre-experiment and post-exposure metric behavior can lead to additional variance reduction when the relationship between the two varies.

You can also read more technical details about when post-stratification is useful and how, with large enough data, it is often as good as stratifying your experiment ahead of time (see section 3.4 in [Xie and Aurisset 2016](https://www.kdd.org/kdd2016/papers/files/adp0945-xieA.pdf)).
