...
If you Navigate to the Experiment page, and click Add Experiment, you'll see the following panel.

![Experiment Import Modal](/images/import-experiment-modal.png)

On this page you can either create an experiment using some metadata that we infer from your metric source using the Import buttons on the right hand side.
...
Attribution models deal with how we attribute metric values (conversions, counts, any arbitrary value to be aggregated at the unit level) to individual units (e.g. users). They rely on the concept of "conversion windows" which is discussed more in depth on the 9Metric documentation page](metrics).
...
Currently, all analyses will use the "First Exposure" model by default. The _Experiment Duration_ model replaced _All Exposures_ as of GrowthBook 2.0. You can read more about why [here](BLOGPOST_LINK).
