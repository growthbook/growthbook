...
When you connect a supported datasource to GrowthBook, we automatically generate metadata that is consumed by our Schema Browser. The Schema Browser is a user-friendly interface where you can easily explore information about the datasource such as databases, schemas, tables, columns, and data types.

![GrowthBook Schema Browser](/images/growthbook-schema-browser.png)

Below are the datasources that currently support the Schema Browser:
...
**Note:** If you added a supported datasource prior to GrowthBook v2.0, you can generate the schema manually by clicking "Datasources" on the left-nav, selecting the datasource, and then clicking the "View Schema Browser" button and following the on-screen prompt.
