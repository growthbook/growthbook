...
If you've set up multiple projects within GrowthBook, you can optionally scope your SDK Endpoint to a specific project. By default, features from all projects are returned.
...
You will need to decrypt the features first before passing into the SDK. Our front-end SDKs (Javascript and React) have a `setEncryptedFeatures` helper method to make this easy:

```ts
growthbook.setEncryptedFeatures(json.encryptedFeatures, "MY_ENCRYPTION_KEY");
```

You can find your encryption key on the **Features -> Environments** page in GrowthBook.
