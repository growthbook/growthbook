...
To use our Visual Editor, the only requirement is to install the [GrowthBook Chrome Extension](https://chrome.google.com/webstore/detail/growthbook-devtools/opemhndcehfgipokneipaafbglcecjia).
...
useEffect(() => {
  const onChange = () => growthbook.setURL(window.location.href);
