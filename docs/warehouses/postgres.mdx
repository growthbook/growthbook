...
description: Connect Postgres to GrowthBook with a read-only user, allowlist the Cloud IP if needed, and enter host, port, database, and SSL settings.
...
For Postgres that means your firewall, security group, and `pg_hba.conf`.
