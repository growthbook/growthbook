...
Prompts can also depend on the value chosen. On a broad `events` table, a `path` column only matters for page views and `properties.modalType` only for modal events. Under the checkbox, map values of this column to the extra column or JSON field to prompt for, such as `Page View` → `path`. As soon as a metric or exploration filters `event_name` to exactly one mapped value, GrowthBook adds the extra prompt, and removes it again while still empty if the value changes. Filtering to several events at once never adds a prompt, since a column like `path` only exists for some of them.
