...
The prompt can also be conditional. On a broad `events` table, a `path` column only matters for page views, so check "Only when another column has certain values" and pick `event_name` = `Page View`. GrowthBook then prompts for `path` as soon as a metric or exploration filters `event_name` to `Page View`, and removes the empty prompt again if that filter changes. Properties that live inside a JSON column can be exposed this way through a virtual column on the fact table.
