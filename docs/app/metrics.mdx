...
description: Define Fact Tables and metrics so GrowthBook can compute experiment results from your warehouse.
slug: /app/metrics
...
There are 7 types of metrics you can create within GrowthBook - Proportion, Retention, Daily Participation, Mean, Ratio, Quantile, and Funnel.
...
Quantile Metrics are useful when you care about comparing variations at a quantile (e.g. latency at P99) rather than the mean. These are also called percentile metrics. Learn more about when to use [Quantile Metrics](/statistics/quantile).
...
Funnel Metrics measure the share of exposed units who complete a series of steps in order, such as viewing a product, adding it to a cart, and then purchasing. Instead of a numerator, you define an ordered list of steps, each with its own Fact Table, row filters, and conversion window.

Each step is measured against all exposed units, not just the units who completed the previous step, which keeps every step directly comparable to your other metrics. The metric's own value is the share of units who reach the last step.

See [Funnel Metrics](/app/metrics/funnel) for step settings, conversion windows, optional steps, and the API.
...
- `>=`
- `<=`
- `between` (takes two values: a lower and an upper bound)
- `not_between`
- `in`
- `not_in`
- `is_null`
...
- `starts_with`
- `ends_with`
- `matches_pattern` (glob: `*` matches any run of characters, `?` matches one)
- `not_matches_pattern`
- `sql_expr` (raw SQL for advanced use cases)
- `saved_filter` (a saved SQL expression that can be reused across metrics)

Filters on date and timestamp columns are compared in UTC. The UI offers `=`, `<`, `<=`, `>`, `>=`, `between`, `not_between`, and `is not null` for these columns, and a value is either a calendar day (`2024-01-15`) or a minute-precision timestamp (`2024-01-15T09:30`). A calendar day matches the whole day rather than the instant at its midnight, so `= 2024-01-15` includes every row stamped on that date.
...
After creating your Fact Table, you can edit a column and check the “Prompt metrics to filter on this column” box. This helps GrowthBook better streamline the UI when creating metrics.

<Frame>![Marking an Event Type column](/static/images/metrics/inline-filter.png)</Frame>
...
<Frame>![Event Type Filter UI](/static/images/metrics/event-type-filter-metric-form.png)</Frame>
...
GrowthBook also supports secondary filters. On a broad Events table, after marking the `event_name` column to always filter, you can then choose secondary filter columns based on the selected event. For example, if the user picks the `Form Submit` event, you can have it automatically prompt for the `formId` column.
...
<Frame>![Metric Windows](/static/images/metric-windows.png)</Frame>
...
<Frame>![Metric Windows (User 2)](/static/images/metric-windows-user-2.png)</Frame>
...
**1. Absolute capping** - if set above zero, all aggregated user values will be capped at exactly this value. For example, if the cap is \$100 on total revenue per user, then after we sum all of a user's orders up, any user with an aggregate sum of greater than \$100 will be set to \$100.

**2. Percentile capping** - when this is set to between 0 and 1, it uses that percentile to select a cap based on the data in your experiment so far. This cap is therefore specific to each experiment and specific to each analysis run in that experiment if new data has come in. It works like so: after we calculate the unit-level aggregate values for all units (e.g. users) during an experiment analysis, we find the specified percentile of these unit-level aggregates and then cap these aggregated values at this percentile. Using the above example, if you were to specify percentile capping with a value of `0.95`, then we find the 95th percentile of total revenue per user (say this turns out to be \$135). We then cap those user-level aggregates at \$135.
...
Fact Metrics can also set a lower bound (floor). Absolute floors can be zero or negative; percentile floors must be strictly between `0` and `1`, such as `0.05` for the fifth percentile.

When both tails use the same type, the lower value must be less than the upper value. Two percentile tails must use the same **Ignore zeros** setting. Mixed types are allowed: if the computed percentile crosses the absolute bound, the absolute bound takes precedence.

In the API, `cappingSettings` controls the upper tail and `lowerCappingSettings` is a separate top-level field:
...
<Frame>
  <img src="/static/images/metrics/metric-dim-config.png" alt="Configuring a column for metric slicing" width="700" />
</Frame>
...
<Frame>
  <img src="/static/images/metrics/metric-dim-levels.png" alt="Configuring slice levels" width="350" />
</Frame>
...
In the **Fact Table** modal, check the **Auto-update slice levels** checkbox. When enabled, a scheduled job periodically refreshes auto slice levels from **top values**, scanning the lookback window configured in your organization's **Data Source settings** (default 14 days). The job runs:
...
<Frame>
  <img src="/static/images/metrics/auto-slice-updates-fact-table.png" alt="Fact Table modal with auto-update slice levels checkbox" width="350" />
</Frame>
...
<Frame>
  <img src="/static/images/metrics/auto-slice-levels-locking.png" alt="Column modal showing slice levels with Lock checkboxes" width="350" />
</Frame>
...
<Frame>
  <img src="/static/images/metrics/custom-slices.png" alt="Adding custom slices" width="700" />
</Frame>
...
<Frame>
  <img src="/static/images/metrics/metric-dim-results.png" alt="Metric slice results" width="800" />
</Frame>

<Note>
**Dimensions versus Slices**
...
Define Metric Groups to accomplish this. Add multiple metrics, drag & drop to order them, and add them in bulk to experiments as Goals, Guardrails, or Secondary Metrics.
