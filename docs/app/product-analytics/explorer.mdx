...
:::caution Redshift and Microsoft SQL Server
Custom SQL containing a CTE (`WITH` clause) can run as a report, but it cannot be used in a Product Analytics visualization on Redshift or Microsoft SQL Server. GrowthBook wraps the custom query with generated SQL for filtering and aggregation, and these warehouses do not support the resulting nested `WITH` clause. Use inline subqueries or a database view instead.
:::
