...
description: Export every stored experiment snapshot over a date window through the bulk results API for downstream reporting.
sidebarTitle: Bulk Results API
...
| `dimension`             | `{ type, id?, precomputed, dimensions? }`. `type` is `none`, `experiment`, `user`, a pre-exposure kind such as `date`, `datecutoff` (`id` is the ISO cutoff), or `combo` (`dimensions` lists the two constituents).                                                                                                      |
