...
This page covers the **funnel metric type** built on Fact Tables. It is unrelated to the [legacy denominator behavior](/app/metrics/legacy#denominator-ratio-funnel-metrics), where a binomial denominator metric acted as a filter on the numerator. Those legacy metrics should be replaced by these funnel metrics, although there are some slight differences in behavior.
...
3. Select the **Fact Table** the first step's events come from.
4. Name the first step and add a row filter that defines it, such as `event_name = view_product`.
5. Click **Add a step** for each additional step, in the order units should complete them. New steps start on the previous step's Fact Table and can be repointed to any other Fact Table in the same Data Source, although funnel metrics with steps on different Fact Tables will take longer to compute.
...
:::info Steps can span Fact Tables
Each step selects its own Fact Table, so a funnel can move between tables — say a `view_product` event in an events table followed by a purchase in an orders table. Every step's table must belong to the metric's Data Source, and changing a step's Fact Table clears that step's row filters.

In experiments, every step's Fact Table must share an identifier type with the experiment's assignment table (or reach it through a Join Table); otherwise the metric is excluded from analysis.
:::
...
- **Fact Table**: The table this step's events come from. New steps default to the previous step's table.
...
| [Bandit experiments](/bandits/overview)                       | Funnel metrics cannot be used as the decision metric or as secondary or guardrail metrics on Bandits.                                                                                                                              |
...
Create a funnel metric by setting `metricType` to `funnel` and passing `funnelSettings`. Omit `numerator` and `denominator`; they are not allowed on funnel metrics. The metric inherits its data source from the first step's Fact Table, and steps may reference different Fact Tables as long as they all belong to that data source.
...
- **steps**: Required. An ordered array of 2 to 20 steps. Each step needs `name`, `factTableId`, `rowFilters`, and `optional`; `conversionWindow` is optional and may be `null`. Steps may use different `factTableId`s within the metric's data source.
