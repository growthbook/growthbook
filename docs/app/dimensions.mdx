---
title: Dimensions
description: Break experiment results down by browser, country, or other dimensions from assignment queries or a dimension table.
...
The two best use cases are identifying bugs (the `browser` example) and getting ideas for dedicated follow-up experiments.

Dimensions are supported for both Mixpanel and SQL data sources.

## Custom dimensions

In addition to the dimensions you define, the dimension selector on experiment results has a **Custom** group with two dimensions you build on the fly. Neither requires any setup on your data source.

### First exposed after...

Pick a datetime (in UTC) within the experiment phase. Units are split into two slices based on whether they were _first_ exposed to the experiment before or after that time. This is useful for checking whether a fix, release, or traffic change partway through the experiment affected results.

### Combination of dimensions

Pick two different dimensions (Experiment Dimensions or User Dimensions) and GrowthBook breaks results down by their intersection, with one slice per pair of values such as `country: US & plan: premium`. As with any dimension, only the top 20 slices are kept and the rest are grouped into `(other)`. Because two dimensions multiply the number of slices, keep the number of values in each small to avoid the false positive issues discussed above.

Both are also available in the REST API when creating a snapshot: use `cutoff:<ISO datetime>` (e.g. `cutoff:2026-01-15T00:12:00.000Z`) or `combo:<dimA>::<dimB>` (e.g. `combo:exp:country::dim_abc123`) as the `dimension`.
...
<Frame>
  <img src="/static/images/configure-dimensions.png" alt="Configure Dimensions Option" width="800" />
</Frame>
