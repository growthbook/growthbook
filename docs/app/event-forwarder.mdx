...
description: Forward GrowthBook Cloud tracking events to your warehouse so you can analyze experiments without instrumenting a tracker first.
sidebarTitle: Event Forwarder
...
- **Destination** as `catalog.schema.prefix`, for example `main.analytics.gb`. GrowthBook creates `gb_events`, `gb_experiment_viewed`, and `gb_feature_usage` in that schema, clustered on `received_at`, with `attributes` and `properties` stored as `VARIANT`.
- **Zerobus endpoint** for your workspace: `https://<workspace-id>.zerobus.<region>.cloud.databricks.com` on AWS or `https://<workspace-id>.zerobus.<region>.azuredatabricks.net` on Azure. The workspace ID is the number in your workspace URL (`?o=<workspace-id>`).
