...
- **Destination** as `catalog.schema.prefix`, for example `main.growthbook.gb`. The catalog is pre-filled from your connection; use a dedicated schema such as `growthbook` that the service principal can create tables in. GrowthBook creates `gb_events`, `gb_experiment_viewed`, and `gb_feature_usage` in that schema, clustered on `received_at`, with `attributes` and `properties` stored as `VARIANT`.
