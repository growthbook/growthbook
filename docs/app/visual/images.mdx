---
title: "A/B Test Images: Upload, AI Generation, Library"
...
All three paths land in the same `<img>` (or `<picture>`) replacement and are managed identically in the [Changes panel](/app/visual/manual-mode).
...
#### `<picture>` and `srcset` handling {#picture-and-srcset-handling}
...
When you're happy with a candidate, click **Use this image** to accept it into the variation. Generated images are stored in the [image library](#library-tab) so you can reuse them later.
