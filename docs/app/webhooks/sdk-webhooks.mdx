---
title: SDK Webhooks
description: Fire a webhook whenever a single SDK Connection payload changes so you can refresh caches or rebuild clients.
sidebarTitle: SDK Webhooks
...
Once an SDK webhook is created you will be able to view the status and fire a test event.
...
Follows the **Standard Webhooks** specification. Includes a JSON-encoded SDK Payload in the `data.payload` field.
...
<Tip>
**Updating Edge Config items owned by a team**
