---
title: SQL Templates
description: Insert start and end dates, experiment ids, and other variables into SQL so warehouse queries stay efficient.
sidebarTitle: SQL Templates
...
The `1=1` part is a common SQL trick to make appending additional `AND` conditions easier. When the Custom Field is unavailable, the WHERE clause will simply be `WHERE 1=1`, which is always true and does not filter out any data.
