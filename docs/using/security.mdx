---
title: Security
description: "How GrowthBook handles data security: what we store, what stays in your warehouse, and how to avoid exposing PII in client-side targeting."
---
...
on the [GrowthBook security page](https://www.growthbook.io/security) and on [GitHub](https://github.com/growthbook/growthbook/blob/main/SECURITY).
...
[updating GrowthBook](/self-host#docker-tags)
...
- Set the `NODE_ENV` environment variable to `production`. This will enable additional optimizations and disable some debugging features.

## Audit logs

GrowthBook keeps an audit log of all actions taken on the platform. The audit log is useful if you need to
determine what actions a user has done. You can access this page from the Settings → Log in the left navigation.
See the [audit logs docs](/account/audit-logs).
