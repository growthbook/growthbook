...
export class GrowthBookFlagProvider implements FeatureProvider {
  private constructor(private readonly client: GrowthBookClient<AppFeatures>) {}
  ...
  public static async connect(options: GrowthBookConnectOptions): Promise<{
    ...
    return {
      provider: new GrowthBookFlagProvider(client),
      receipt,
    };
  }
...
The SDK's `init` method reports failures in its return value instead of throwing for normal network and timeout failures. Until a usable payload loads, feature evaluation returns `null`; this adapter converts that state to the explicit call-site fallback and marks it unusable.
...
For a long-running Node process, decide how feature definitions refresh. The [Node.js SDK documentation](/lib/node#refreshing-features) covers streaming and polling. If you use polling, do not create one timer per request. If you use streaming, configure the required `EventSource` support before constructing the client. Keep the initial migration on the simplest refresh mechanism already supported by your runtime, then test propagation separately.
