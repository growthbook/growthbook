---
title: "Test an LLM Prompt or Model Change"
...
1. Capture at least 50 to 200 representative, redacted inputs from production or construct a reviewed evaluation set.
...
export class InvalidLlmOutputError extends Error {
  constructor() {
    super("LLM response failed validation");
...
export function validateGenerateResult(value: unknown): GenerateResult {
  if (!isRecord(value) || !isRecord(value.output) || !isRecord(value.usage)) {
    throw new InvalidLlmOutputError();
  }
  const { output, usage } = value;
  if (
    typeof output.answer !== "string" ||
    typeof output.needsEscalation !== "boolean" ||
    !Array.isArray(output.citedSnippetIds) ||
    !output.citedSnippetIds.every((id: unknown) => typeof id === "string") ||
  ...
  return {
    output: {
      ...
      citedSnippetIds: output.citedSnippetIds,
    },
...
Your real provider adapter must parse and validate the structured response before returning it. Throw `InvalidLlmOutputError` for malformed JSON or a schema failure; preserve ordinary transport/provider failures as other errors. Do not put raw prompts or responses in the error message. The workflow also validates the normalized result at its boundary, so a faulty adapter cannot silently record malformed output as a success. Shape validation does not establish factual accuracy or policy compliance; retain the separate evaluation and policy checks.
...
```ts title="src/experimentation/feature-refresh.ts"
...
import { startFeaturePolling } from "./feature-refresh";
...
export async function initializeGrowthBook(): Promise<void> {
  ...
  stopPolling?.();
  stopPolling = startFeaturePolling(growthbook);
}
...
First add a deadline wrapper. Aborting requests relies on cooperative cancellation; racing against a rejecting timer also bounds the caller's wait when an adapter ignores the signal. The elapsed-time check rejects a result that arrives after the deadline even if the timer callback was delayed.

```ts title="src/llm/provider-deadline.ts"
import { performance } from "node:perf_hooks";

export class LlmTimeoutError extends Error {
  constructor() {
    super("LLM provider deadline exceeded");
...
export async function withProviderDeadline<T>(
  ...
  try {
    const pending = Promise.resolve().then(() => generate(controller.signal));
...
This bounds the asynchronous caller's wait, not the provider's resource use. An adapter must still forward the signal and configure its provider-native timeout to stop work and charges. Late fulfillment is ignored, and late rejection is handled by the race; neither becomes a second completion event. JavaScript cannot preempt a provider adapter that blocks the event loop synchronously.
...
import { LlmTimeoutError, withProviderDeadline } from "./provider-deadline";
...
export async function runSupportAnswer(
  ...
  try {
    const rawResult = await withProviderDeadline(
      (signal) => dependencies.provider.generate({
        variant,
    ...
    const result = validateGenerateResult(rawResult);
    ...
    logCompletion(gb, {
      treatment,
      status: error instanceof LlmTimeoutError
        ? "timeout"
...
import { InvalidLlmOutputError, type GenerateResult, type LlmProvider } from "../src/llm/provider";
import { LlmTimeoutError } from "../src/llm/provider-deadline";

vi.hoisted(() => {
  process.env.GROWTHBOOK_CLIENT_KEY = "sdk-test-no-network";
});
...
describe("runSupportAnswer", () => {
  ...
  it("times out an adapter that ignores cancellation", async () => {
    vi.useFakeTimers();
    const scope = scopeFor("prompt-v2");
    const events = vi.spyOn(scope, "logEvent").mockResolvedValue(undefined);
    let signal: AbortSignal | undefined;
    const pending = runSupportAnswer(baseInput, {
    ...
    const failure = expect(pending).rejects.toBeInstanceOf(LlmTimeoutError);
    await vi.advanceTimersByTimeAsync(25);
    await failure;
    expect(signal?.aborted).toBe(true);
    expect(events).toHaveBeenCalledWith("support_llm_completed", expect.objectContaining({ status: "timeout" }));
    expect(vi.getTimerCount()).toBe(0);
  });

  it("does not record a late result as a successful completion", async () => {
    vi.useFakeTimers();
    const scope = scopeFor("prompt-v2");
    const events = vi.spyOn(scope, "logEvent").mockResolvedValue(undefined);
    let finish!: (value: GenerateResult) => void;
    const pending = runSupportAnswer(baseInput, {
      provider: { generate: () => new Promise((resolve) => { finish = resolve; }) },
      estimateCostUsd: () => 0,
      createScope: () => scope,
      timeoutMs: 25,
    });
    ...
    await failure;
    finish({
      output: { answer: "late", needsEscalation: false, citedSnippetIds: [] },
      usage: { inputTokens: 1, outputTokens: 1 },
    });
    await vi.advanceTimersByTimeAsync(0);
    const completions = events.mock.calls.filter(([name]) => name === "support_llm_completed");
    expect(completions).toHaveLength(1);
    expect(completions[0][1]).toMatchObject({ status: "timeout" });
  });

  it("handles a late rejection without a second terminal event", async () => {
    vi.useFakeTimers();
    const scope = scopeFor("control");
    const events = vi.spyOn(scope, "logEvent").mockResolvedValue(undefined);
    let fail!: (error: Error) => void;
    const pending = runSupportAnswer(baseInput, {
      provider: { generate: () => new Promise((resolve, reject) => { fail = reject; }) },
      estimateCostUsd: () => 0,
      createScope: () => scope,
      timeoutMs: 25,
    });
    ...
    await vi.advanceTimersByTimeAsync(0);
    expect(events.mock.calls.filter(([name]) => name === "support_llm_completed")).toHaveLength(1);
  });

  it("classifies malformed normalized output", async () => {
    const scope = scopeFor("control");
    const events = vi.spyOn(scope, "logEvent").mockResolvedValue(undefined);
    await expect(runSupportAnswer(baseInput, {
      provider: { generate: async () => ({ output: { answer: 42 } } as unknown as GenerateResult) },
      estimateCostUsd: () => 0,
      createScope: () => scope,
    })).rejects.toBeInstanceOf(InvalidLlmOutputError);
    expect(events).toHaveBeenCalledWith("support_llm_completed", expect.objectContaining({ status: "invalid_output" }));
  });

  it("classifies an adapter's JSON/schema rejection", async () => {
    const scope = scopeFor("control");
    const events = vi.spyOn(scope, "logEvent").mockResolvedValue(undefined);
    await expect(runSupportAnswer(baseInput, {
      provider: { generate: async () => { throw new InvalidLlmOutputError(); } },
      estimateCostUsd: () => 0,
      createScope: () => scope,
    })).rejects.toBeInstanceOf(InvalidLlmOutputError);
    expect(events).toHaveBeenCalledWith("support_llm_completed", expect.objectContaining({ status: "invalid_output" }));
...
| Guardrail | Failed LLM attempt          | Proportion     | completed status in `provider_error`, `timeout`, `invalid_output`, or a dedicated failure event | Decrease            |
...
| Provider timeout                        | Reject at the deadline and request cancellation; emit one timeout guardrail; follow product fallback policy | Fake provider ignores cancellation and completes or rejects late |
...
With healthy feature delivery, roll back through GrowthBook without a deploy:

1. Open `support-llm-treatment`.
2. Stop or disable the experiment rule.
3. Force production to `control`.
4. Publish.
5. Allow for the 60-second polling interval plus fetch time. Verify fresh requests on each serving process receive `control` and the provider uses the control model/prompt; publishing alone is not a rollback receipt.
6. Monitor until in-flight treatment calls drain.

If feature delivery is unavailable or a process keeps evaluating a stale treatment, use the deployment-level rollback to a known-good control implementation. Do not rely on another flag delivered through the same failing connection. If the control path itself is unhealthy, disable the LLM capability at the product’s independent kill switch or route to the deterministic/human fallback. A feature experiment is not a substitute for a system-level safety control.
