...
Saving and publishing the experiment rule makes it available to the SDK. Users are not randomized until your application
evaluates the feature (for example with `isOn` or `getFeatureValue`) and they qualify. That is when `trackingCallback`
runs. 100% traffic means 100% of those evaluations, not 100% of all visitors. Results only count people who got that
exposure. There is a link to view experiment results on the bottom of each experiment override rule. Read more about
