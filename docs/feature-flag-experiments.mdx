...
qualify. That is when `trackingCallback` runs. If they are not included, you still get a value and `trackingCallback`
does not run.
...
Results only include people who got an exposure. There is a link to view experiment results on the bottom of each
experiment override rule. Read more about [experiment results](/app/experiment-results).
