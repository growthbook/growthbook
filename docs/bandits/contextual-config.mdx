...
description: "Checklist for running a contextual bandit: compatible SDK, tracking callback, and GrowthBook prerequisites."
sidebarTitle: Setup
slug: /bandits/contextual-config
---
...
`result.variationId` is the 0-based index of the assigned variation. Log that for `variation_id`, not `result.key`: a custom variation key is not matched during snapshot processing.
...
<Warning>
**`variation_id` must be the index or GrowthBook id**

`variation_id` must return the variation's 0-based index as a string (`'0'`, `'1'`, …) or the variation's GrowthBook id (the `id` field, typically `var_…`). Custom variation keys are not matched.

Rows with an unrecognized `variation_id` are dropped during snapshot processing. The symptom is a snapshot that succeeds in seconds with 0 users on every variation, even though the same SQL returns rows in the warehouse.

If the warehouse stores something else (a campaign id, SKU, etc.), map it in the query:

```sql
CASE CAST(campaign_id AS STRING)
  WHEN '19782307' THEN '0'
  WHEN '19782308' THEN '1'
  -- one branch per variation, in the bandit's variation order
END AS variation_id
```

The indexes must match the order of the variations on the bandit.

</Warning>

Requirements:

- `variation_id` must return the variation's 0-based index as a string (`'0'`, `'1'`, …) or the variation's GrowthBook id. Custom variation keys are not matched.
