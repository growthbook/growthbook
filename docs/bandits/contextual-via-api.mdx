...
The `key` values (`control`, `treatment`) are labels the SDK exposes as `result.key`. They are not matched during snapshot processing. Your assignment query's `variation_id` must still resolve to the variation's 0-based index or GrowthBook id, not these keys. See [`variation_id` must be the index or GrowthBook id](/bandits/contextual-config#4-create-a-contextual-bandit-assignment-query).
...
A Contextual Bandit serves its variations through a `contextual-bandit-ref` rule on a Feature Flag, and it needs at least one linked Feature Flag before it can start. Create the flag first with [`POST /v2/features`](/api/features-v2/operation/postFeatureV2), then link it to the contextual bandit:
...
Triggers the contextual bandit update. Each update runs the warehouse query and the stats engine and updates weights. Each successful update that changes weights increments the CB's `banditVersion`.
...
- `GET /v1/contextual-bandits/:id/events` — list bandit event outputs, one per successful snapshot (optional `?limit=`, max 100).
- `GET /v1/contextual-bandits/:id/events/:eventId` — single contextual bandit event.
