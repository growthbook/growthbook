...
The response includes the new query id (`cbq_…`). You can manage queries with the standard CRUD endpoints ([`GET /v1/contextual-bandit-queries`](/api/ContextualBanditQueries/operation/listContextualBanditQueries), [`GET`](/api/ContextualBanditQueries/operation/getContextualBanditQuery)/[`PUT`](/api/ContextualBanditQueries/operation/updateContextualBanditQuery)/[`DELETE`](/api/ContextualBanditQueries/operation/deleteContextualBanditQuery) `/v1/contextual-bandit-queries/:id`); pass `?datasourceId=ds_abc` on the list endpoint to scope by datasource.
