...
description: Run multi-armed bandits in GrowthBook to shift traffic toward the best variation on a single decision metric while reducing exposure to losers.
---
...
5. **What’s the difference between multi-armed bandit and contextual bandit experiments?**<br />
   GrowthBook uses multi-armed bandit experiments, which focus on optimizing traffic allocation among variations based purely on overall performance ([the Decision Metric](/bandits/config#5-selecting-a-decision-metric)). These experiments treat all users the same, without taking individual user characteristics into account.
