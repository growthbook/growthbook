...
Reach for a contextual bandit when the question you want answered isn't "which variation wins?" but "which variation wins **for whom**?" It's a good fit when:
...
- **Plain [targeting rules](/features/rules)** when you already know what each segment should get. If there's nothing left to learn, there's no need for a bandit.
...
4. **What happens if a context has very little traffic?**<br />
   GrowthBook controls how finely it splits users using settings like `minUsersPerLeaf` and `maxLeaves`. A segment without enough traffic to learn on its own isn't given its own leaf; it's pooled with similar users and shares their weights.
