...
description: How GrowthBook estimates CUPED-adjusted means and variances for experiment metrics, including the covariate model.
sidebarTitle: CUPED Technical
...
Due to randomization, $\mu_{MXC}$ equals $\mu_{DXC}$ and $\mu_{MXT}$ equals $\mu_{DXT}$, but for bookkeeping purposes, it is easier to have separate parameters.
