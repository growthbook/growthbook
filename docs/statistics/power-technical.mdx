...
description: How GrowthBook computes experiment power, sample size, and minimum detectable effect in the frequentist engine.
sidebarTitle: Power Technical
...
For our posterior approximation, this occurs if the posterior mean for $\Delta|\hat{\Delta}$ (i.e., $\Omega^{-1}\omega$) divided by its posterior standard deviation$\left(\text{i.e.,  }\sqrt{\Omega^{-1}}\right)$ is beyond the appropriate critical threshold $Z^{\star}$ (e.g., $\Phi^{-1}(0.975)$ for $\alpha=0.05$).
...
We assume that $\sigma_{\star}^{2}=0$ for simplicity and because large values of $\sigma_{\star}^{2}$ can result in negative MDEs (see [Bayesian minimum detectable effect](#bayesian-minimum-detectable-effect)).
...
<Frame caption="Figure 1">
  <img src="/static/images/statistics/power-decreasing.png" alt="Metric Icon showing CUPED Disabled Reason" width="500" />
</Frame>
