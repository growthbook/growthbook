---
title: Power
description: Estimate how long to run an A/B test with power analysis, including sample size, minimum detectable effect, and when to run a check.
...
- For each metric, enter the "Expected Effect Size" for each metric. Effect size is the percentage improvement in your metric (i.e., the lift) you anticipate your feature will cause. Inputting your effect size can require care. See [how to pick your effect size](#how-should-i-pick-my-effect-size).
- For each metric and for the users per week, we pre-populate the data with values from your selected population, but you can also customize these values, if you wish.

5. Now you have results! See [how to interpret power analysis results](#how-do-i-interpret-power-analysis-results).
...
<Frame>![Select a Fact Table population and a binomial metric in the power calculator.](/static/images/statistics/power_1.png)</Frame>
...
<Frame>![Selecting expected effect sizes and reviewing the power query results.](/static/images/statistics/power_2.png)</Frame>
...
<Frame>![metric and users input](/static/images/statistics/power_3.png)</Frame>
...
Ideally your experiment has [high power](#what-is-a-high-powered-experiment) across a range of effect sizes.
...
That being said, running an experiment with less than 80% power can still help your organization. The purpose of an experiment is to learn, not simply to roll out features that achieve statistically significant improvement. The biggest cost to running low-powered experiments is that your results will be noisy. This usually leads to ambiguity in the rollout decision.

## How do I run Bayesian power analysis?

For Bayesian power analysis, you specify the prior distribution of the treatment effect (see the [Bayesian engine](/statistics/details#bayesian-engine) for guidance regarding prior selection). We then estimate Bayesian power, which is the probability that the $(1 - \alpha)$ credible interval does not contain 0.
...
Each engine arrives at these values differently. Below we describe high-level technical details of our implementation. See [technical power details](/statistics/power-technical) for the full implementation.
...
$$
\begin{align}
\pi = 1 - \Phi\left(Z_{1-\alpha/2}-\frac{\Delta}{\hat{\sigma}_{\Delta}}\right) + \Phi(Z_{\alpha/2} - \frac{\Delta}{\hat{\sigma}_{\Delta}}). \tag{1}
\end{align}
$$
...
Inverting this equation requires some care, as the uncertainty estimate $\hat{\sigma}_{\Delta}$ depends upon $\Delta$. See [technical power details](/statistics/power-technical).
...
For Bayesian power analysis, we let users specify the prior distribution of the treatment effect (see the [Bayesian engine](/statistics/details#bayesian-engine) for guidance regarding prior selection). We then estimate Bayesian power, which is the probability that the $(1 - \alpha)$ credible interval does not contain 0.
