...
A number of GrowthBook features make HTTP requests to a URL that a user configured in the app: webhooks (including Slack and Discord notifications), data source connections, imports from other platforms, and custom hooks. These requests originate from the GrowthBook container, so anyone with permission to create them can potentially reach hosts that are not otherwise exposed, such as services on your private subnet or your cloud provider's instance metadata endpoint. This is a form of Server-Side Request Forgery (SSRF).

We recommend sending this traffic through an egress proxy that denies requests to internal IP ranges, such as [Smokescreen](https://github.com/stripe/smokescreen). Set the **WEBHOOK_PROXY** environment variable to the proxy's address (e.g. `http://smokescreen:4750`) and GrowthBook will route its outgoing requests through it. Standard `http_proxy` / `https_proxy` environment variables are also respected.

As additional layers, restrict outbound network access for the GrowthBook container at the infrastructure level, and limit which users have permission to create webhooks and data sources.
