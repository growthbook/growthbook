---
title: Production
description: "Harden a self-hosted GrowthBook install for production: HTTPS, secrets, backups, and resource limits."
slug: production
---
...
docker compose run --workdir /usr/local/src/app/packages/back-end \
  growthbook node dist/scripts/migrate-encryption-key.js OLD_KEY
```
...
Background jobs run on the instances where `CRON_DISABLED` is not set. They are split across two queues so that short, high-volume work does not get stuck behind long-running jobs:

- The **event queue** handles event fan-out and webhook delivery, which normally finish in milliseconds.
- The **default queue** handles everything else, including experiment result refreshes that can run for tens of minutes.

Each queue polls MongoDB on its own interval, and each job type has its own limit on how many jobs one instance may claim at a time (its lock limit) and run at a time (its concurrency). Raising the lock limit lets an instance drain a backlog without waiting for the next poll. The trade-off is that if that instance crashes hard, the jobs it had already claimed stay locked until their lock lifetime expires.

| Variable                              | Default                        | Description                                                                            |
| ------------------------------------- | ------------------------------ | -------------------------------------------------------------------------------------- |
| `GB_AGENDA_DEFAULT_LOCK_LIMIT`        | `5`                            | Jobs of one type an instance may claim at a time, for every job without its own limit. |
| `GB_AGENDA_EVENT_POLL_INTERVAL_MS`    | `5000`                         | How often the event queue checks MongoDB for new work. Accepts 100 to 60000.           |
| `GB_AGENDA_EVENT_CREATED_CONCURRENCY` | `5`                            | Event fan-out jobs an instance runs at a time.                                         |
| `GB_AGENDA_EVENT_CREATED_LOCK_LIMIT`  | `GB_AGENDA_DEFAULT_LOCK_LIMIT` | Event fan-out jobs an instance claims at a time.                                       |
| `GB_AGENDA_EVENT_WEBHOOK_CONCURRENCY` | `5`                            | Webhook deliveries an instance has in flight at a time.                                |
| `GB_AGENDA_EVENT_WEBHOOK_LOCK_LIMIT`  | `GB_AGENDA_DEFAULT_LOCK_LIMIT` | Webhook delivery jobs an instance claims at a time.                                    |

The defaults match GrowthBook's behavior before the queues were split, so you only need to change them if webhook delivery falls behind. The lock limit is usually the more effective knob; setting it below the matching concurrency will cap that concurrency.
...
To enable, you must set the environment variable `TRACING_PROVIDER=opentelemetry`
