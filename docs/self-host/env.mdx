...
Skills tell the in-app AI Assistant how to carry out a task with the GrowthBook REST API. GrowthBook ships built-in skills for Feature Flags, experiments, analytics and the docs. You can add your own, replace built-ins, or turn built-ins off.

- **AGENT_SKILLS_DIR** - Path to a directory of custom skills. Each skill is a folder containing a `SKILL.md` file, with optional workflow files in a `references/` subfolder:
  ...
  Each file starts with YAML frontmatter holding a `name` and a `description`. The Assistant sees every skill's description, then loads a skill's full instructions when a request matches it. Users can also pick a skill from the `/` menu in the chat composer. A custom skill with the same name as a built-in replaces the built-in along with all of its workflows.

- **AGENT_SKILLS_DISABLE_BUILTINS** - `true` to turn off every built-in skill, or a comma-separated list of built-in skill names to turn off, such as `experiments,analytics`.

GrowthBook checks `AGENT_SKILLS_DIR` for changes every 30 seconds, so edits to your skills take effect without a restart. Changing either environment variable still needs a restart. With Docker, mount the directory into the container:
...
Admins can also turn individual skills on or off for their organization under **Settings → General → AI Settings**. Built-ins turned off with `AGENT_SKILLS_DISABLE_BUILTINS` don't appear there.

Skills can only guide the Assistant. It calls the REST API with the signed-in user's permissions, asks for confirmation before any change, and never runs scripts that a skill includes. These settings have no effect on GrowthBook Cloud.
