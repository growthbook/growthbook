...
description: Set environment variables for a self-hosted GrowthBook deployment, including the MongoDB connection and app origin.
slug: env
---
...
Skills tell the in-app AI Assistant how to carry out a task with the GrowthBook REST API. GrowthBook ships built-in skills for Feature Flags, experiments, analytics and the docs, and you can add your own or replace the built-ins.

- **AGENT_SKILLS_DIR** - Path to a directory of custom skills. Each skill is a folder containing a `SKILL.md` file, with optional workflow files in a `references/` subfolder and any other files the skill needs:
  ...
  skills/
    release-checklist/
      ...
      examples/
        payload.json
    feature-flags/          # same name as a built-in, so it replaces it
      SKILL.md
  ```

`SKILL.md` and each workflow file start with YAML frontmatter holding a `name` and a `description`. A skill's `name` must be lowercase letters, digits, hyphens and underscores; one that isn't is skipped with a warning in the server logs.

The Assistant sees every skill's description, then loads a skill's full instructions when a request matches it. Users can also pick a workflow from the `/` menu in the chat composer. Other files in a skill's folder, such as `examples/payload.json` above, are loaded when `SKILL.md` or a workflow refers to them by that relative path. The Assistant can read scripts but can't run them, so it carries out what a script does with REST API calls. A custom skill with the same name as a built-in replaces the built-in along with all of its files.

GrowthBook checks `AGENT_SKILLS_DIR` for changes every 30 seconds, so edits to your skills take effect without a restart. Use real folders inside the directory: changes inside a symlinked skill folder are only picked up when the link itself changes. With Docker, mount the directory into the container:

```yaml
volumes:
  - ./skills:/usr/local/src/app/skills
environment:
  - AGENT_SKILLS_DIR=/usr/local/src/app/skills
```

To turn off a built-in or custom skill for an organization, see [AI Assistant skills](/integrations/ai#ai-assistant-skills).

Skills only guide the Assistant. It calls the REST API with the signed-in user's permissions and asks for confirmation before any change. `AGENT_SKILLS_DIR` has no effect on GrowthBook Cloud.
...
If you are running your application behind a firewall you will need to allow outbound traffic to both `75.2.109.47` and `99.83.189.109` to be able to have your
license verified — the license server is served from two anycast addresses and traffic can use either. See [IP addresses](/ip-addresses). If this is an issue,
...
Enable GrowthBook as an OAuth 2.1 Authorization Server so remote MCP clients (and future CLI login) can sign users in via the dashboard session instead of pasting an API key.

- **OAUTH_AS_ENABLED** - Set to `1` or `true` to expose the Authorization Server endpoints. Default off.
- **OAUTH_ISSUER** - Optional. Issuer identifier advertised to clients (RFC 8414). Defaults to `API_HOST` when unset. Must be a stable absolute URL.
- **OAUTH_ACCESS_TOKEN_TTL_SECONDS** - Access token lifetime in seconds (default `3600`).
- **OAUTH_REFRESH_TOKEN_TTL_SECONDS** - Refresh token lifetime in seconds (default `2592000` / 30 days).

When running the [official MCP Docker image](/integrations/mcp#option-b-docker) against your API, set `OAUTH_AS_ENABLED=1` on the API so clients can complete the OAuth consent flow.
...
- **KEEP_ALIVE_TIMEOUT_MS** - How long (in milliseconds) the API server holds an idle keep-alive connection open before closing it (default `3605000`, just over an hour). This must be longer than the idle timeout of any load balancer or reverse proxy in front of GrowthBook. If it is shorter, the proxy can reuse a connection GrowthBook has already closed, and the request fails with a 502 that never reaches your application logs.
