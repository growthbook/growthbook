...
- **WEBHOOK_PROXY** - Route all outgoing HTTP requests (webhooks, data source connections, imports, custom hooks) through this proxy. Recommended in production, see [Outbound Requests](/self-host/production#outbound-requests). The standard `http_proxy` / `https_proxy` variables are also respected when this is not set.
