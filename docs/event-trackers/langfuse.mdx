...
- Fact table `Langfuse Scores` (one row per eval score, annotation, or piece of user feedback). `score_name` is an inline-filter column, so create a mean metric on `score_value` filtered by the score name to get, for example, average helpfulness. No score metrics are generated because score names are user-defined.
